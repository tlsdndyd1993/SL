...
def _draw_memo_block(frame: np.ndarray, lines: list[str],
    ...
    """
    ★ v8: PIL 기반 렌더링 → 한글/유니코드 완전 지원.
    font_size 는 픽셀 단위 (기본 18px ≈ 기존 0.52 scale).
    """
    if not lines: return
    fnt = _get_font(font_size)
    pad = 8
    ...
    for line in lines:
        try:
            ...
            tw = (bb[2] - bb[0]) if bb else len(line) * font_size // 2
        except Exception:
            tw = len(line) * font_size // 2
        max_tw = max(max_tw, tw)

    box_w = min(max_tw + 20, fw - 20)
    ...
    for j, line in enumerate(lines):
        ...
        _put_unicode_text(frame, line, x0, cy, font_size, (100, 240, 255))  # BGR yellow
