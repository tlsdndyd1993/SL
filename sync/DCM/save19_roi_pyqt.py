...
class RecorderEngine:
    ...
    def select_camera(self, idx: int):
        """카메라를 백그라운드 스레드에서 안전하게 전환."""
        cam = next((c for c in self.camera_list if c["idx"] == idx), None)
        if not cam:
            return
        # UI 스레드를 블로킹하지 않도록 백그라운드에서 전환
        threading.Thread(target=self._switch_camera, args=(idx, cam), daemon=True).start()

    def _switch_camera(self, idx: int, cam: dict):
        """
        ① 기존 스레드에 stop 신호 → join(최대 3초 대기)으로 완전 종료 확인
        ② active_camera_idx / actual_camera_fps 업데이트
        ③ 새 스레드 시작
        """
        # 1. 기존 스레드 종료 신호
        self._camera_stop.set()

        # 2. 기존 스레드가 실제로 끝날 때까지 대기 (최대 3초)
        if self._camera_thread and self._camera_thread.is_alive():
            self._camera_thread.join(timeout=3.0)
            if self._camera_thread.is_alive():
                self.signals.status_message.emit(
                    "[Warning] 이전 카메라 스레드가 제때 종료되지 않았습니다.")
    ...
    def _camera_loop(self):
        ...
        # 백엔드가 지원하면 read 타임아웃 설정 (CAP_PROP_OPEN_TIMEOUT_MSEC)
        # → cap.read()가 최대 500 ms 안에 리턴되어 stop 신호를 빠르게 감지
        ...
        while not self._camera_stop.is_set():
            now = time.perf_counter()
            if now < next_t:
                # stop 신호를 짧게 쪼개서 확인 (최대 50 ms씩 대기)
                wait = next_t - now
                slept = 0.0
                while slept < wait and not self._camera_stop.is_set():
                    chunk = min(0.05, wait - slept)
                    time.sleep(chunk)
                    slept += chunk
            ...
            ret, frame = cap.read()
            if not ret:
                # read 실패: stop 신호 확인 후 짧게 대기하고 재시도
                if self._camera_stop.is_set():
                    break
                time.sleep(0.01)
                continue
    ...
    def start_camera_thread(self):
        """카메라 캡처 스레드를 시작합니다.
        이미 살아있는 스레드가 있으면 먼저 stop 후 새로 시작합니다."""
        if self._camera_thread and self._camera_thread.is_alive():
            # 기존 스레드가 아직 살아 있으면 stop 신호 후 join
            self._camera_stop.set()
