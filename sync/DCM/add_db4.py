...
def _find_unicode_font(size: int = 18):
    """
    ★ v9 수정: 플랫폼별 가장 확실한 방법으로 한글 폰트를 탐색합니다.

    탐색 순서:
      Windows  → winreg 레지스트리 → WINDIR 환경변수 폴더 → 고정 경로
      macOS    → /System/Library/Fonts + /Library/Fonts glob
      Linux    → fc-list :lang=ko → glob (Noto/Nanum/Un폰트)
    """
    ...
    # =========================================================================
    if _sys == "Windows":
        # ── 1순위: winreg 레지스트리에서 실제 설치 경로 조회 ─────────────────
        try:
            import winreg
            font_dir_reg = None
            # 시스템 폰트 폴더 경로
        ...
        # ── 2순위: WINDIR/Fonts 폴더 직접 탐색 ──────────────────────────────
        win_fonts_dir = os.path.join(
            os.environ.get("WINDIR", "C:\\Windows"), "Fonts")
        for fname in [
            "malgun.ttf", "malgunbd.ttf",   # 맑은 고딕 (Vista+, 가장 보편)
    ...
    # =========================================================================
        ...
        # ── 1순위: fc-list :lang=ko 로 실제 설치된 한국어 폰트 조회 ──────────
        try:
            result = _sp.run(
    ...
    # =========================================================================
    # 공통 fallback: DejaVu (ASCII 전용, 한글 미지원이지만 최후 수단)
    # =========================================================================
    candidates += [
    ...
    # =========================================================================
    # 실제 로드 시도 — 한글 렌더링 가능 여부도 검증
    # =========================================================================
    _test_text = "가나다"   # 한글 렌더링 검증용
    ...
    for path in candidates:
        ...
        try:
            ...
            # ★ 실제로 한글을 렌더링할 수 있는지 검증
            try:
                # getbbox 또는 getmask로 실제 글리프 크기 확인
                bbox = fnt.getbbox(_test_text)
                if bbox and (bbox[2] - bbox[0]) > 5:   # 너비가 5px 이상이면 OK
                    return fnt
                # bbox 너비가 너무 작으면 대체 문자(□)로 렌더된 것 → 스킵
                ...
                try:
                    ...
                    return fnt   # 검증 불가 시 일단 사용
            except Exception:
                return fnt   # 검증 실패 시 일단 사용

        except Exception:
            continue

    # 최후 수단: PIL 기본 폰트 (한글 깨짐)
    try:
...
def check_korean_font() -> tuple[bool, str]:
    """
    시작 시 한글 폰트 사용 가능 여부를 확인합니다.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ★ 시작 직후 한글 폰트 진단 (비동기 — UI 준비 후 로그에 출력)
    ...
    def _check_font_on_startup(self):
        """★ 시작 시 한글 폰트 사용 가능 여부를 진단하고 로그에 출력합니다."""
        ...
        if ok:
            ...
            box.setText(
                "<b>메모 오버레이에 한글이 ???로 표시될 수 있습니다.</b><br><br>"
                + msg.replace("\n", "<br>") +
                "<br><br>"
                "<b>해결 방법 (Windows):</b><br>"
                "1. 제어판 → 글꼴 → '맑은 고딕' 확인<br>"
                "2. 또는 <a href='https://naver.github.io/nanumfont/'>나눔고딕</a> 설치<br>"
                "3. pip install Pillow 로 Pillow 최신 버전 설치")
