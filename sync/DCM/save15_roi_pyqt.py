...
class RecorderEngine:
    def __init__(self, signals: Signals):
        ...
        # Playback speed (영상 저장 배속): 1.0=정배속, 2.0=2배속(빠르게), 0.5=0.5배속(느리게)
...
class FeatureBar(QFrame):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setStyleSheet(
            "QFrame{background:#0a0a1a;border-bottom:2px solid #2a3a5a;}")

        outer = QVBoxLayout(self)
        outer.setContentsMargins(8, 6, 8, 6)
        outer.setSpacing(4)

        title_lbl = QLabel("⚙  표시할 기능 선택")
        title_lbl.setStyleSheet(
            "color:#7ab4d4; font-size:12px; font-weight:bold; padding:0;")
        ...
        scroll.setStyleSheet(
            ...
            "QScrollBar:horizontal{background:#0d0d1e;height:6px;border-radius:3px;}"
            "QScrollBar::handle:horizontal{background:#336;border-radius:3px;min-width:20px;}"
        ...
        for key, text in self.FEATURES:
            ...
            cb.setChecked(True)
            cb.setStyleSheet(
                "QCheckBox{font-size:12px;color:#dde;spacing:5px;padding:2px 4px;}"
