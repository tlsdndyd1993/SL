...
class RecorderEngine:
    ...
    def macro_start_recording(self):
        """클릭 기록 모드 시작.
        200 ms 지연 후 리스너를 시작해 '기록 시작' 버튼 클릭 자체가 기록되지 않도록 함.
        클릭 이벤트는 pyqtSignal(macro_step_recorded)로 Qt 메인 스레드에 안전하게 전달.
        """
        ...
        def _delayed_start():
            time.sleep(0.25)          # 버튼 클릭 이벤트가 지나갈 때까지 대기
            if not self.macro_recording:
                return                # 그 사이 취소됐으면 무시
            self._macro_last_ts = time.time()
...
class MainWindow(QMainWindow):
    ...
    def _on_macro_rec_toggle(self, recording: bool):
        if recording:
            ...
            self.engine.macro_start_recording()   # 콜백 없이 시그널로
        else:
            self._macro_rec_btn.setText("⏺  기록 시작")
            self._macro_rec_status.setText("● 대기")
            self._macro_rec_status.setStyleSheet(
                "color:#888;font-size:11px;font-weight:bold;")
    ...
    def _on_macro_step_signal(self, x: int, y: int, delay: float):
        ...
        self._append_macro_row(step)

    def _append_macro_row(self, step):
