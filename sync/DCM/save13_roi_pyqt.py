...
from PyQt5.QtWidgets import (
    ...
    QSizePolicy, QDialog, QLCDNumber, QDateTimeEdit, QRadioButton,
    QButtonGroup, QMessageBox
)
from PyQt5.QtCore import (
    Qt, QTimer, pyqtSignal, QObject, QPoint, QRect, QDateTime
)
from PyQt5.QtCore import (
    Qt, QTimer, pyqtSignal, QObject, QPoint, QRect, QDateTime
)
...
class RecorderEngine:
    def __init__(self, signals: Signals):
        ...
        self.schedule_start_dt: datetime = None   # None = not set
        self.schedule_stop_dt:  datetime = None   # None = not set
        self._schedule_started  = False   # guard: started by scheduler
        self._schedule_stopped  = False   # guard: stopped by scheduler
    ...
    # ── Schedule tick (called from UI timer every 1 s) ───
    def schedule_tick(self):
        """Check schedule and return (action, secs_to_start, secs_to_stop).
        ...
        secs_to_start = (self.schedule_start_dt - now).total_seconds() \
                        if self.schedule_start_dt else None
        secs_to_stop  = (self.schedule_stop_dt  - now).total_seconds() \
                        if self.schedule_stop_dt  else None

        action = None
        if (self.schedule_start_dt and not self._schedule_started
                and secs_to_start is not None and -2 <= secs_to_start <= 1):
            if not self.recording:
                action = 'start'
            self._schedule_started = True

        if (self.schedule_stop_dt and not self._schedule_stopped
                and secs_to_stop is not None and -2 <= secs_to_stop <= 1):
            if self.recording:
                action = 'stop'
            self._schedule_stopped = True

        return action, secs_to_start, secs_to_stop

    def reset_schedule_guards(self):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Schedule ticker — every 1 second
        self._sched_timer = QTimer(self)
        self._sched_timer.timeout.connect(self._tick_schedule)
    ...
    def _build_ui(self):
        ...
        panel_layout.addWidget(self._build_schedule_group())
    ...
    def _build_schedule_group(self):
        grp = QGroupBox("⏰  Schedule (예약 녹화)")
        v   = QVBoxLayout(grp)
        v.setSpacing(8)

        # Enable toggle
        self._sched_chk = QCheckBox("예약 녹화 활성화")
        self._sched_chk.setStyleSheet("font-weight:bold; color:#f0c040;")
        self._sched_chk.toggled.connect(self._on_schedule_toggle)
        v.addWidget(self._sched_chk)

        # ── Start datetime ────────────────────
        start_grp = QGroupBox("🟢  녹화 시작")
        sg = QVBoxLayout(start_grp)
        sg.setSpacing(4)

        # Enable start schedule
        self._sched_start_chk = QCheckBox("시작 시각 설정")
        self._sched_start_chk.setChecked(True)
        self._sched_start_chk.toggled.connect(self._on_sched_start_chk)
        sg.addWidget(self._sched_start_chk)
        ...
        self._sched_start_dt.setDateTime(
        ...
        self._sched_start_dt.setStyleSheet(
            ...
            "border-radius:4px; padding:4px; font-size:12px; font-family:monospace;")
        sg.addWidget(self._sched_start_dt)

        btn_now_start = QPushButton("📌 현재 시각으로 설정")
        btn_now_start.setFixedHeight(24)
        btn_now_start.clicked.connect(
            lambda: self._sched_start_dt.setDateTime(
                QDateTime.currentDateTime()))
        sg.addWidget(btn_now_start)
        v.addWidget(start_grp)

        # ── Stop datetime ─────────────────────
        stop_grp = QGroupBox("🔴  녹화 종료")
        stg = QVBoxLayout(stop_grp)
        stg.setSpacing(4)

        self._sched_stop_chk = QCheckBox("종료 시각 설정")
        self._sched_stop_chk.setChecked(True)
        self._sched_stop_chk.toggled.connect(self._on_sched_stop_chk)
        stg.addWidget(self._sched_stop_chk)
        ...
        self._sched_stop_dt.setDateTime(
            QDateTime.currentDateTime().addSecs(3660))  # +1h 1m
        self._sched_stop_dt.setStyleSheet(
            "background:#1a1a3a; color:#e74c3c; border:1px solid #6a2a2a; "
            "border-radius:4px; padding:4px; font-size:12px; font-family:monospace;")
        stg.addWidget(self._sched_stop_dt)

        btn_now_stop = QPushButton("📌 현재 시각으로 설정")
        btn_now_stop.setFixedHeight(24)
        btn_now_stop.clicked.connect(
            lambda: self._sched_stop_dt.setDateTime(
                QDateTime.currentDateTime()))
        stg.addWidget(btn_now_stop)
        v.addWidget(stop_grp)

        # ── Apply / Cancel ────────────────────
        btn_row = QHBoxLayout()
        self._btn_sched_apply = QPushButton("✅  예약 적용")
        self._btn_sched_apply.setStyleSheet(
            "background:#1a5a3a; color:#afffcf; border:1px solid #2a8a5a; "
            "border-radius:4px; padding:5px; font-weight:bold;")
        self._btn_sched_apply.clicked.connect(self._on_schedule_apply)

        self._btn_sched_cancel = QPushButton("✖  취소")
        self._btn_sched_cancel.setStyleSheet(
            "background:#3a1a1a; color:#ffaaaa; border:1px solid #7a2a2a; "
            "border-radius:4px; padding:5px;")
        self._btn_sched_cancel.clicked.connect(self._on_schedule_cancel)

        btn_row.addWidget(self._btn_sched_apply)
        btn_row.addWidget(self._btn_sched_cancel)
        v.addLayout(btn_row)

        # ── Countdown display ─────────────────
        cd_grp = QGroupBox("카운트다운")
        cdl = QVBoxLayout(cd_grp)

        self._sched_status_lbl = QLabel("● 예약 없음")
        self._sched_status_lbl.setStyleSheet(
            "color:#888; font-size:11px; font-weight:bold;")
        cdl.addWidget(self._sched_status_lbl)

        self._sched_start_cd_lbl = QLabel("시작까지:  —")
        self._sched_start_cd_lbl.setStyleSheet(
            "color:#2ecc71; font-family:monospace; font-size:11px;")
        cdl.addWidget(self._sched_start_cd_lbl)

        self._sched_stop_cd_lbl = QLabel("종료까지:  —")
        self._sched_stop_cd_lbl.setStyleSheet(
            "color:#e74c3c; font-family:monospace; font-size:11px;")
        cdl.addWidget(self._sched_stop_cd_lbl)

        v.addWidget(cd_grp)

        # Initially grey-out sub-controls
        grp.setEnabled(True)
        return grp
    ...
    #  Schedule handlers
    # ─────────────────────────────────────────
    def _on_schedule_toggle(self, checked: bool):
        self.engine.schedule_enabled = checked
        if not checked:
            self.engine.reset_schedule_guards()
            self._sched_status_lbl.setText("● 예약 없음")
            self._sched_status_lbl.setStyleSheet("color:#888; font-size:11px; font-weight:bold;")
            self._sched_start_cd_lbl.setText("시작까지:  —")
            self._sched_stop_cd_lbl.setText("종료까지:  —")

    def _on_sched_start_chk(self, checked: bool):
        self._sched_start_dt.setEnabled(checked)

    def _on_sched_stop_chk(self, checked: bool):
        self._sched_stop_dt.setEnabled(checked)

    def _on_schedule_apply(self):
        # Validate
        start_dt = None
        stop_dt  = None
        ...
        if self._sched_start_chk.isChecked():
            ...
            start_dt = datetime(qdt.date().year(), qdt.date().month(),
        ...
        if self._sched_stop_chk.isChecked():
            ...
            stop_dt = datetime(qdt.date().year(), qdt.date().month(),
                               qdt.date().day(), qdt.time().hour(),
                               qdt.time().minute(), qdt.time().second())
        ...
        if start_dt and start_dt < now:
            QMessageBox.warning(self, "설정 오류",
                "녹화 시작 시각이 현재 시각보다 이전입니다.")
            return
        if stop_dt and stop_dt < now:
            QMessageBox.warning(self, "설정 오류",
                "녹화 종료 시각이 현재 시각보다 이전입니다.")
            return
        ...
        # Apply
        self.engine.schedule_start_dt = start_dt
        self.engine.schedule_stop_dt  = stop_dt
        self.engine.reset_schedule_guards()

        # Auto-enable schedule
        self._sched_chk.setChecked(True)
        self.engine.schedule_enabled = True

        parts = []
        if start_dt:
            parts.append(f"시작: {start_dt.strftime('%Y-%m-%d %H:%M:%S')}")
        if stop_dt:
            parts.append(f"종료: {stop_dt.strftime('%Y-%m-%d %H:%M:%S')}")
        self._append_log(f"[Schedule] 예약 적용됨 — {' | '.join(parts)}")
        self._sched_status_lbl.setText("● 예약 대기 중")
        self._sched_status_lbl.setStyleSheet(
            "color:#f0c040; font-size:11px; font-weight:bold;")

    def _on_schedule_cancel(self):
        self.engine.schedule_enabled   = False
        self.engine.schedule_start_dt  = None
        self.engine.schedule_stop_dt   = None
        self.engine.reset_schedule_guards()
        self._sched_chk.setChecked(False)
        self._sched_status_lbl.setText("● 예약 없음")
        self._sched_status_lbl.setStyleSheet("color:#888; font-size:11px; font-weight:bold;")
        self._sched_start_cd_lbl.setText("시작까지:  —")
        self._sched_stop_cd_lbl.setText("종료까지:  —")
        self._append_log("[Schedule] 예약 취소됨")

    def _tick_schedule(self):
        action, s_start, s_stop = self.engine.schedule_tick()
        ...
        if self.engine.schedule_enabled:
            if self.engine.schedule_start_dt:
                self._sched_start_cd_lbl.setText(f"시작까지:  {_fmt(s_start)}")
            if self.engine.schedule_stop_dt:
                self._sched_stop_cd_lbl.setText(f"종료까지:  {_fmt(s_stop)}")

        # Trigger actions on main thread
        if action == 'start':
            self._append_log("[Schedule] ⏺ 예약 녹화 시작!")
            self._on_start_rec()
            self._sched_status_lbl.setText("● 녹화 중 (예약)")
            self._sched_status_lbl.setStyleSheet(
                "color:#2ecc71; font-size:11px; font-weight:bold;")
        elif action == 'stop':
            self._append_log("[Schedule] ⏹ 예약 녹화 종료!")
            self._on_stop_rec()
            self._sched_status_lbl.setText("● 예약 완료")
            self._sched_status_lbl.setStyleSheet(
                "color:#888; font-size:11px; font-weight:bold;")
