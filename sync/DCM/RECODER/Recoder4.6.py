    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame, zoom_hint: float = 1.0):
            ...
            try:
                ...
                # ★ v4.5 psm 구성:
                #   psm 10: 단일 문자 전용 — 소형 ROI 핵심 (신규)
                #   psm 8 : 단어 단위
                #   psm 7 : 단일 텍스트 행
                #   psm 6 : 균일 블록 (다자리)
                #   psm 13 제거: 노이즈에 취약, 극소 ROI 오탐 원인
                DIGIT_CFGS = [
                    f"--psm 10 --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 8 --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 7 --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 6 --oem 3 -c tessedit_char_whitelist={WL}",
                ]
                ...
                for img_bin, tag in cands:
                    ...
                    for cfg in DIGIT_CFGS:
                        try:
                            ...
                            if parts:
                                # ★ 다자리 수: 최대 4자까지만 join (오탐 방지)
                                joined   = "".join(parts[:4])
                                avg_conf = conf_sum / conf_cnt if conf_cnt else 0
                                # ★ v4.5: avg_conf를 _score에 전달
                                sc       = _score(joined, avg_conf)
                                if sc > best_score:
                                    best_score      = sc
