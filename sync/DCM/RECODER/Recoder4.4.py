...
class CoreEngine:
    ...
    def _save_bo_clip(self, source: str, timestamp: str):
        ...
        if self.tc_blackout_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            tag_dir = self.tc_tag_target_dir if self.tc_tag_target_dir else save_dir
    ...
    def stop_recording(self):
        ...
        if self.tc_rec_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            # ★ 수정: tc_tag_target_dir이 미설정이거나 output_dir 기준 상위 폴더인 경우
            #   output_dir을 기준으로 폴더 태깅 후 output_dir도 함께 갱신
            tag_dir = self.tc_tag_target_dir if self.tc_tag_target_dir else self.output_dir
            new_path = self._apply_tc_result_to_folder(self.tc_verify_result, tag_dir)
            # ★ 수정: 태깅 대상이 output_dir 자신이거나, output_dir이 tag_dir 하위인 경우 모두 반영
            if new_path and new_path != tag_dir:
                if tag_dir == self.output_dir:
                    # 직접 녹화 폴더가 rename된 경우
                    self.output_dir = new_path
                elif self.output_dir.startswith(tag_dir + os.sep):
                    # 상위 폴더가 rename된 경우 — output_dir의 하위 경로 유지
                    rel = os.path.relpath(self.output_dir, tag_dir)
                    self.output_dir = os.path.join(new_path, rel)
            # ★ 수정: tc_tag_target_dir은 다음 녹화 시작 시 재계산되므로 초기화
            self.tc_tag_target_dir = ""
            self.tc_verify_result = ""
            if new_path:
                self.signals.status_message.emit(
                    f"[T/C] 최종 저장 경로 → {new_path}")
        self.start_time = 0.0
    ...
    def _do_manual(self, source: str):
        ...
        with self._buf_lock:
            ...
            # ★ 수정: pre 수집 시점의 버퍼 전체 크기를 스냅샷으로 기록.
            #   버퍼는 maxlen 없는 deque로 popleft()로 크기를 유지하므로,
            #   트리거 시점의 총 프레임 수를 기억해두고
            #   이후 버퍼가 커진 만큼만 post로 추출.
            snap_len = len(buf)
        ...
        while len(post) < n_post and time.time() < deadline:
            ...
            new_frames = cur_len - snap_len
            if new_frames > 0:
                # 버퍼가 덜 찬 경우: 트리거 이후 추가된 프레임만 추출
                with self._buf_lock:
                    buf = self._scr_buf if source == "screen" else self._cam_buf
                    post = list(buf)[-new_frames:]
            elif cur_len >= snap_len and cur_len > 0:
                # 버퍼가 꽉 차서 popleft로 크기 유지 중인 경우:
                # 버퍼 끝 n_post 프레임이 트리거 이후 새 프레임임
                with self._buf_lock:
                    buf = self._scr_buf if source == "screen" else self._cam_buf
                    post = list(buf)[-n_post:]
        ...
        if self.tc_manual_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            tag_dir = self.tc_tag_target_dir if self.tc_tag_target_dir else save_dir
            new_path = self._apply_tc_result_to_folder(self.tc_verify_result, tag_dir)
            # ★ 수정: save_dir이 태깅된 경우 상태 메시지에 최종 경로 표시
            if new_path and new_path != tag_dir:
                self.signals.status_message.emit(
                    f"[수동녹화/T/C] 최종 저장 경로 → {new_path}")
...
class PathSettingsPanel(QWidget):
    ...
    def get_tc_target_folder(self) -> str:
        """
        ★ 수정: output_dir(이전 녹화 경로) 기반 탐색을 완전히 제거하고,
          _build_path와 동일한 구조로 경로를 직접 계산.

          이전 방식의 문제:
            output_dir이 '(PASS) 001/Rec_...' 상태일 때 os.path.dirname()으로
            부모를 구하면 '(PASS) 001'이 나오고, 거기에 새 tc_id를 붙여
            '(PASS) 001/002' 경로가 만들어짐 — 폴더 생성 여부와 무관하게
            tc_tag_target_dir 자체가 잘못된 경로가 됨.

          수정 방식:
            base_dir / [vehicle_type] / 날짜 / [tc_id] 구조로 직접 계산.
            폴더를 절대 미리 생성하지 않음 (녹화 시작 시 os.makedirs 호출됨).
        """
        ...
        if idx == 0:
            ...
            if not vt:
                return eng.base_dir
            return os.path.join(eng.base_dir, vt)
        elif idx == 2:
            # 녹화 폴더 자체 (output_dir): 녹화 시작 전이면 빈 문자열
    ...
    def _upd_tc_id(self):
        ...
        #   (녹화 시작 시 _resync_tc_tag_target()에서 현재 tc_id 기준으로 재계산됨)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  예시: 매크로 3회 반복 실행
    engine.macro_start_run(repeat=3, gap=2.0)
    while not kernel.is_stopped():
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  텍스트 파일 형식:
    ...
    ─────────────────────────────────────────────────────────────
    TC_001_무드램프ON    1/3      PASS        2026-04-13 10:00:05
    TC_001_무드램프ON    2/3      FAIL        2026-04-13 10:00:35
    TC_001_무드램프ON    3/3      PASS        2026-04-13 10:01:05
...
class MainWindow(QMainWindow):
    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame, zoom_hint: float = 1.0):
            ...
            MIN_DIM = 400   # 최소 픽셀 (작은 숫자 대응 핵심값)
            MAX_DIM = 1200  # 메모리 방어선

            def _force_upscale(img):
                """물리 픽셀 기준 강제 확대 — zoom_hint 무관."""
                h, w = img.shape[:2]
                if h <= 0 or w <= 0:
                    return img
                # 종횡비가 극단(가로/세로 > 8)이면 위아래 패딩 먼저
                if h > 0 and w / h > 8:
                    pad = int(w / 8) - h
                    val = img[0,0].tolist() if img.ndim == 3 else int(img[0,0])
                    img = cv2.copyMakeBorder(
            ...
            def _make_candidates(bgr):
                """
                전처리 후보 생성 — 총 6가지 이진화 조합.
                작은 ROI를 물리 픽셀 기준으로 충분히 키운 후 이진화.
                """
                cands = []
                gray  = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
                gray  = _force_upscale(gray)

                # ── 전처리 경로 A: CLAHE + 샤프닝 (권장) ──
                enhanced = _sharpen(_clahe(gray))
                ...
                # ② Otsu 반전 (어두운 배경 + 밝은 글자 — LCD 화면 대응)
                _, th_inv = cv2.threshold(
                    enhanced, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
                cands.append((th_inv, "clahe_otsu_inv"))

                # ③ 적응형 이진화 (지역적 밝기 변화 대응)
                th_adapt = cv2.adaptiveThreshold(
                    enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                    cv2.THRESH_BINARY, 21, 8)
                cands.append((th_adapt, "adaptive"))

                # ── 전처리 경로 B: 원본 gray (보험용) ──
            ...
            try:
                ...
                WL = "0123456789ABCDEFabcdef"
                # ★ v4.4: psm 4종으로 확대 — 다자리 수 인식률 향상
                #   psm 7: 단일 텍스트 행 (기본)
                #   psm 8: 단어 단위
                #   psm 6: 균일 블록 — 다자리 수 통째 인식
                #   psm 13: 원시 행 — 전처리 없이 직접 OCR
                DIGIT_CFGS = [
                    f"--psm 7 --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 8 --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 6 --oem 3 -c tessedit_char_whitelist={WL}",
                ...
                for img_bin, tag in cands:
                    ...
                    for cfg in DIGIT_CFGS:
                        try:
                            ...
                            for t, c in zip(texts, confs):
                                ...
                                # ★ confidence 기준 완화: 0 이상이면 수집
                                #   (소형 숫자는 confidence 낮게 나오는 경향)
                                if t_s and c_val >= 0:
                                    cleaned = _clean_number(t_s)
                                    if cleaned:
                    ...
                    # ★ confidence 조기종료 기준 완화: 40으로 낮춤
                    #   소형 숫자는 80~50 달성이 어렵기 때문
