...
class PreviewLabel(QLabel):
    ...
    def update_frame(self, frame: np.ndarray):
        ...
        def _draw_label(img, text, x, y, color=(255,255,255),
            ...
            return tw  # 텍스트 폭 반환 (연속 배치용)

        for i, roi in enumerate(self._rois()):
            rx, ry, rw, rh = roi.rect()

            # ── ROI 밝기 캐시 업데이트 — 블랙아웃 ON 또는 표시용 ★ ────
            # 블랙아웃 기능이 꺼져 있어도 오버레이 표시를 위해 계산은 유지.
            # 단, 계산은 가볍고(픽셀 평균 1회) 핵심 부하 원인이 아님.
            ...
            # ── 박스 위 정보바 레이아웃 (겹침 없이 좌→우 순서 배치) ─────
            #   [ROI이름]  [L:밝기]  [OCR: 값]
            #
            # 기준 y좌표: 박스 위쪽 (글자 하단이 박스 상단에 닿도록)
            INFO_SCALE = 0.40
            font       = cv2.FONT_HERSHEY_SIMPLEX
            LABEL_H    = 14  # 글자 높이 + 여백 (approximate)

            # y가 너무 위로 나가면 박스 아래에 표시
            above_y = ry - 4
            if above_y < LABEL_H:
                above_y = ry + rh + LABEL_H
            ...
            if ocr_txt.startswith("[ERR:"):
                ...
                ocr_show = f"OCR:{ocr_txt[:14]}"
                ocr_color = (60, 230, 255)
            else:
                ocr_show = "OCR:—"
                ocr_color = (100, 100, 120)
...
class BlackoutPanel(QWidget):
    ...
    def _build(self):
        ...
        # ── OCR ON/OFF ★ v2.9.3 — 비활성화 시 OCR 연산 완전 중단 ─────
        self.ocr_chk = QCheckBox("🔤 ROI OCR 활성화  (OCR 끄면 자원 절약)")
        self.ocr_chk.setChecked(True)
        self.ocr_chk.setStyleSheet(
            "QCheckBox{color:#3de;font-size:10px;}"
            "QCheckBox::indicator{width:14px;height:14px;"
            "border:1px solid #1a5a6a;border-radius:3px;background:#0d0d1e;}"
            "QCheckBox::indicator:checked{background:#0a5a6a;border-color:#3de;}")
        self.ocr_chk.setToolTip(
            "체크 해제 시: OCR 백그라운드 루프 중단\n"
            "ROI 박스 위 OCR 값 표시 없어짐\n"
            "블랙아웃/밝기 계산은 별도로 동작")
        self.ocr_chk.toggled.connect(self._on_ocr_toggle)
        v.addWidget(self.ocr_chk)
    ...
    def _on_ocr_toggle(self, enabled: bool):
        """OCR ON/OFF — engine.ocr_enabled 플래그 설정."""
        self.engine.ocr_enabled = enabled
        if enabled:
            self.ocr_chk.setText("🔤 ROI OCR 활성화  (OCR 끄면 자원 절약)")
            self.ocr_chk.setStyleSheet(
                "QCheckBox{color:#3de;font-size:10px;}"
                "QCheckBox::indicator{width:14px;height:14px;"
                "border:1px solid #1a5a6a;border-radius:3px;background:#0d0d1e;}"
            ...
            self.ocr_chk.setText("🔤 ROI OCR 비활성화  (OCR 연산 중단 중)")
            self.ocr_chk.setStyleSheet(
                "QCheckBox{color:#556;font-size:10px;}"
                "QCheckBox::indicator{width:14px;height:14px;"
                "border:1px solid #334;border-radius:3px;background:#0a0a18;}"
                "QCheckBox::indicator:checked{background:#0a5a6a;border-color:#3de;}"
                "QCheckBox::indicator:unchecked{background:#1a1a2a;}")
        ...
        try:
            ...
            for inv in (False, True):
                ...
                padded = cv2.copyMakeBorder(
                    th, 20, 20, 20, 20, cv2.BORDER_CONSTANT, value=255)
        ...
        if not best:
            try:
                ...
                res = self._easyocr_reader.readtext(
                    region, detail=1, allowlist='0123456789')
                parts = [_re.sub(r'[^\d]', '', str(r[1]))
                         for r in res if float(r[2]) >= 0.3]
                best = "".join(p for p in parts if p)
            except Exception:
                pass

        result = _re.sub(r'[^\d]', '', best)  # 최종 숫자만
...
class MainWindow(QMainWindow):
    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame):
            ...
            · 전처리 후보별로 실행 후 숫자로만 구성된 결과 우선 선택
            · 신뢰도 기반 최적 결과 선택
            ...
            # ── 전처리 헬퍼 ──────────────────────────────────────────
            def _upscale(img, target_h=200):
                """크게 확대 — 손글씨 숫자 OCR 정확도의 핵심"""
                h, w = img.shape[:2]
                if h < target_h:
                    s = target_h / h
            ...
            def _make_candidates(bgr):
                """
                전처리 후보 생성.
                손글씨 숫자는 배경이 밝고 글씨가 어두우므로
                Otsu 정방향이 가장 유효하지만, 역방향도 포함.
                """
                cands = []
                ...
                # ③ CLAHE 후 Otsu
                enh = _enhance_contrast(gray)
                _, th2 = cv2.threshold(
                    enh, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                cands.append((th2, "clahe_otsu"))

                # ④ Otsu 반전 (어두운 배경)
                _, th3 = cv2.threshold(
                    gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
                cands.append((th3, "otsu_inv"))
            ...
            def _score(text: str) -> int:
                ...
                if not t:
                    return 0
                digit_only = all(c.isdigit() for c in t.replace(" ", ""))
                return len(t) * (10 if digit_only else 1)
