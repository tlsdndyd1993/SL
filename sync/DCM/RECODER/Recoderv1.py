...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QComboBox, QLineEdit, QSplitter, QInputDialog, QAction,
    QMenuBar,
)
from PyQt5.QtCore import (Qt, QTimer, pyqtSignal, QObject, QPoint,
                           QRect, QDateTime, QSize)
from PyQt5.QtGui import QImage, QPixmap, QColor, QPainter, QPen, QTextCursor, QFont
...
def _put_text_pil(frame: np.ndarray, text: str, x: int, y: int,
                  font_size: int, color_bgr: tuple) -> None:
    """PIL을 이용해 유니코드 텍스트를 프레임에 합성."""
    if not PIL_AVAILABLE:
    ...
    try:
        bb = fnt.getbbox(text); tw = bb[2]-bb[0]; th = bb[3]-bb[1]
    except AttributeError:
        try: tw, th = fnt.getsize(text)
        except: tw, th = font_size*len(text), font_size
    pad = font_size + 8
    x1 = max(0, x-pad); y1 = max(0, y-th-pad)
    x2 = min(w_f, x+tw+pad*2); y2 = min(h_f, y+pad)
    if x2 <= x1 or y2 <= y1: return
    roi = frame[y1:y2, x1:x2].copy()
    pil_img = _PIL_Image.fromarray(cv2.cvtColor(roi, cv2.COLOR_BGR2RGB))
    draw = _PIL_Draw.Draw(pil_img)
    draw.text((x-x1, y-th-y1), text, font=fnt,
...
class SettingsDB:
        ...
        with self._lock:
            with self._conn() as c:
                ...
                c.executemany(
                    ...
                    [(s.get('title','슬롯'), json.dumps(s.get('steps',[]),ensure_ascii=False), i)
    ...
    def load_macro_slots(self) -> list:
        ...
        for title, sj in rows:
            try: steps = json.loads(sj) if sj else []
            except: steps = []
...
class Signals(QObject):
    ...
    macro_step_rec     = pyqtSignal(int, int, float)
...
class CoreEngine:
    ...
    def __init__(self, signals: Signals, base_dir: str = BASE_DIR):
        ...
        self.memo_overlays: list = [MemoOverlayCfg(0,"bottom-right","both",True)]
    ...
    def _fourcc(self):
        m = {"avc1": cv2.VideoWriter_fourcc(*'avc1'),
             "xvid": cv2.VideoWriter_fourcc(*'XVID')}
        return m.get(self.video_codec, cv2.VideoWriter_fourcc(*'mp4v'))
        ...
        for idx in range(8):
            ...
            if not (fps and 0<fps<300):
                ...
                while frames < 10:
                    ret,_ = cap.read()
                    if ret: frames += 1
                el = time.time()-t0; fps = frames/el if el>0 else 30.0
            try: nm = cap.getBackendName()
            except: nm = "Camera"
            cap.release()
            found.append({"idx":idx,"name":f"Camera {idx} [{nm}] {fps:.1f}fps","fps":float(fps)})
    ...
    def restart_screen(self):
        self.stop_screen()
        QTimer.singleShot(150, self.start_screen)
    ...
    def macro_start_recording(self):
        ...
        def _delayed():
            ...
            def on_click(x,y,button,pressed):
                ...
                try: _q.put_nowait((int(x),int(y),t))
                except: pass
            ...
            def _emit_loop():
                ...
                while True:
                    ...
                    self.signals.macro_step_rec.emit(x,y,delay)
            ...
            self.signals.status_message.emit("매크로 기록 중 — 클릭하세요")
    ...
    def macro_stop_recording(self):
        ...
        def _stop():
            ...
            if self._mac_listener:
                self._mac_listener.stop(); self._mac_listener=None
    ...
    def _mac_loop(self):
        ...
        while not self._mac_stop.is_set():
            for step in list(self.macro_steps):
                ...
                waited=0.0
                while waited<step.delay and not self._mac_stop.is_set():
            ...
            waited=0.0
            while waited<self.macro_gap and not self._mac_stop.is_set():
                chunk=min(0.05,self.macro_gap-waited); time.sleep(chunk); waited+=chunk
        self.macro_running=False
...
#  공용 위젯 유틸
# =============================================================================
def _style_btn(btn: QPushButton, bg="#1e2a3a", fg="#ccd",
               hover="#2a3e56", border="#4a5a7a",
               checked=None, disabled_bg="#141424", disabled_fg="#3a3a5a"):
    base = (f"QPushButton{{background:{bg};color:{fg};border:1px solid {border};"
    ...
    if checked:
        base += f"QPushButton:checked{{background:{checked};}}"
...
def _sep_h() -> QFrame:
    ...
    f.setStyleSheet("border:1px solid #2a2a4a;"); return f
...
class MemoOverlayRow(QWidget):
    ...
    def __init__(self, cfg: MemoOverlayCfg, tab_count: int, parent=None):
        ...
        lay=QHBoxLayout(self); lay.setContentsMargins(2,2,2,2); lay.setSpacing(5)
        ...
        self._tab.setValue(cfg.tab_idx+1); self._tab.setFixedWidth(48)
        ...
        self._pos.setFixedWidth(60); lay.addWidget(self._pos)
        ...
        self._tgt.setFixedWidth(70); lay.addWidget(self._tgt)
...
class RecordingPanel(QWidget):
    ...
    def _build(self):
        ...
        self.status_lbl=QLabel("● 대기"); self.status_lbl.setStyleSheet("color:#e74c3c;font-weight:bold;font-size:14px;")
        self.timer_lbl=QLabel("00:00:00"); self.timer_lbl.setStyleSheet("font-size:28px;font-weight:bold;color:#2ecc71;font-family:monospace;")
        self.datetime_lbl=QLabel("—"); self.datetime_lbl.setStyleSheet("color:#7bc8e0;font-size:11px;font-family:monospace;")
        ...
        g.addWidget(QLabel("Screen FPS:"),3,0); self.scr_fps_lbl=QLabel("—"); g.addWidget(self.scr_fps_lbl,3,1)
        g.addWidget(QLabel("Camera FPS:"),4,0); self.cam_fps_lbl=QLabel("—"); g.addWidget(self.cam_fps_lbl,4,1)
        open_btn=QPushButton("📂 녹화 폴더"); open_btn.setFixedHeight(26)
        open_btn.clicked.connect(lambda: open_folder(self.engine.output_dir or self.engine.base_dir))
        g.addWidget(open_btn,5,0,1,2)
        v.addWidget(st)

        # 컨트롤
        ctrl=QGroupBox("컨트롤"); cv=QVBoxLayout(ctrl); cv.setSpacing(8)
        self.btn_start=QPushButton("⏺  녹화 시작  [Ctrl+Alt+W]")
        self.btn_start.setStyleSheet("QPushButton{background:#27ae60;color:white;font-size:12px;padding:8px;border-radius:5px;border:none;font-weight:bold;}QPushButton:hover{background:#2ecc71;}QPushButton:disabled{background:#1a3a28;color:#4a7a5a;}")
        self.btn_start.clicked.connect(self._on_start)
        self.btn_stop=QPushButton("⏹  녹화 종료  [Ctrl+Alt+E]")
        self.btn_stop.setStyleSheet("QPushButton{background:#c0392b;color:white;font-size:12px;padding:8px;border-radius:5px;border:none;font-weight:bold;}QPushButton:hover{background:#e74c3c;}QPushButton:disabled{background:#3a1a1a;color:#7a4a4a;}")
        ...
        # 스크린 녹화 ON/OFF
        sr=QGroupBox("화면 녹화"); sv=QVBoxLayout(sr)
        self.scr_chk=QCheckBox("화면 녹화 활성화  [Ctrl+Alt+D]")
        self.scr_chk.setChecked(True)
        self.scr_chk.toggled.connect(lambda c: setattr(self.engine,'screen_rec_enabled',c))
        sv.addWidget(self.scr_chk); v.addWidget(sr)
        ...
        self.fps_spin.valueChanged.connect(lambda v: setattr(self.engine,'actual_screen_fps',v))
        ...
        self.speed_spin.setStyleSheet("QDoubleSpinBox{background:#1a1a2a;color:#f0c040;border:1px solid #5a5a20;border-radius:4px;padding:3px;font-size:13px;font-weight:bold;}")
        ...
        for lbl,val in [("0.5×",.5),("1×",1.),("2×",2.),("4×",4.)]:
            ...
            b.setStyleSheet("QPushButton{background:#2a2a1a;color:#f0c040;border:1px solid #4a4a20;border-radius:3px;font-size:10px;}")
            b.clicked.connect(lambda _,v=val: self.speed_spin.setValue(v)); pr.addWidget(b)
        ...
        self.codec_cb.addItems(["mp4v (기본)","avc1 / H.264 (소형)","xvid (호환)"])
        self.codec_cb.setStyleSheet("QComboBox{background:#1a1a3a;color:#ddd;border:1px solid #3a4a6a;border-radius:3px;padding:2px;}")
        self.codec_cb.currentIndexChanged.connect(self._on_codec)
        cl.addWidget(self.codec_cb,0,1)
        cl.addWidget(QLabel("해상도:"),1,0)
        self.scale_cb=QComboBox()
        self.scale_cb.addItems(["100% 원본","75%","50%"])
        self.scale_cb.setStyleSheet("QComboBox{background:#1a1a3a;color:#ddd;border:1px solid #3a4a6a;border-radius:3px;padding:2px;}")
        self.scale_cb.currentIndexChanged.connect(self._on_scale)
        cl.addWidget(self.scale_cb,1,1)
        v.addWidget(cg)
    ...
    def _on_rec_started(self):
        ...
        self.status_lbl.setText("● 녹화 중"); self.status_lbl.setStyleSheet("color:#2ecc71;font-weight:bold;font-size:14px;")
        self.speed_spin.setEnabled(False); self.speed_lock.setVisible(True)
        self._timer.start(500)

    def _on_rec_stopped(self):
        self.btn_start.setEnabled(True); self.btn_stop.setEnabled(False)
        self.status_lbl.setText("● 대기"); self.status_lbl.setStyleSheet("color:#e74c3c;font-weight:bold;font-size:14px;")
    ...
    def _on_speed(self, v):
        self.engine.playback_speed=v
        if v==1.0: t="정배속"
        elif v<1.0: t=f"{v:.2f}× 슬로우모션"
        else:       t=f"{v:.2f}× 타임랩스"
        self.speed_info.setText(t)

    def _on_codec(self, i):
        self.engine.video_codec=["mp4v","avc1","xvid"][i]
...
class MacroPanel(QWidget):
    ...
    def _build(self):
        ...
        self.slot_cb.setStyleSheet("QComboBox{background:#0f2a1a;color:#afffcf;border:1px solid #2a8a5a;border-radius:3px;font-weight:bold;font-size:11px;min-width:100px;}")
        self.slot_cb.currentIndexChanged.connect(self._on_slot_changed)
        sc.addWidget(self.slot_cb,1)
        for lbl,fn,c in [("＋","_add_slot","#1a4a2a"),("－","_del_slot","#3a1a1a"),("✏","_rename_slot","#1a2a3a")]:
            b=QPushButton(lbl); b.setFixedSize(28,26)
            b.setStyleSheet(f"QPushButton{{background:{c};color:#ddd;border:1px solid #4a6a4a;border-radius:3px;}}")
        ...
        # 기록
        rg=QGroupBox("📍 좌표 기록"); rl=QVBoxLayout(rg); rl.setSpacing(5)
        rb=QHBoxLayout()
        self.rec_btn=QPushButton("⏺  기록 시작"); self.rec_btn.setCheckable(True); self.rec_btn.setFixedHeight(30)
        ...
        self.rec_st=QLabel("● 대기"); self.rec_st.setStyleSheet("color:#888;font-size:11px;font-weight:bold;")
        rb.addWidget(self.rec_btn,1); rb.addWidget(self.rec_st); rl.addLayout(rb)
        self.pos_lbl=QLabel("마지막 클릭: —")
        self.pos_lbl.setStyleSheet("color:#f0c040;font-size:11px;font-family:monospace;")
        rl.addWidget(self.pos_lbl); v.addWidget(rg)

        # 스텝 테이블
        tg=QGroupBox("📋 클릭 스텝"); tv=QVBoxLayout(tg); tv.setSpacing(4)
        self.tbl=QTableWidget(0,4)
        self.tbl.setHorizontalHeaderLabels(["#","X","Y","딜레이(s)"])
        for c,m in [(0,QHeaderView.Fixed),(1,QHeaderView.Stretch),(2,QHeaderView.Stretch),(3,QHeaderView.Stretch)]:
            self.tbl.horizontalHeader().setSectionResizeMode(c,m)
        self.tbl.setColumnWidth(0,32); self.tbl.setFixedHeight(150)
        self.tbl.setStyleSheet("QTableWidget{background:#0a0a18;color:#ccc;font-size:11px;border:1px solid #1a2a3a;}QHeaderView::section{background:#0f1a2a;color:#7ab4d4;font-size:11px;border:none;padding:4px;}")
        self.tbl.itemChanged.connect(self._on_item_changed); tv.addWidget(self.tbl)
        tb=QHBoxLayout(); tb.setSpacing(4)
        for lbl,fn in [("↑","_step_up"),("↓","_step_dn")]:
            b=QPushButton(lbl); b.setFixedSize(28,24); b.clicked.connect(getattr(self,fn)); tb.addWidget(b)
        tb.addStretch()
        for lbl,fn in [("선택 삭제","_del_step"),("전체 삭제","_clear_steps")]:
            b=QPushButton(lbl); b.setFixedHeight(24); b.clicked.connect(getattr(self,fn)); tb.addWidget(b)
        tv.addLayout(tb)
        br=QHBoxLayout(); br.addWidget(QLabel("일괄 딜레이:"))
        self.bulk_spin=_make_spinbox(0.05,60,0.5,0.1,2,"",70)
        bb=QPushButton("적용"); bb.setFixedHeight(24); bb.clicked.connect(self._bulk_delay)
        br.addWidget(self.bulk_spin); br.addWidget(QLabel("초")); br.addWidget(bb); br.addStretch()
        ...
        self.rep_spin.valueChanged.connect(lambda v: setattr(self.engine,'macro_repeat',int(v)))
        rn.addWidget(self.rep_spin,0,1)
        rn.addWidget(QLabel("루프 간격(초):"),1,0)
        self.gap_spin=_make_spinbox(0,60,1,0.5,1,"",70)
        self.gap_spin.valueChanged.connect(lambda v: setattr(self.engine,'macro_gap',v))
        rn.addWidget(self.gap_spin,1,1)
        rb2=QHBoxLayout(); rb2.setSpacing(6)
        self.run_btn=QPushButton("▶  실행"); self.run_btn.setFixedHeight(32)
        self.run_btn.setStyleSheet("QPushButton{background:#2980b9;color:#fff;border:none;border-radius:5px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#3498db;}QPushButton:disabled{background:#1a3a5a;color:#555;}")
        self.run_btn.clicked.connect(self._on_run)
        self.stop_btn=QPushButton("■  중단"); self.stop_btn.setFixedHeight(32)
        self.stop_btn.setStyleSheet("QPushButton{background:#5a6a7a;color:#fff;border:none;border-radius:5px;font-size:12px;}QPushButton:hover{background:#7f8c8d;}QPushButton:disabled{background:#2a2a2a;color:#555;}")
        self.stop_btn.setEnabled(False); self.stop_btn.clicked.connect(self._on_stop)
        self.run_st=QLabel("● 대기"); self.run_st.setStyleSheet("color:#888;font-size:11px;font-weight:bold;")
        rb2.addWidget(self.run_btn,1); rb2.addWidget(self.stop_btn,1)
        rn.addLayout(rb2,2,0,1,2); rn.addWidget(self.run_st,3,0,1,2); v.addWidget(run)
    ...
    def _sync(self):
        if 0 <= self._active_slot < len(self._slots):
            steps = self._slots[self._active_slot]['steps']
            self.engine.macro_steps.clear(); self.engine.macro_steps.extend(steps)
        else: self.engine.macro_steps.clear()
    ...
    def _info_upd(self):
        ...
        self.slot_info.setText(f"{nm}  |  슬롯 {idx+1}/{n}  |  {len(self.engine.macro_steps)} 스텝")
    ...
    def _add_slot(self):
        ...
        self.slot_cb.blockSignals(True); self.slot_cb.addItem(t); self.slot_cb.blockSignals(False)
        self.slot_cb.setCurrentIndex(len(self._slots)-1)

    def _del_slot(self):
        if len(self._slots)<=1: return
        idx=self._active_slot; self._slots.pop(idx)
        self.slot_cb.blockSignals(True); self.slot_cb.removeItem(idx); self.slot_cb.blockSignals(False)
        new=max(0,idx-1); self._active_slot=new
        self.slot_cb.setCurrentIndex(new); self._sync(); self._rebuild_tbl(); self._info_upd()
    ...
    def _rename_slot(self):
        ...
        if ok and new.strip():
            ...
            self.slot_cb.blockSignals(True); self.slot_cb.setItemText(idx,new.strip()); self.slot_cb.blockSignals(False)
            self._info_upd()

    def _on_rec_toggle(self, recording):
        if recording:
            self.rec_btn.setText("⏹  기록 중단")
            self.rec_st.setText("● 기록 중"); self.rec_st.setStyleSheet("color:#e74c3c;font-size:11px;font-weight:bold;")
            self.engine.macro_start_recording()
        else:
            self.rec_btn.setText("⏺  기록 시작")
            self.rec_st.setText("● 완료"); self.rec_st.setStyleSheet("color:#2ecc71;font-size:11px;font-weight:bold;")
            self.engine.macro_stop_recording()
            QTimer.singleShot(150, self._make_editable)

    def _on_step(self, x, y, delay):
        step=ClickStep(x,y,delay); self._append_row(step,False); self._info_upd()

    def _append_row(self, step: ClickStep, editable=True):
        ...
        idx_it.setForeground(QColor("#556")); self.tbl.setItem(r,0,idx_it)
        ...
        self.pos_lbl.setText(f"({step.x},{step.y}) delay={step.delay:.3f}s")
    ...
    def _rebuild_tbl(self):
        self.tbl.blockSignals(True); self.tbl.setRowCount(0); self.tbl.blockSignals(False)
        for step in self.engine.macro_steps: self._append_row(step,True)
    ...
    def _on_run(self):
        if not self.engine.macro_steps: return
        self.engine.macro_start_run(); self.run_btn.setEnabled(False); self.stop_btn.setEnabled(True)
        self.run_st.setText("● 실행 중"); self.run_st.setStyleSheet("color:#2ecc71;font-size:11px;font-weight:bold;")
        self._watch=QTimer(self); self._watch.timeout.connect(self._check_done); self._watch.start(300)

    def _check_done(self):
        if not self.engine.macro_running:
            self._watch.stop(); self.run_btn.setEnabled(True); self.stop_btn.setEnabled(False)
            self.run_st.setText("● 완료"); self.run_st.setStyleSheet("color:#888;font-size:11px;font-weight:bold;")

    def _on_stop(self):
        self.engine.macro_stop_run(); self.run_btn.setEnabled(True); self.stop_btn.setEnabled(False)
        self.run_st.setText("● 중단"); self.run_st.setStyleSheet("color:#e74c3c;font-size:11px;font-weight:bold;")

    def get_slots_data(self):
        self._save_cur()
        return [{'title':s['title'],
                 'steps':[{'x':st.x,'y':st.y,'delay':st.delay} for st in s['steps']]}
...
class MemoPanel(QWidget):
    ...
    def _build(self):
        ...
        # 현재 탭 설정
        cg=QGroupBox("📝 현재 탭 설정  (탭 전환 시 자동 반영)"); cl=QVBoxLayout(cg); cl.setSpacing(6)
        font_row=QHBoxLayout(); font_row.setSpacing(6); font_row.addWidget(QLabel("에디터 글꼴:"))
        self.font_spin=QSpinBox(); self.font_spin.setRange(8,36); self.font_spin.setValue(11); self.font_spin.setFixedWidth(52)
        self.font_spin.setStyleSheet("QSpinBox{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;border-radius:3px;font-weight:bold;font-size:13px;}")
        self.font_sl=QSlider(Qt.Horizontal); self.font_sl.setRange(8,36); self.font_sl.setValue(11)
        self.font_sl.setStyleSheet("QSlider::groove:horizontal{background:#1a2030;height:6px;border-radius:3px;}QSlider::handle:horizontal{background:#f0c040;width:16px;height:16px;margin-top:-5px;margin-bottom:-5px;border-radius:8px;}QSlider::sub-page:horizontal{background:#5a5a20;border-radius:3px;}")
        self.font_spin.valueChanged.connect(lambda v:(self.font_sl.blockSignals(True),self.font_sl.setValue(v),self.font_sl.blockSignals(False),self._apply_font_cur(v)))
        self.font_sl.valueChanged.connect(lambda v:(self.font_spin.blockSignals(True),self.font_spin.setValue(v),self.font_spin.blockSignals(False),self._apply_font_cur(v)))
        ...
        for lbl,sz in [("S",10),("M",13),("L",16),("XL",20)]:
            ...
            b.setStyleSheet("QPushButton{background:#2a2a1a;color:#f0c040;border:1px solid #4a4a20;border-radius:3px;font-size:10px;}")
            b.clicked.connect(lambda _,s=sz: self.font_spin.setValue(s)); font_row.addWidget(b)
        ...
        ts_row.addWidget(self.ts_chk); ts_row.addWidget(QLabel("(우클릭: 제거)")); ts_row.addStretch()
        cl.addLayout(ts_row); v.addWidget(cg)

        # 오버레이 글꼴 크기 (공통)
        og=QGroupBox("🎬 영상 오버레이 글자 크기  (공통)"); ol=QHBoxLayout(og); ol.setSpacing(6)
        self.ov_spin=QSpinBox(); self.ov_spin.setRange(8,72); self.ov_spin.setValue(18); self.ov_spin.setFixedWidth(52)
        self.ov_spin.setStyleSheet("QSpinBox{background:#1a1a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:3px;font-weight:bold;font-size:13px;}")
        self.ov_sl=QSlider(Qt.Horizontal); self.ov_sl.setRange(8,72); self.ov_sl.setValue(18)
        self.ov_sl.setStyleSheet("QSlider::groove:horizontal{background:#1a2030;height:6px;border-radius:3px;}QSlider::handle:horizontal{background:#7bc8e0;width:16px;height:16px;margin-top:-5px;margin-bottom:-5px;border-radius:8px;}QSlider::sub-page:horizontal{background:#2a4a6a;border-radius:3px;}")
        self.ov_spin.valueChanged.connect(lambda v:(self.ov_sl.blockSignals(True),self.ov_sl.setValue(v),self.ov_sl.blockSignals(False),setattr(self.engine,'overlay_font_size',v)))
        self.ov_sl.valueChanged.connect(lambda v:(self.ov_spin.blockSignals(True),self.ov_spin.setValue(v),self.ov_spin.blockSignals(False),setattr(self.engine,'overlay_font_size',v)))
        ol.addWidget(self.ov_sl,1); ol.addWidget(self.ov_spin)
        for lbl,sz in [("S",14),("M",18),("L",24),("XL",32)]:
            b=QPushButton(lbl); b.setFixedSize(28,22)
            b.setStyleSheet("QPushButton{background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:3px;font-size:10px;}")
            b.clicked.connect(lambda _,s=sz: self.ov_spin.setValue(s)); ol.addWidget(b)
        v.addWidget(og)

        # 탭 조작 버튼
        tc=QHBoxLayout(); tc.setSpacing(6)
        for lbl,fn,st in [("＋ 탭","_add_tab","background:#1a3a1a;color:#8fa;border:1px solid #2a6a2a;border-radius:4px;font-size:11px;"),
                          ("－ 탭","_del_tab","background:#3a1a1a;color:#f88;border:1px solid #6a2a2a;border-radius:4px;font-size:11px;"),
                          ("현재 탭 지우기","_clear_cur","background:#1a1a3a;color:#aaa;border:1px solid #334;border-radius:4px;font-size:11px;")]:
            b=QPushButton(lbl); b.setFixedHeight(26); b.setStyleSheet(st); b.clicked.connect(getattr(self,fn)); tc.addWidget(b)
    ...
    def _on_tab_changed(self, idx):
        ...
        self.ts_chk.blockSignals(True); self.ts_chk.setChecked(ed.timestamp_enabled); self.ts_chk.blockSignals(False)
    ...
    def _add_tab_new(self):
        n=self.tabs.count()+1; self._add_tab(f"메모 {n}"); self.tabs.setCurrentIndex(self.tabs.count()-1)
    _add_tab = _add_tab  # preserve original signature
    ...
    def add_tab_public(self, title="메모", content="", font_size=11, ts_enabled=True):
        self._add_tab(title, content, font_size, ts_enabled)
...
class PathSettingsPanel(QWidget):
    ...
    def _build(self):
        ...
        info=QLabel(
            "저장 경로 구조:\n"
            "~/Desktop/bltn_rec/[차종]/YYYYMMDD/[TC-ID]/Rec_HHMMSS/\n\n"
            "차종, TC-ID를 비워두면 해당 폴더 단계가 생략됩니다.")
        info.setWordWrap(True); info.setStyleSheet("color:#778;font-size:10px;border:1px solid #1a2a3a;border-radius:4px;padding:6px;background:#0d0d1e;")
        v.addWidget(info)
        g=QGroupBox("경로 항목"); gl=QGridLayout(g); gl.setSpacing(8)
        gl.addWidget(QLabel("차종 (ex: TK1, NX5):"),0,0)
        self.vehicle_ed=QLineEdit(); self.vehicle_ed.setPlaceholderText("예: TK1")
        self.vehicle_ed.setStyleSheet("QLineEdit{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;border-radius:3px;padding:4px 8px;font-size:12px;font-weight:bold;}")
        ...
        self.tc_ed=QLineEdit(); self.tc_ed.setPlaceholderText("예: BLTN_CAM_TC_3-0001")
        self.tc_ed.setStyleSheet("QLineEdit{background:#1a1a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:3px;padding:4px 8px;font-size:12px;}")
        ...
        save_btn.setStyleSheet("QPushButton{background:#1a3a2a;color:#afffcf;border:1px solid #2a8a5a;border-radius:4px;font-weight:bold;}QPushButton:hover{background:#225a3a;}")
        save_btn.clicked.connect(self._save); v.addWidget(save_btn)
        self.result_lbl=QLabel(""); self.result_lbl.setStyleSheet("font-size:11px;"); v.addWidget(self.result_lbl)
        self.preview_lbl=QLabel(""); self.preview_lbl.setStyleSheet("color:#556;font-size:10px;font-family:monospace;word-wrap:break-word;")
        self.preview_lbl.setWordWrap(True); v.addWidget(self.preview_lbl)
        ...
        open_btn.clicked.connect(lambda: open_folder(self.engine.base_dir)); v.addWidget(open_btn)

    def _upd_preview(self):
        v=self.vehicle_ed.text().strip(); t=self.tc_ed.text().strip()
    ...
    def _save(self):
        self.db.set_path_settings(self.vehicle_ed.text().strip(), self.tc_ed.text().strip())
        self.result_lbl.setText("✅ 저장 완료"); self.result_lbl.setStyleSheet("color:#2ecc71;font-size:11px;")
...
class CameraWindow(QDialog):
    ...
    def _build(self):
        ...
        hl.addWidget(QLabel("📷")); t=QLabel("Camera Preview"); t.setStyleSheet("color:#9ab;font-weight:bold;font-size:13px;"); hl.addWidget(t); hl.addStretch()
        self._fps_lbl=QLabel("FPS: —"); self._fps_lbl.setStyleSheet("color:#888;font-size:11px;"); hl.addWidget(self._fps_lbl)
        self._toggle=QPushButton("⏸ 일시정지"); self._toggle.setCheckable(True); self._toggle.setChecked(False)
        self._toggle.setStyleSheet("QPushButton{background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:13px;font-size:10px;padding:2px 10px;}QPushButton:checked{background:#3a1a1a;color:#f88;}")
        self._toggle.toggled.connect(self._on_toggle); hl.addWidget(self._toggle)
        root.addWidget(hdr)
        # 카메라 선택
        cam_f=QFrame(); cam_f.setStyleSheet("QFrame{background:#0c0c1e;border-bottom:1px solid #1a2a3a;}"); cam_f.setMaximumHeight(140)
        cf=QVBoxLayout(cam_f); cf.setContentsMargins(10,8,10,8); cf.setSpacing(5)
        sr=QHBoxLayout()
        ...
        sr.addWidget(self._scan_btn); sr.addStretch(); sr.addWidget(self._sel_lbl); cf.addLayout(sr)
        ...
        scroll.setWidget(self._cb_cont); cf.addWidget(scroll); root.addWidget(cam_f)
        # 미리보기
        pc=QWidget(); pc.setStyleSheet("background:#0d0d1e;"); pv=QVBoxLayout(pc); pv.setContentsMargins(6,4,6,4); pv.setSpacing(3)
        self.lbl=PreviewLabel("camera",self.engine); pv.addWidget(self.lbl,1)
        hint=QLabel("Left-drag: ROI 추가  |  Right-click: ROI 제거"); hint.setStyleSheet("color:#444;font-size:10px;"); hint.setAlignment(Qt.AlignCenter); pv.addWidget(hint)
        root.addWidget(pc,1)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v1.0")
    ...
    def _build_ui(self):
        ...
        hint=QLabel("Left-drag: ROI 추가  |  Right-click: ROI 제거"); hint.setStyleSheet("color:#555;font-size:10px;"); hint.setAlignment(Qt.AlignCenter); sf.addWidget(hint)
    ...
    def _build_feature_bar(self) -> QWidget:
        bar=QFrame(); bar.setStyleSheet("QFrame{background:#08081a;border-bottom:2px solid #2a3a5a;}")
        bv=QVBoxLayout(bar); bv.setContentsMargins(8,6,8,4); bv.setSpacing(4)
        t=QLabel("⚙  기능 표시 / 숨기기"); t.setStyleSheet("color:#7ab4d4;font-size:12px;font-weight:bold;"); bv.addWidget(t)
        row=QHBoxLayout(); row.setSpacing(4)
        self._feat_cbs={}
        labels={"recording":"녹화","manual":"수동녹화","memo":"메모","autoclick":"오토클릭",
                "schedule":"예약","macro":"매크로","blackout":"블랙아웃",
                "path":"경로","log":"로그","reset":"초기화"}
        for key,txt in labels.items():
            cb=QCheckBox(txt); cb.setChecked(True)
            cb.setStyleSheet("QCheckBox{color:#ccd;font-size:10px;spacing:4px;}")
            cb.toggled.connect(lambda v,k=key: self._on_feat_toggle(k,v))
            self._feat_cbs[key]=cb; row.addWidget(cb)
        row.addStretch(); bv.addLayout(row)
        return bar
    ...
    def _pump_preview(self):
        try:
            ...
            if not self._scr_toggle.isChecked():
                self._scr_lbl.update_frame(CoreEngine.stamp_preview(sf,self.engine,"screen"))
        except queue.Empty: pass
        try:
            cf=self.engine.camera_queue.get_nowait()
            if self._cam_win.isVisible():
                self._cam_win.lbl.update_frame(CoreEngine.stamp_preview(cf,self.engine,"camera"))
        except queue.Empty: pass
    ...
    def _save_settings(self):
        ...
        db.set("overlay_font",   p["memo"].ov_spin.value())
        # 기능 ON/OFF
        for key,cb in self._feat_cbs.items():
            db.set(f"feat_{key}", cb.isChecked())
        # 섹션 접힘
    ...
    def _load_settings(self):
        ...
        memo=p["memo"]
        memo.ov_spin.setValue(db.get_int("overlay_font",18))
        # 기능 ON/OFF
        for key,cb in self._feat_cbs.items():
            cb.setChecked(db.get_bool(f"feat_{key}",True))
        # 섹션 접힘
        for key,sec in self._sections.items():
    ...
    def _on_reset(self, do_settings, do_memo, do_sections, do_db):
        ...
        if do_settings:
            ...
            rp.scr_chk.setChecked(True); rp.codec_cb.setCurrentIndex(0); rp.scale_cb.setCurrentIndex(0)
            bp=self._panels["blackout"]; bp.thr_spin.setValue(30.0); bp.cd_spin.setValue(5.0); bp.rec_chk.setChecked(True)
            ...
            self._panels["memo"].ov_spin.setValue(18)
        if do_memo:
            m=self._panels["memo"]
    ...
    def _setup_hotkeys(self):
        ...
        hk={
            ...
            '<ctrl>+<alt>+e': self.engine.stop_recording,
            '<ctrl>+<alt>+d': lambda: self._panels["recording"].scr_chk.setChecked(
                not self._panels["recording"].scr_chk.isChecked()),
            '<ctrl>+<alt>+a': self._panels["autoclick"]._on_start,
            '<ctrl>+<alt>+s': self._panels["autoclick"]._on_stop,
...
# =============================================================================
#  MemoPanel 메서드 패치 (클래스 정의 후 리네임)
# =============================================================================
# _add_tab 메서드를 퍼블릭으로 사용하도록 alias 재정의
MemoPanel.add_new_tab = MemoPanel._add_tab  # 원본 보존
def _memo_add_tab_outer(self):
    n = self.tabs.count() + 1
    self._add_tab(f"메모 {n}")
    self.tabs.setCurrentIndex(self.tabs.count() - 1)
MemoPanel._add_tab_btn = _memo_add_tab_outer
# 탭 추가 버튼 연결 픽스 (build 이후 동적으로 연결됨)
_orig_build_memo = MemoPanel._build
def _patched_build_memo(self):
    _orig_build_memo(self)
    # 탭 추가 버튼 찾아서 재연결
    for b in self.findChildren(QPushButton):
        if b.text() == "＋ 탭":
            try: b.clicked.disconnect()
            except: pass
            b.clicked.connect(lambda: _memo_add_tab_outer(self))
MemoPanel._build = _patched_build_memo
