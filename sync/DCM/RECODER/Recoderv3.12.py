...
class CoreEngine:
    ...
    def start_recording(self):
        ...
        # ★ 수정: output_dir 확정 후 tc_tag_target_dir 재동기화
        if self.tc_rec_enabled:
            self._resync_tc_tag_target()
    ...
    def _resync_tc_tag_target(self):
        """
        output_dir 확정 이후 tc_tag_target_dir을 재계산.

        ★ 수정: output_dir 경로를 직접 파싱해 tc_id 폴더를 찾음.
          _build_path 결과는 base_dir/[vehicle]/날짜/[tc_id]/[extra]/Rec_HHMMSS 구조이므로
          output_dir의 상위 경로를 올라가며 현재 tc_id와 일치하는 폴더를 찾음.
          이전 녹화의 잔재(tc_tag_target_dir)는 stop_recording에서 초기화하므로 여기서는
          항상 현재 tc_id 기준으로 새로 계산.
        """
        od = self.output_dir
        if not od:
            return
        ...
        if tc:
            # output_dir 상위를 탐색해 현재 tc_id 폴더 찾기
            cur = os.path.dirname(od)  # Rec_HHMMSS 의 부모부터 시작
            for _ in range(8):
                if os.path.basename(cur) == tc:
                    self.tc_tag_target_dir = cur
                    return
                parent = os.path.dirname(cur)
                if parent == cur:
                    break
                cur = parent
        # tc_id 폴더를 찾지 못한 경우: output_dir 자체를 태깅 대상으로
        self.tc_tag_target_dir = od
...
class PathSettingsPanel(QWidget):
    ...
    def _sync_tc_tag_target(self):
        ...
        # ★ 수정: output_dir이 아직 없는 시점이므로 녹화 시작 시 _resync_tc_tag_target()으로 재동기화됨
