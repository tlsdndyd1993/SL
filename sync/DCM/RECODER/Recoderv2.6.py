...
class PathSettingsPanel(QWidget):
    ...
    def _build(self):
        ...
        scroll.setStyleSheet(
            ...
            "QScrollBar:vertical{background:#0d0d1e;width:7px;border-radius:3px;}"
            "QScrollBar::handle:vertical{background:#336;border-radius:3px;min-height:16px;}"
            "QScrollBar::add-line:vertical,QScrollBar::sub-line:vertical{height:0;}")
        inner = QWidget(); inner.setStyleSheet("background:transparent;")
        v = QVBoxLayout(inner); v.setContentsMargins(2, 2, 4, 6); v.setSpacing(8)
        scroll.setWidget(inner)
        root_lay.addWidget(scroll, 1)

        # ── 안내 ─────────────────────────────────────────────────────────────
        info = QLabel(
            "지정 경로 구조:  ~/Desktop/bltn_rec / [차종_버전] / YYYYMMDD / [TC-ID] / [추가항목…]\n"
            "기능별로 '지정 경로 사용' 을 켜야 해당 경로가 적용됩니다.\n"
            "T/C 검증: 데이터 처리 완료 후 TC-ID 폴더명 앞에 (PASS)/(FAIL) 태그 삽입.")
        ...
        fl = QGridLayout(fixed_g); fl.setSpacing(8)
        fl.addWidget(QLabel("차종_버전:"), 0, 0)
        ...
        fl.addWidget(QLabel("(prefix + 숫자 | 휠로 숫자 조절)"), 2, 1)
        v.addWidget(fixed_g)

        # ── 동적 추가 항목 ────────────────────────────────────────────────────
        extra_g = QGroupBox("➕ 추가 경로 항목  (원하는 폴더 단계 추가)")
        extra_outer = QVBoxLayout(extra_g); extra_outer.setSpacing(5)

        # ★ 크래시 방지: 동적 행을 별도 컨테이너 위젯에 넣어 ScrollArea와 분리
        ...
        # ── 기능별 경로 설정 ──────────────────────────────────────────────────
        feat_g = QGroupBox("🗂 기능별 저장 경로  (default vs 지정 경로)")
        feat_l = QVBoxLayout(feat_g); feat_l.setSpacing(5)
        feat_l.addWidget(QLabel(
            "OFF = ~/Desktop/bltn_rec 아래 기본 경로\n"
            "ON  = 위에서 설정한 지정 경로 사용"))
        ...
        for engine_attr, label, color, chk_attr in _FEATS:
            chk = QCheckBox(label)
            chk.setStyleSheet(
                f"QCheckBox{{font-size:10px;font-weight:bold;color:{color};"
            ...
            feat_row.addWidget(chk)
        feat_row.addStretch()
        feat_l.addLayout(feat_row)
        v.addWidget(feat_g)
        ...
        self.preview_lbl.setStyleSheet(
            ...
            "border:1px solid #1a2030;border-radius:3px;padding:5px;background:#08080f;")
