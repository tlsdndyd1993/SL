...
class PreviewLabel(QLabel):
    ...
    def update_frame(self, frame: np.ndarray):
        ...
        for i, roi in enumerate(self._rois()):
            ...
            label = roi.name if roi.name else f"ROI{i+1}"
            cv2.putText(disp,label,(rx,max(ry-4,12)),
                        cv2.FONT_HERSHEY_SIMPLEX,0.45,(255,60,60),1)
        h,w,_=disp.shape
...
class RoiManagerPanel(QWidget):
    ...
    def _make_row(self, idx: int, roi: RoiItem) -> QFrame:
        ...
        hl = QHBoxLayout(row); hl.setContentsMargins(6,3,6,3); hl.setSpacing(6)

        # 드래그 핸들
        grip = QLabel("⠿"); grip.setFixedWidth(16)
        grip.setStyleSheet("color:#4a5a8a;font-size:18px;background:transparent;")

        # 번호
        num = QLabel(f"{idx+1}"); num.setFixedWidth(18)
        ...
        hl.addWidget(grip); hl.addWidget(num)
        hl.addLayout(info_col, 1)
        hl.addWidget(edit_btn); hl.addWidget(del_btn)

        # 마우스 이벤트를 행 위젯에 직접 연결 (드래그앤드랍)
...
_API_DOC = """
╔══════════════════════════════════════════════════════════════════╗
║       Screen & Camera Recorder  v2.9  —  Kernel API Reference   ║
╚══════════════════════════════════════════════════════════════════╝

커널 스크립트에서 사용 가능한 전역 객체:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  engine.stop_recording()
      → 녹화 종료. TC 검증 결과가 설정돼 있으면 폴더명 태그 적용.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  engine.save_manual_clip() → bool
      → 현재 시점 기준 Pre/Post 클립을 백그라운드 저장.
         쿨다운 중이면 False 반환.

  engine.manual_pre_sec   : float  → Pre  구간(초)
  engine.manual_post_sec  : float  → Post 구간(초)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  engine.capture_frame(source, tc_tag="") → str
      ...
      return  : 저장된 PNG 절대경로 (실패 시 "")

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [ROI 읽기]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  kernel.read_roi_value(roi_idx, source="screen") → float | None
      → 지정 ROI 영역의 평균 밝기(brightness) 반환.
         ROI 없거나 버퍼 비면 None.

  kernel.read_roi_text(roi_idx, source="screen") → str
      → ROI 영역의 텍스트를 OCR로 읽어 반환.
         pytesseract 미설치 시 빈 문자열.

  kernel.get_roi_avg(roi_idx, source="screen") → (B,G,R) tuple | None
      → ROI 영역 평균 BGR 값 반환.

  engine.screen_rois  : List[RoiItem]  → 화면 ROI 목록
  engine.camera_rois  : List[RoiItem]  → 카메라 ROI 목록
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [커널 유틸리티]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  kernel.wait(seconds)              → 슬립 (중단 이벤트 감지 포함)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [스크립트 예시 — 조건부 수동녹화]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  # ROI 0번 밝기가 10초내 50 미만으로 떨어지면 수동클립 + 캡처
  import time
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  · UI 위젯을 직접 조작하지 마세요 (thread-safe 하지 않음).
  · PyQt5 시그널 emit은 가능합니다.
  · kernel.wait() 로 슬립하면 is_stopped() 를 자동 감지합니다.
  · 무한 루프 시 반드시 kernel.is_stopped() 를 조건에 포함하세요.
"""


# =============================================================================
#  ApiDocDialog  — API 레퍼런스 모달
# =============================================================================
...
class KernelEngine:
    """
    내장 Python 인터프리터 기반 커널.

    역할:
      · 사용자가 작성한 .py 스크립트를 백그라운드 스레드에서 실행
      · engine(CoreEngine) 객체를 전역으로 노출
      · ROI 값 읽기 / 조건 감시 / TC 결과 설정 등 헬퍼 제공
      · 여러 스크립트를 순서대로 실행 (스케줄 큐)
    """
    ...
    def set_tc_result(self, result: str):
        """TC 검증 결과를 engine에 즉시 설정."""
        self._engine.tc_verify_result = result

    def read_roi_value(self, roi_idx: int, source: str = "screen") -> float | None:
        """지정 ROI의 평균 밝기(0~255) 반환."""
    ...
    def read_roi_text(self, roi_idx: int, source: str = "screen") -> str:
        """ROI 영역 OCR 텍스트 (pytesseract 필요)."""
        try:
            import pytesseract
            from PIL import Image as _PIL_Image
            rois = self._engine.screen_rois if source == "screen" else self._engine.camera_rois
            if not rois or roi_idx >= len(rois): return ""
            with self._engine._buf_lock:
                buf = self._engine._scr_buf if source == "screen" else self._engine._cam_buf
                frame = buf[-1].copy() if buf else None
            ...
            pil_img = _PIL_Image.fromarray(cv2.cvtColor(region, cv2.COLOR_BGR2RGB))
            return pytesseract.image_to_string(pil_img).strip()
        except ImportError:
            self._log("[Kernel] pytesseract 미설치 — OCR 불가")
            return ""
        except Exception as ex:
            self._log(f"[Kernel] OCR 오류: {ex}")
            return ""
    ...
    def _exec_script(self, script: KernelScript):
        ...
        try:
            code = compile(script.code, f"<kernel:{script.title}>", "exec")
            exec(code, _globals)
        except Exception as ex:
            import traceback
            self._log(f"❌ [{script.title}] 오류: {ex}")
            self._log(traceback.format_exc().strip().split('\n')[-1])
...
class ResetPanel(QWidget):
    ...
    def _reset_io(self):
        ...
        if self.engine._io_db:
            try:
                with self.engine._io_db._conn() as c:
                    c.executescript(
...
class _FloatingPreview(QWidget):
    ...
    def __init__(self, title: str, source: str,
                 engine: CoreEngine, roi_mgr=None, parent=None):
        super().__init__(parent,
        ...
        self.prev = PreviewLabel(source, engine, roi_mgr)
...
class CameraWindow(_FloatingPreview):
    def __init__(self, engine: CoreEngine, roi_mgr=None, parent=None):
        super().__init__("📷 Camera Preview", "camera", engine, roi_mgr, parent)

class DisplayWindow(_FloatingPreview):
    def __init__(self, engine: CoreEngine, roi_mgr=None, parent=None):
        super().__init__("🖥 Display Preview", "screen", engine, roi_mgr, parent)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v2.9")
    ...
    def _build_ui(self):
        ...
        self._tabs.setFixedWidth(480)
        self._tabs.setStyleSheet(
            "QTabWidget::pane{border:1px solid #2a3a5a;background:#0d0d1e;}"
            "QTabBar::tab{background:#0d0d1a;color:#556;border:1px solid #1a2a3a;"
            "border-bottom:none;border-radius:4px 4px 0 0;"
            "padding:6px 10px;font-size:10px;min-width:58px;}"
            "QTabBar::tab:selected{background:#14142a;color:#dde;border-color:#3a3a6a;}"
            "QTabBar::tab:hover{background:#12122a;color:#9ab;}")

        self._panels  = {}
        self._roi_mgr = None   # RoiManagerPanel 인스턴스 (screen/camera 내부 탭 통합)
        ...
        for key, label, color in _SECTION_DEFS:
            ...
            inner = QWidget()
            inner.setStyleSheet("background:#0d0d1e;")
            inner_v = QVBoxLayout(inner)
            inner_v.setContentsMargins(8,8,8,12); inner_v.setSpacing(8)

            panel = self._make_panel(key)
            self._panels[key] = panel
            inner_v.addWidget(panel); inner_v.addStretch()
        ...
        root_h.addWidget(self._tabs)

        # ── 우측: 미리보기 + 상태 ─────────────────────────────────────────
        right_v = QVBoxLayout(); right_v.setSpacing(6)

        # 미리보기 버튼
        prev_btns = QHBoxLayout(); prev_btns.setSpacing(6)
        self._disp_btn = QPushButton("🖥 Display 미리보기 창")
        self._disp_btn.setCheckable(True); self._disp_btn.setFixedHeight(32)
        self._disp_btn.setStyleSheet(
            "QPushButton{background:#1a2a3a;color:#7bc8e0;"
            "border:1px solid #2a4a6a;border-radius:4px;font-size:11px;font-weight:bold;}"
            "QPushButton:checked{background:#0d1a2a;border-color:#3a6a9a;}"
            "QPushButton:hover{background:#22334a;}")
        self._disp_btn.toggled.connect(
            lambda c: self._disp_win.show_win() if c else self._disp_win.hide_win())

        self._cam_btn = QPushButton("📷 Camera 미리보기 창")
        self._cam_btn.setCheckable(True); self._cam_btn.setFixedHeight(32)
        self._cam_btn.setStyleSheet(
            "QPushButton{background:#1a2a1a;color:#f0c040;"
            "border:1px solid #4a4a20;border-radius:4px;font-size:11px;font-weight:bold;}"
            "QPushButton:checked{background:#0d1a0d;border-color:#7a7a30;}"
        ...
        scan_btn = QPushButton("🔄 장치 스캔")
        scan_btn.setFixedHeight(32)
        scan_btn.clicked.connect(self._scan_devices)

        # ★ v2.9: API Doc 버튼
        api_btn = QPushButton("📖 API Doc")
        api_btn.setFixedHeight(32)
        api_btn.setToolTip("커널 스크립트에서 사용 가능한 API 레퍼런스")
        ...
        prev_btns.addWidget(self._disp_btn, 1)
        prev_btns.addWidget(self._cam_btn, 1)
        prev_btns.addWidget(scan_btn)
        prev_btns.addWidget(api_btn)
        right_v.addLayout(prev_btns)

        # 인라인 미리보기 (소형)
        inline_splitter = QSplitter(Qt.Vertical)
        self._scr_inline = PreviewLabel("screen", self._engine)
        self._scr_inline.setMinimumHeight(180)
        self._cam_inline = PreviewLabel("camera", self._engine)
        self._cam_inline.setMinimumHeight(180)
        inline_splitter.addWidget(self._scr_inline)
        inline_splitter.addWidget(self._cam_inline)
        inline_splitter.setSizes([300, 300])
        ...
        # IO채널 상태
        self._io_lbl = QLabel("IO채널: 초기화 중…")
        ...
        root_h.addLayout(right_v, 1)
        # ROI 매니저는 _make_panel("roi") 에서 생성됨 — _build_ui 완료 후 자동 연결
    ...
    def _refresh_roi_panels(self):
        if self._roi_mgr: self._roi_mgr._refresh()
    ...
    def _slow_tick(self):
        ...
        # IO채널 상태
        if self._engine._io_db:
            try:
                recording = self._engine._io_db.get_state("recording", False)
    ...
    def _load_settings(self):
        ...
        if hasattr(self, '_kernel_panel'):
            ...
            if raw:
                try:
                    data = json.loads(raw)
                    self._kernel_panel.set_scripts_data(data)
                except Exception:
                    pass
    ...
    def _save_settings(self):
        ...
        if hasattr(self, '_kernel_panel'):
            try:
                data = self._kernel_panel.get_scripts_data()
                db.set("kernel_scripts", json.dumps(data, ensure_ascii=False))
            except Exception:
                pass
