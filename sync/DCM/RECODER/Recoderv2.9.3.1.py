    ...
    def update_frame(self, frame: np.ndarray):
        ...
        for i, roi in enumerate(self._rois()):
            ...
            def _draw_label(img, text, x, y, color=(255,255,255),
                ...
                x1 = max(x - 1, 0);      y1 = max(y - th - 2, 0)
                x2 = min(x + tw + 3, img.shape[1]-1)
                y2 = min(y + bl + 2, img.shape[0]-1)
                sub = img[y1:y2, x1:x2]
                if sub.size > 0:
                    img[y1:y2, x1:x2] = (sub * 0.40).astype(sub.dtype)
                cv2.putText(img, text, (x, y), font,
                            scale, color, thickness, cv2.LINE_AA)

            # ── 이름 (좌상단 외부) ───────────────────────────────────────
            name_lbl = roi.name if roi.name else f"ROI{i+1}"
            _draw_label(disp, name_lbl,
                        rx, max(ry - 2, 12), color=(255, 80, 80))

            # ── 밝기값  L:xxx  (좌하단 내부) ────────────────────────────
            _draw_label(disp, f"L:{roi.last_brightness:.0f}",
                        rx + 2, ry + rh - 4,
                        color=(255, 200, 60), scale=0.38)

            # ── OCR 결과 (우하단 내부) ★ v2.9.3 ─────────────────────────
            ocr_txt = roi.last_text.strip() if roi.last_text else ""
            if ocr_txt and not ocr_txt.startswith("~"):
                show = ocr_txt[:18]
                font  = cv2.FONT_HERSHEY_SIMPLEX
                scale = 0.44
                (tw, _), _ = cv2.getTextSize(show, font, scale, 1)
                ox = rx + rw - tw - 4
                oy = ry + rh - 4
                _draw_label(disp, show,
                            max(ox, rx + 2), oy,
                            color=(60, 230, 255), scale=scale)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [ROI 조건 판단 — 읽은 OCR 값으로 분기 ★ v2.9.3]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  kernel.roi_text_equals(roi_idx, value, source="screen",
                         ignore_case=True) → bool
      → OCR 텍스트 == value 이면 True

  kernel.roi_text_contains(roi_idx, value, source="screen",
                           ignore_case=True) → bool
      → OCR 텍스트에 value 포함이면 True

  kernel.roi_number_equals(roi_idx, value, source="screen",
                           tolerance=0.0) → bool
      → OCR 숫자 == value (±tolerance) 이면 True

  kernel.roi_number_compare(roi_idx, op, value,
                            source="screen") → bool
      → op: "==" | "!=" | ">" | ">=" | "<" | "<="
      → OCR 숫자를 op로 value와 비교

  kernel.roi_match(roi_idx, source="screen") → bool
      → ROI 관리 패널에서 설정한 조건값과 매치 여부 (UI 연동)

  ┌── 예시 — ROI 값이 2가 아닐 때 FAIL ─────────────────────────┐
  │  # 방법 1: 조건 함수 사용 (권장)                             │
  ...
  │  elif kernel.roi_text_contains(0, "ERR"):                    │
  │      kernel.set_tc_result("FAIL")                            │
  │                                                              │
  │  # UI 패널 조건값과 동기화                                   │
  ...
  ┌── ROI 패널 UI 사용법 ─────────────────────────────────────────┐
  │  ROI 관리 패널 각 행 하단에 [OCR =] 입력란이 있음.           │
  │  비교값을 입력하면 (예: 2, OK, >=10):                        │
  │    · 미리보기 OCR 결과와 실시간 비교                         │
  │    · ✅ MATCH / ✗ NO 로 즉시 표시                            │
  │    · kernel.roi_match(idx) 로 스크립트에서도 접근 가능       │
  └──────────────────────────────────────────────────────────────┘
