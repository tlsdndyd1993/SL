...
class MemoPanel(QWidget):
    ...
    def _build(self):
        ...
        tc.addWidget(exp_tb); tc.addStretch(); v.addLayout(tc)
    ...
    def _export_txt(self):
        ...
        # 저장 경로: 메모 내용에서 첫 줄이나 탭 이름을 파일명으로
...
class PathSettingsPanel(QWidget):
    ...
    class _PathDragList(QWidget):
        ...
        def _make_row(self, item: dict) -> QWidget:
            ...
            bg  = "#0d1525" if fixed else self._BG
            bdr = "#2a4a6a" if fixed else "#1a1a3a"
            row.setStyleSheet(
                f"QWidget{{background:{bg};border:1px solid {bdr};"
                "border-radius:4px;}")
            hl = QHBoxLayout(row); hl.setContentsMargins(6,2,6,2); hl.setSpacing(6)

            if fixed:
                pin = QLabel("📌"); pin.setFixedWidth(16)
                pin.setStyleSheet("font-size:11px;")
            else:
                pin = QLabel("⠿"); pin.setFixedWidth(16)
                pin.setStyleSheet("color:#4a5a8a;font-size:18px;")
                row.setCursor(Qt.OpenHandCursor)

            lbl = QLabel(item['label'])
            lbl.setStyleSheet(
                f"color:{'#7bc8e0' if fixed else '#ccd'};"
                "font-size:11px;background:transparent;")
            hl.addWidget(pin); hl.addWidget(lbl, 1)
            return row
        ...
        def mousePressEvent(self, e):
            if e.button() == Qt.LeftButton:
                ...
                if idx >= 0 and not self._items[idx].get('fixed', False):
                    self._drag_idx  = idx
        ...
        def mouseReleaseEvent(self, e):
            if self._dragging and self._drag_idx >= 0:
                ...
                if 0 <= tgt < len(self._items) and tgt != self._drag_idx \
                        and not self._items[tgt].get('fixed', False):
                    # 순서 교환
                    ...
                    w = self._items[self._drag_idx].get('widget')
                    if w:
                        fixed = self._items[self._drag_idx].get('fixed', False)
                        bg  = "#0d1525" if fixed else self._BG
                        bdr = "#2a4a6a" if fixed else "#1a1a3a"
                        w.setStyleSheet(
                            f"QWidget{{background:{bg};border:1px solid {bdr};"
    ...
    def _build(self):
        ...
        # ══ 1. T/C 검증 목적 (기능별) ════════════════════════════════════════
        tc_g = QGroupBox("🔬 T/C 검증 목적  (기능별 개별 활성화)")
        tc_g.setStyleSheet(
            "QGroupBox{border:1px solid #4a3a1a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#f0c040;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        tc_lay = QVBoxLayout(tc_g)
        tc_lay.setContentsMargins(10, 8, 10, 10); tc_lay.setSpacing(8)

        tc_hint = QLabel(
            "활성화된 기능의 데이터 처리 완료 후 PASS/FAIL 선택 → 아래 '태그 적용 폴더'에 (PASS)/(FAIL) 태그 삽입\n"
        ...
        tc_lay.addWidget(self.tc_custom_folder_ed)
        v.addWidget(tc_g)

        # ══ 2. TC-ID 범위 설정 ═══════════════════════════════════════════════
        range_g = QGroupBox("🔢 TC-ID 설정")
        range_g.setStyleSheet(
            "QGroupBox{border:1px solid #2a3a5a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#7bc8e0;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        range_lay = QGridLayout(range_g)
        range_lay.setContentsMargins(10,8,10,10); range_lay.setSpacing(8)

        range_lay.addWidget(QLabel("prefix"), 0, 0)
        ...
        range_lay.addWidget(self.tc_prefix_ed, 0, 1, 1, 3)

        range_lay.addWidget(QLabel("번호 범위"), 1, 0)
        self.tc_start_spin = QSpinBox()
        self.tc_start_spin.setRange(0, 99999); self.tc_start_spin.setValue(1)
        self.tc_start_spin.setFixedWidth(70)
        self.tc_start_spin.setToolTip("시작 번호")
        ...
        range_lay.addWidget(self.tc_start_spin, 1, 1)
        range_lay.addWidget(QLabel("~"), 1, 2)

        self.tc_end_spin = QSpinBox()
        self.tc_end_spin.setRange(0, 99999); self.tc_end_spin.setValue(99999)
        self.tc_end_spin.setFixedWidth(70)
        self.tc_end_spin.setToolTip("끝 번호 (현재 번호 제한)")
        self.tc_end_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;"
            "border-radius:3px;padding:3px;font-size:12px;font-weight:bold;}")
        range_lay.addWidget(self.tc_end_spin, 1, 3)

        range_lay.addWidget(QLabel("현재 번호"), 2, 0)
        self.tc_cur_spin = QSpinBox()
        self.tc_cur_spin.setRange(0, 99999); self.tc_cur_spin.setValue(1)
        self.tc_cur_spin.setFixedWidth(80)
        self.tc_cur_spin.setToolTip("현재 TC-ID 번호  (마우스 휠로 조절)")
        self.tc_cur_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#2ecc71;border:1px solid #2a6a2a;"
            "border-radius:3px;padding:3px;font-size:13px;font-weight:bold;}")
        self.tc_cur_spin.valueChanged.connect(self._upd_tc_id)
        range_lay.addWidget(self.tc_cur_spin, 2, 1)

        range_lay.addWidget(QLabel("자릿수"), 2, 2)
        ...
        range_lay.addWidget(self.tc_digits_spin, 2, 3)
        ...
        range_lay.addWidget(self.tc_preview_lbl, 3, 0, 1, 4)
        range_lay.setColumnStretch(1, 1)
        v.addWidget(range_g)

        # ══ 3. 저장 경로 항목 (드래그앤드랍) ══════════════════════════════════
        path_g = QGroupBox("📁 저장 경로 구성  (드래그로 순서 변경)")
        path_g.setStyleSheet(
            "QGroupBox{border:1px solid #2a3a5a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#7bc8e0;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        path_outer = QVBoxLayout(path_g)
        path_outer.setContentsMargins(10,8,10,8); path_outer.setSpacing(6)

        path_hint = QLabel(
            "📌 고정 항목(차종_버전·TC-ID)은 순서 고정  |  추가 항목은 드래그로 이동\n"
            "차종_버전 입력:")
        path_hint.setStyleSheet("color:#556;font-size:9px;")
        path_outer.addWidget(path_hint)
        ...
        path_outer.addWidget(self._extra_container)
        ...
        path_outer.addWidget(add_btn)
        v.addWidget(path_g)

        # ══ 4. 기능별 저장 경로 선택 ══════════════════════════════════════════
        feat_g = QGroupBox("🗂 기능별 저장 경로  (OFF=기본경로 / ON=지정경로)")
        feat_g.setStyleSheet(
            "QGroupBox{border:1px solid #2a3a5a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#7bc8e0;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        feat_lay = QVBoxLayout(feat_g)
        feat_lay.setContentsMargins(10,8,10,10); feat_lay.setSpacing(6)
        ...
        feat_grid = QGridLayout(); feat_grid.setSpacing(8)
        for i, (engine_attr, label, color, chk_attr) in enumerate(_FEATS):
            ...
            feat_grid.addWidget(chk, i//2, i%2)
        feat_lay.addLayout(feat_grid)
        v.addWidget(feat_g)

        # ══ 5. 경로 미리보기 ══════════════════════════════════════════════════
        prev_g = QGroupBox("🔍 경로 미리보기")
        prev_g.setStyleSheet(
            "QGroupBox{border:1px solid #1a2a3a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#556;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        prev_lay = QVBoxLayout(prev_g)
        prev_lay.setContentsMargins(10,6,10,8)
    ...
    def _upd_tc_id(self):
        ...
        num    = self.tc_cur_spin.value()
        digits = self.tc_digits_spin.value()
        # 범위 제한
        lo = self.tc_start_spin.value()
        hi = self.tc_end_spin.value()
        if num < lo: num = lo; self.tc_cur_spin.blockSignals(True); self.tc_cur_spin.setValue(lo); self.tc_cur_spin.blockSignals(False)
        if num > hi: num = hi; self.tc_cur_spin.blockSignals(True); self.tc_cur_spin.setValue(hi); self.tc_cur_spin.blockSignals(False)
        tc_val = f"{prefix}{num:0{digits}d}"
        self.engine.tc_id = tc_val
        self.tc_preview_lbl.setText(f"TC-ID: {tc_val}  (범위: {lo:0{digits}d} ~ {hi:0{digits}d})")
        self._upd_preview()

    # ── 동적 경로 항목 ────────────────────────────────────────────────────────
    def _rebuild_drag_list(self):
        """드래그 리스트를 현재 _seg_rows 기준으로 재구성."""
        items = []
        items.append({'key': '__vehicle__', 'label': f"📌 차종_버전  [{self.engine.vehicle_type or '미입력'}]", 'fixed': True})
        items.append({'key': '__date__',    'label': '📌 YYYYMMDD  (자동)', 'fixed': True})
        items.append({'key': '__tcid__',    'label': f"📌 TC-ID  [{self.engine.tc_id or '미설정'}]", 'fixed': True})
        for i, seg in enumerate(self._seg_rows):
            items.append({'key': f'seg_{i}', 'label': f"➕ {seg.get('val','')or '(비어있음)'}", 'fixed': False})
        items.append({'key': '__suffix__',  'label': '📌 Rec_HHMMSS / blackout / … (자동)', 'fixed': True})
    ...
    def load_from_db(self):
        ...
        # TC-ID 파싱
        tc_raw = ps.get('tc_id', '')
        m = re.match(r'^(.*?)(\d+)$', tc_raw)
        if m:
            prefix  = m.group(1)
            num_str = m.group(2)
            self.tc_prefix_ed.setText(prefix)
            self.tc_cur_spin.setValue(int(num_str))
            self.tc_digits_spin.setValue(max(1, len(num_str)))
        else:
