...
class PreviewLabel(QLabel):
    ...
    def update_frame(self, frame: np.ndarray):
        ...
        for i, roi in enumerate(self._rois()):
            ...
            # ── 오버레이: 이름 + 밝기 + OCR 캐시 텍스트 ────────────────
            name_lbl = roi.name if roi.name else f"ROI{i+1}"
            cv2.putText(disp, name_lbl,
                        (rx, max(ry-4, 12)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.42, (255,60,60), 1)

            # 밝기 표시 (ROI 안 좌하단)
            bright_lbl = f"{roi.last_brightness:.0f}"
            cv2.putText(disp, bright_lbl,
                        (rx+2, ry+rh-4),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.38, (255,200,60), 1)

            # OCR 결과 캐시가 있으면 표시 (ROI 안 상단)
            if roi.last_text and not roi.last_text.startswith("~"):
                text_lbl = roi.last_text[:12]  # 최대 12자
                cv2.putText(disp, text_lbl,
                            (rx+2, ry+14),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.40, (60,255,200), 1)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  kernel.read_roi_text(roi_idx, source="screen",
      ...
      → OCR 텍스트 인식.
        우선순위: pytesseract → easyocr → cv2 윤곽 fallback
        lang="kor+eng" 이면 한국어 포함 인식
        numeric_only=True 면 숫자·소수점·부호만 추출
        결과는 RoiItem.last_text 에 자동 캐시됨
        미리보기 창에 ROI 오버레이로도 표시됨
  ...
  예시 — "ROI 값이 2가 아닐 경우 클립 저장":
    n = kernel.read_roi_number(0, "screen")
    if n is not None and n != 2.0:
        log(f"값 {n} 이 2가 아님 — FAIL 처리")
        engine.save_manual_clip()
        kernel.set_tc_result("FAIL")

  예시 — "ROI 텍스트가 'OK'가 아닐 경우":
    txt = kernel.read_roi_text(0, "screen").strip().upper()
    if txt != "OK":
        log(f"텍스트 '{txt}' 불일치 — FAIL")
        kernel.set_tc_result("FAIL")
...
class KernelEngine:
    ...
    def read_roi_text(self, roi_idx: int, source: str = "screen",
        ...
        """
        ROI 영역의 텍스트/숫자를 OCR로 읽어 반환.
        결과는 RoiItem.last_text에 캐시됨.

        Parameters
        ----------
        roi_idx      : ROI 인덱스 (0-based)
        source       : "screen" | "camera"
        lang         : pytesseract 언어 (기본 "eng", 한국어 "kor+eng")
        numeric_only : True면 숫자·소수점·부호만 추출
        ...
        사용 예 (커널 스크립트):
            val_str = kernel.read_roi_text(0, "screen", numeric_only=True)
            if val_str.strip() != "2":
                engine.save_manual_clip()
        """
        ...
        # ── 전처리: 이진화 + 스케일업 → OCR 정확도 향상 ────────────────
        # ── 전처리 함수들 ────────────────────────────────────────────────
        def _to_gray(img):
            return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        def _upscale(gray, target_h=80):
            """항상 일정 높이 이상으로 확대 (OCR 정확도 핵심)."""
            h, w = gray.shape
            if h < target_h:
                scale = target_h / h
        ...
        def _binarize_light_bg(gray):
            """밝은 배경 + 어두운/컬러 텍스트 (흰/연파랑 배경 + 파란/검정 글씨)."""
            _, th = cv2.threshold(gray, 0, 255,
                                  cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            return th

        def _binarize_dark_bg(gray):
            """어두운 배경 + 밝은 텍스트."""
            _, th = cv2.threshold(gray, 0, 255,
        ...
        def _pad(img, px=10):
            """테두리 여백 추가 (Tesseract 정확도 향상)."""
            return cv2.copyMakeBorder(img, px, px, px, px,
                                      cv2.BORDER_CONSTANT, value=255)

        def _make_candidates(bgr_img):
            """
            여러 전처리 버전을 생성.
            밝은배경/어두운배경/채널분리 등 모두 시도.
            """
            cands = []
            gray  = _to_gray(bgr_img)
            gray  = _upscale(gray, target_h=100)
            gray  = _denoise(gray)

            # 1) Otsu 이진화 (밝은배경)
            th1 = _binarize_light_bg(gray)
            th1 = _morphology(th1)
            cands.append(_pad(th1))

            # 2) Otsu 이진화 반전 (어두운배경)
            th2 = _binarize_dark_bg(gray)
            th2 = _morphology(th2)
            cands.append(_pad(th2))

            # 3) 적응형 (경계가 복잡한 배경)
            th3 = cv2.adaptiveThreshold(
                gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                cv2.THRESH_BINARY, 15, 10)
            cands.append(_pad(_morphology(th3)))

            # 4) 컬러 채널 분리 — 파란 텍스트는 R채널에서 어둡게 나옴
            b, g, r = cv2.split(bgr_img)
            for ch in (r, g, b):
                ch_up = _upscale(ch, target_h=100)
                _, th = cv2.threshold(ch_up, 0, 255,
                                      cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                cands.append(_pad(_morphology(th)))

            return cands

        text = ""

        # ── OCR 엔진 1: pytesseract — 다중 전처리 전략 ──────────────────
        try:
            import pytesseract as _tess
            from PIL import Image as _PIL

            # 시간포맷(16:53.95)과 일반텍스트 모두 인식하려면
            # whitelist 없이 psm 7/8 둘 다 시도
            cfgs = [
                "--psm 7 --oem 3",       # 단일 행
                "--psm 8 --oem 3",       # 단일 단어
                "--psm 13 --oem 3",      # Raw line (레이아웃 분석 없음)
            ]
            if numeric_only:
                # 숫자·콜론·점·부호 포함 (시간포맷 대응)
                cfgs = [
            ...
            for img_bin in cands:
                pil = _PIL.fromarray(img_bin)
                for cfg in cfgs:
                    try:
                        t = _tess.image_to_string(
                            pil, lang=lang, config=cfg).strip()
                        # 더 긴 결과 우선 (빈 문자열 제외)
                        t_clean = t.replace(" ", "")
                        if len(t_clean) > len(best.replace(" ", "")):
                            best = t
                    except Exception:
                        continue
                if best:
                    break   # 첫 이미지에서 결과 나오면 나머지 스킵

            text = best
        except ImportError:
            pass
        except Exception as ex:
            self._log(f"[OCR/tess] {ex}")
        ...
        if not text:
            try:
                ...
                # 원본 컬러 이미지 그대로 전달 (easyocr 자체 전처리가 더 나음)
                results = self._easyocr_reader.readtext(region, detail=0)
                text = " ".join(str(r) for r in results).strip()
            except ImportError:
                pass
            except Exception as ex:
                self._log(f"[OCR/easy] {ex}")

        # ── OCR 엔진 3: 없음 (cv2 윤곽 fallback 제거 — 무의미한 결과) ──
        # fallback 결과 (~Ncnts)는 혼란만 줘서 제거함
    ...
    def get_env_info(self) -> str:
        ...
        try:
            ...
            try:
                ...
                lines.append(f"  ⚠ pytesseract 설치됨 but Tesseract 실행파일 없음")
                lines.append(f"    → https://github.com/UB-Mannheim/tesseract/wiki")
                lines.append(f"    → 설치 후 PATH에 추가하거나 pytesseract.pytesseract.tesseract_cmd 설정")
        except ImportError:
