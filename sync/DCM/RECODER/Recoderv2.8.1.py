...
class CoreEngine:
    ...
    def _apply_tc_result_to_folder(self, result: str, target_dir: str = "") -> str:
        ...
        target_dir 생략 시 self.output_dir 사용.
        성공 시 새 경로 반환, 실패 시 원본 반환.
        """
        d = target_dir or self.output_dir
    ...
    def _save_bo_clip(self, source: str, timestamp: str):
        ...
        if self.tc_blackout_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            self._apply_tc_result_to_folder(self.tc_verify_result, src_dir)
    ...
    def stop_recording(self):
        ...
        if self.tc_rec_enabled and self.tc_verify_result in ("PASS","FAIL"):
            self.output_dir = self._apply_tc_result_to_folder(
                self.tc_verify_result, self.output_dir)
    ...
    def _do_manual(self, source: str):
        ...
        if self.tc_manual_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            self._apply_tc_result_to_folder(self.tc_verify_result, save_dir)
...
class PathSettingsPanel(QWidget):
    ...
    def get_tc_target_folder(self) -> str:
        ...
        if idx == 0:   return self.engine.tc_id or ""          # TC-ID 폴더
        elif idx == 1: return self.engine.vehicle_type or ""   # 차종_버전
        elif idx == 2: return self.engine.output_dir or ""     # Rec_ 폴더
        else:          return self.tc_custom_folder_ed.text().strip()
