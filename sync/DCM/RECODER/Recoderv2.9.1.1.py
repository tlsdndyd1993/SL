...
class KernelEngine:
    ...
    def get_env_info(self) -> str:
        """현재 Python 환경 정보 문자열 반환 (커널 로그/Doc용)."""
        ...
        pkgs = [
            ...
            "pynput", "fastapi", "uvicorn", "pytesseract",
        ]
        for pkg in pkgs:
            try:
                mod = __import__(pkg)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v2.9.1")
    ...
    def _build_ui(self):
        ...
        # ════════════════════════════════════════════════════════════════
        left_w = QWidget(); left_w.setFixedWidth(500)
        ...
        self._move_up_btn.setFixedSize(26,26)
        self._move_up_btn.setToolTip("선택 기능 위로")
        self._move_up_btn.setStyleSheet(
            "QPushButton{background:#1a2a3a;color:#7bc8e0;"
            "border:1px solid #2a4a6a;border-radius:3px;font-size:11px;}"
            "QPushButton:hover{background:#223344;}")
        self._move_up_btn.clicked.connect(lambda: self._move_section(-1))

        self._move_dn_btn = QPushButton("▼")
        self._move_dn_btn.setFixedSize(26,26)
        self._move_dn_btn.setToolTip("선택 기능 아래로")
        self._move_dn_btn.setStyleSheet(
            "QPushButton{background:#1a2a3a;color:#7bc8e0;"
            "border:1px solid #2a4a6a;border-radius:3px;font-size:11px;}"
            "QPushButton:hover{background:#223344;}")
        self._move_dn_btn.clicked.connect(lambda: self._move_section(+1))

        # 전체 펼치기/접기
        expand_btn = QPushButton("⊞ 전체")
        expand_btn.setFixedHeight(26)
        expand_btn.setStyleSheet(
            "QPushButton{background:#1a1a3a;color:#9ab;"
            "border:1px solid #2a2a4a;border-radius:3px;font-size:10px;padding:0 6px;}"
            "QPushButton:hover{background:#22224a;}")
        expand_btn.clicked.connect(lambda: self._set_all_collapsed(False))

        collapse_btn = QPushButton("⊟ 접기")
        ...
        tb_lay.addWidget(self._move_up_btn)
        tb_lay.addWidget(self._move_dn_btn)
        tb_lay.addSpacing(6)
        ...
        # ── 기능 순서 선택 리스트 (컴팩트, 토글 체크박스 포함) ──────────
        self._feat_list = QWidget()
        self._feat_list.setStyleSheet("background:#080818;")
        self._feat_list_lay = QVBoxLayout(self._feat_list)
        self._feat_list_lay.setContentsMargins(4,2,4,2)
        self._feat_list_lay.setSpacing(1)

        feat_scroll = QScrollArea()
        feat_scroll.setWidgetResizable(True)
        feat_scroll.setFixedHeight(130)
        ...
        self._feat_rows: dict = {}   # key → QWidget (리스트 행)
        ...
        root_h.addWidget(left_w)

        # ════════════════════════════════════════════════════════════════
        #  우측: 미리보기 + 상태
        # ════════════════════════════════════════════════════════════════
        right_v = QVBoxLayout(); right_v.setSpacing(4)
        ...
        # ▶ 미리보기 창 버튼
        self._disp_btn = QPushButton("🖥 Display 창")
        self._disp_btn.setCheckable(True); self._disp_btn.setFixedHeight(28)
        ...
        self._cam_btn = QPushButton("📷 Camera 창")
        self._cam_btn.setCheckable(True); self._cam_btn.setFixedHeight(28)
        ...
        scan_btn.setFixedHeight(28); scan_btn.clicked.connect(self._scan_devices)
        scan_btn.setStyleSheet(
            "QPushButton{background:#1a1a2a;color:#9ab;"
            "border:1px solid #2a2a4a;border-radius:4px;font-size:10px;}"
            "QPushButton:hover{background:#22224a;}")

        btn_row1.addWidget(self._disp_btn, 1)
        btn_row1.addWidget(self._cam_btn, 1)
        btn_row1.addWidget(scan_btn)

        # ▶ 미리보기 ON/OFF (영상 처리 중지 — 자원 절약) ── ★ 수정1 복구
        self._scr_preview_btn = QPushButton("🖥 Display 출력 ▶ ON")
        self._scr_preview_btn.setCheckable(True)
        self._scr_preview_btn.setChecked(True)
        self._scr_preview_btn.setFixedHeight(28)
        self._scr_preview_btn.setStyleSheet(
            "QPushButton{background:#1a2a3a;color:#7bc8e0;"
            "border:1px solid #2a4a6a;border-radius:4px;font-size:10px;font-weight:bold;}"
        ...
        self._cam_preview_btn = QPushButton("📷 Camera 출력 ▶ ON")
        self._cam_preview_btn.setCheckable(True)
        self._cam_preview_btn.setChecked(True)
        self._cam_preview_btn.setFixedHeight(28)
        self._cam_preview_btn.setStyleSheet(
            "QPushButton{background:#1a2a1a;color:#f0c040;"
            "border:1px solid #4a4a20;border-radius:4px;font-size:10px;font-weight:bold;}"
            "QPushButton:!checked{background:#0a0a0a;color:#334;"
            "border-color:#1a1a1a;}"
            "QPushButton:hover{background:#22331a;}")
        self._cam_preview_btn.toggled.connect(self._on_cam_preview_toggle)

        btn_row2.addWidget(self._scr_preview_btn, 1)
        btn_row2.addWidget(self._cam_preview_btn, 1)
        ...
        self._scr_inline.setMinimumHeight(160)
        self._cam_inline = PreviewLabel("camera", self._engine)
        self._cam_inline.setMinimumHeight(160)
    ...
    def _rebuild_feat_list(self):
        """
        기능 순서 리스트 재빌드.
        각 행: [체크박스(ON/OFF)] [기능명] — 클릭하면 해당 섹션으로 스크롤
        """
        while self._feat_list_lay.count():
            it = self._feat_list_lay.takeAt(0)
            if it.widget(): it.widget().setParent(None)
        ...
        for key in self._section_order:
            ...
            is_selected = (key == self._selected_feat)
            bg = "#1a2a4a" if is_selected else "#0a0a18"
            row.setStyleSheet(
                f"QFrame{{background:{bg};border-radius:3px;}}"
                f"QFrame:hover{{background:#121228;}}")
            rl = QHBoxLayout(row)
            rl.setContentsMargins(4,1,4,1); rl.setSpacing(4)

            chk = QCheckBox()
            chk.setChecked(self._section_visible.get(key, True))
            chk.setStyleSheet(
                "QCheckBox::indicator{width:13px;height:13px;"
                "border:1px solid #3a4a6a;border-radius:3px;background:#0d0d1e;}"
                "QCheckBox::indicator:checked{background:#2980b9;border-color:#3a9ad9;}")
            chk.toggled.connect(lambda v, k=key: self._toggle_section(k, v))
            rl.addWidget(chk)

            color = _colors.get(key, "#7bc8e0")
            lbl = QLabel(_labels.get(key, key))
            lbl.setStyleSheet(
                f"color:{color};font-size:10px;font-weight:bold;background:transparent;")
            lbl.mousePressEvent = lambda e, k=key: self._select_and_scroll(k)
            rl.addWidget(lbl, 1)

            self._feat_rows[key] = row
            self._feat_list_lay.addWidget(row)
    ...
    def _move_section(self, direction: int):
        """선택된 기능을 위(-1)/아래(+1)로 이동."""
        if not self._selected_feat: return
        order = self._section_order
        idx = next((i for i,k in enumerate(order) if k==self._selected_feat), -1)
        if idx < 0: return
        new_idx = idx + direction
        if not 0 <= new_idx < len(order): return
        order[idx], order[new_idx] = order[new_idx], order[idx]
        self._selected_feat = order[new_idx]
        # 섹션 위젯 순서 재배치
    ...
    def _on_scr_preview_toggle(self, on: bool):
        """Display 영상 출력 ON/OFF."""
        self._scr_inline.set_active(on)
        self._scr_preview_btn.setText(
            "🖥 Display 출력 ▶ ON" if on else "🖥 Display 출력 ⏸ OFF")
        if on:
    ...
    def _on_cam_preview_toggle(self, on: bool):
        ...
        self._cam_preview_btn.setText(
            "📷 Camera 출력 ▶ ON" if on else "📷 Camera 출력 ⏸ OFF")
        if on:
