...
class MainWindow(QMainWindow):
    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame):
            """
            단일 ROI OCR 실행.
            pytesseract 우선, 실패/미설치 시 easyocr 시도.
            오류는 roi.last_text 에 [ERR:...] 형태로 기록해 원인 파악 가능.
            """
            rx, ry, rw, rh = roi.rect()
            region = frame[ry:ry+rh, rx:rx+rw]
            if region.size == 0:
            ...
            bins = _make_bins(region)
            best_text = ""
            tess_ran   = False   # pytesseract가 실제로 실행됐는지
            ...
            try:
                ...
                if not exe or not os.path.isfile(exe):
                    ...
                    if status.startswith("not_found"):
                        roi.last_text = "[ERR:Tesseract실행파일없음-PATH확인]"
                        return

                for th in bins:
                    pil = _PIL.fromarray(_pad(th))
                    for cfg in (
                        "--psm 7 --oem 3",
                        "--psm 8 --oem 3",
                        "--psm 13 --oem 3",
                        ...
                        try:
                            ...
                            if "tesseract" in err.lower() or "not found" in err.lower():
                                roi.last_text = f"[ERR:Tess-{err[:40]}]"
                                return
                    if best_text:
                        break
                tess_ran = True
                ...
                pass  # pytesseract 미설치 → easyocr 시도
            except Exception as ex:
                roi.last_text = f"[ERR:{str(ex)[:40]}]"
                return

            # ── OCR 2: easyocr (pytesseract 미설치 또는 결과 없을 때) ─
            if not best_text:
                try:
                    import easyocr as _eocr
                    ...
                    if not tess_ran:
                        roi.last_text = "[ERR:OCR엔진없음-pytesseract/easyocr설치필요]"
                        return
                except Exception as ex:
