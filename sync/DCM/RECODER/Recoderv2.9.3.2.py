...
class KernelEngine:
    ...
    def _ocr_now(self, roi_idx: int, source: str = "screen",
        ...
        def _upscale(img, h=100):
            ih, iw = img.shape[:2]
            if ih < h:
                s = h / ih
                img = cv2.resize(img, (int(iw*s), int(ih*s)),
        ...
        for inv in (False, True):
            ...
            padded = cv2.copyMakeBorder(
                th, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=255)
            try:
                import pytesseract as _tess
                from PIL import Image as _PIL
                for cfg in ("--psm 7 --oem 3", "--psm 8 --oem 3"):
                    t = _tess.image_to_string(
                        _PIL.fromarray(padded),
        ...
        if not best:
            try:
                ...
                res = self._easyocr_reader.readtext(region, detail=0)
                best = " ".join(str(r) for r in res).strip()
            except Exception:
                pass
...
class MainWindow(QMainWindow):
    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame):
            """
            단일 ROI에 대해 OCR 실행.
            pytesseract → easyocr 순서로 시도.
            결과를 roi.last_text 에 저장.
            """
            rx, ry, rw, rh = roi.rect()
            region = frame[ry:ry+rh, rx:rx+rw]
            if region.size == 0:
            ...
            candidates = []
            # Otsu 밝은 배경
            _, th1 = cv2.threshold(gray, 0, 255,
                                   cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            candidates.append(th1)
            # Otsu 반전 (어두운 배경)
            _, th2 = cv2.threshold(gray, 0, 255,
                                   cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            candidates.append(th2)
            # 채널별 — 컬러 텍스트 대응
            for ch_idx in range(3):
                ch = cv2.split(region)[ch_idx]
                ch = _upscale(ch)
                _, th = cv2.threshold(ch, 0, 255,
                                      cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            ...
            try:
                ...
                for th in candidates:
                    ...
                    for cfg in ("--psm 7 --oem 3",
                                "--psm 8 --oem 3",
                                "--psm 13 --oem 3"):
                        try:
                            t = _tess.image_to_string(
                                pil, config=cfg).strip()
                            t_clean = t.replace(" ", "").replace("\n", "")
                            if len(t_clean) > len(best_text.replace(" ", "")):
                                best_text = t_clean
                        except Exception:
                            pass
                    if best_text:
            ...
            # ── easyocr fallback ──────────────────────────────────────
            if not best_text and not tess_ok:
                try:
                    import easyocr as _eocr
                    if not hasattr(self, '_easyocr_reader'):
                        self._easyocr_reader = _eocr.Reader(
                    ...
                    results = self._easyocr_reader.readtext(
                        region, detail=0)
                    best_text = " ".join(str(r) for r in results).strip()
                except ImportError:
                    pass
                except Exception:
                    pass
