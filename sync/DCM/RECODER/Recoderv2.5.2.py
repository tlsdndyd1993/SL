...
class SettingsDB:
    ...
    def get_path_settings(self) -> dict:
        with self._lock:
            with self._conn() as c:
                # extra_segments 컬럼이 없는 구버전 DB 자동 마이그레이션
                try:
                    c.execute("ALTER TABLE path_settings ADD COLUMN extra_segments TEXT DEFAULT '[]'")
                except Exception:
                    pass
                row = c.execute(
                    "SELECT vehicle_type,tc_id,extra_segments FROM path_settings WHERE id=1"
                ).fetchone()
        if not row:
        ...
        with self._lock:
            with self._conn() as c:
                c.execute(
                    ...
                    "(id,vehicle_type,tc_id,extra_segments) VALUES(1,?,?,?)",
                    (vehicle_type, tc_id,
...
class RecordingPanel(QWidget):
    ...
    def _build(self):
        ...
        # ── T/C 검증 목적 (최상단, 모든 기능에 영향) ─────────────────────────
        tc_g=QGroupBox("🔬 T/C 검증 목적  (녹화·캡처·수동녹화 전체 적용)")
        tc_l=QVBoxLayout(tc_g); tc_l.setSpacing(5)
        tc_row=QHBoxLayout(); tc_row.setSpacing(10)
        self.tc_chk=QCheckBox("T/C 검증 목적 활성화")
        self.tc_chk.setStyleSheet(
            "QCheckBox{font-size:12px;font-weight:bold;color:#f0a040;spacing:8px;}")
        self.tc_chk.toggled.connect(lambda c: setattr(self.engine,'tc_verify_enabled',c))
        tc_row.addWidget(self.tc_chk); tc_row.addStretch()
        tc_l.addLayout(tc_row)
        tc_hint=QLabel(
            "활성화 시:  녹화 종료 → PASS/FAIL 선택창  |  캡처 → 파일명에 PASS/FAIL 태그  |  수동녹화 → 동일")
    ...
    def _on_cap_scr(self):
        """Display 캡처 — T/C 검증 활성화 시 PASS/FAIL 선택 후 파일명에 태그."""
        if self.engine.tc_verify_enabled:
            result = self._show_tc_capture_dialog()
    ...
    def _on_cap_cam(self):
        """Camera 캡처 — T/C 검증 활성화 시 PASS/FAIL 선택 후 파일명에 태그."""
        if self.engine.tc_verify_enabled:
            result = self._show_tc_capture_dialog()
            if result is None: return
    ...
    def _show_tc_capture_dialog(self):
        """캡처용 T/C 결과 선택 (간소화 버전). 반환: 'PASS'|'FAIL'|None"""
        result_box = [None]
        dlg = QDialog(self.parent() or self)
        dlg.setWindowTitle("T/C 캡처 결과")
        ...
        lay = QVBoxLayout(dlg); lay.setSpacing(14); lay.setContentsMargins(24,18,24,16)
        lbl = QLabel("캡처 전 T/C 검증 결과를 선택하세요.")
        lbl.setAlignment(Qt.AlignCenter)
        ...
        for txt, res, bg, hov in [
            ...
            b = QPushButton(txt); b.setMinimumHeight(40)
            b.setStyleSheet(f"QPushButton{{background:{bg};color:white;font-size:14px;"
        ...
        btn_cancel.setStyleSheet("QPushButton{background:#1a1a2a;color:#888;border:1px solid #3a3a5a;"
...
class ManualClipPanel(QWidget):
    ...
    def _on_clip(self):
        """
        수동 클립 저장.
        T/C 검증 활성화 시 저장 전에 PASS/FAIL 선택 → engine.tc_verify_result 에 저장.
        폴더명 태그는 engine.save_manual_clip() 완료 후 적용.
        """
        if self.engine.tc_verify_enabled:
...
class PathSettingsPanel(QWidget):
    """
    저장 경로 설정.
    고정 항목: 차종_버전, TC-ID (숫자 휠 조절 가능)
    동적 항목: 사용자가 경로 세그먼트를 자유롭게 추가/삭제/순서변경
    """
    def __init__(self, engine: CoreEngine, db: SettingsDB, parent=None):
        super().__init__(parent)
        self.engine=engine; self.db=db
        self._extra_rows: list = []   # (QLineEdit, 삭제버튼) 리스트
        self._build()
    ...
    def _build(self):
        ...
        info=QLabel(
            "저장 경로 구조:\n"
            "~/Desktop/bltn_rec / [차종_버전] / YYYYMMDD / [TC-ID] / [추가항목…] / Rec_HHMMSS/\n\n"
            "빈 항목은 해당 폴더 단계가 생략됩니다.  "
            "T/C 검증 사용 시: [(PASS)|(FAIL)] 태그가 Rec_ 폴더명 앞에 붙습니다.")
        ...
        # ── 동적 추가 항목 ────────────────────────────────────────────────────
        self._extra_g = QGroupBox("➕ 추가 경로 항목  (원하는 폴더 단계 추가)")
        self._extra_v = QVBoxLayout(self._extra_g); self._extra_v.setSpacing(5)
        self._extra_container = QWidget()
        self._extra_lay = QVBoxLayout(self._extra_container)
        self._extra_lay.setContentsMargins(0,0,0,0); self._extra_lay.setSpacing(4)
        self._extra_v.addWidget(self._extra_container)

        add_seg_btn = QPushButton("＋ 경로 항목 추가"); add_seg_btn.setFixedHeight(28)
        add_seg_btn.setStyleSheet(
            "QPushButton{background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;"
            "border-radius:4px;font-size:11px;}"
            "QPushButton:hover{background:#223344;}")
        add_seg_btn.clicked.connect(self._add_segment)
        self._extra_v.addWidget(add_seg_btn)
        v.addWidget(self._extra_g)
        ...
        self.result_lbl.setStyleSheet("font-size:11px;"); v.addWidget(self.result_lbl)

        self.preview_lbl = QLabel("")
        self.preview_lbl.setStyleSheet("color:#556;font-size:10px;font-family:monospace;")
    ...
    def _add_segment(self, value: str = ""):
        ...
        lbl = QLabel(f"항목 {len(self._extra_rows)+1}:")
        lbl.setStyleSheet("color:#aaa;font-size:10px;min-width:44px;")
        ed = QLineEdit(value)
        ...
        row_l.addWidget(lbl); row_l.addWidget(ed, 1); row_l.addWidget(del_btn)
        self._extra_lay.addWidget(row_w)
        entry = (row_w, ed, lbl)
        self._extra_rows.append(entry)

        del_btn.clicked.connect(lambda: self._del_segment(entry))
        self._upd_extra_engine()
        self._upd_preview()

    def _del_segment(self, entry):
        row_w, ed, lbl = entry
        self._extra_lay.removeWidget(row_w); row_w.deleteLater()
        if entry in self._extra_rows: self._extra_rows.remove(entry)
        # 레이블 번호 재정렬
        for i, (rw, e, lb) in enumerate(self._extra_rows):
            lb.setText(f"항목 {i+1}:")
        self._upd_extra_engine(); self._upd_preview()
    ...
    def _upd_preview(self):
        ...
        vv = self.vehicle_ed.text().strip()
        tc = self.tc_wid.value().strip()
    ...
    def _save(self):
        segs = [ed.text().strip() for _, ed, _ in self._extra_rows if ed.text().strip()]
        self.db.set_path_settings(
            self.vehicle_ed.text().strip(),
            self.tc_wid.value().strip(),
            segs)
