...
class MainWindow(QMainWindow):
    ...
    def _on_overlay_changed(self):
        ...
        """
        # 스크린 스레드가 없거나 종료됐으면 즉시 시작
        if not (self.engine._scr_thread and self.engine._scr_thread.is_alive()):
            self.engine.start_screen()
            self._scr_lbl.set_active(True)
            self._preview_countdown_lbl.setText("미리보기: 활성화됨")
            ...
            self._preview_now_btn.setVisible(False)
        # 카메라 스레드도 마찬가지
        if not (self.engine._cam_thread and self.engine._cam_thread.is_alive()):
            self.engine.start_camera()
