...
class CoreEngine:
    ...
    def start_recording(self):
        ...
        # ★ 수정: idx==2(녹화 폴더 자체) 선택 시 output_dir 확정 후 tc_tag_target_dir 갱신.
        #   그 외 idx는 get_tc_target_folder()가 이미 올바른 경로를 반환하므로 그대로 사용.
        if self.tc_rec_enabled and not self.tc_tag_target_dir:
            # tc_tag_target_dir이 비어있으면(idx==2이고 output_dir 없었던 경우) output_dir로 설정
...
class PathSettingsPanel(QWidget):
    ...
    def _sync_tc_tag_target(self):
        ...
        # idx==2(녹화 폴더 자체)이고 아직 output_dir이 없으면 빈 문자열로 둠.
        # start_recording()에서 output_dir 확정 후 tc_tag_target_dir = output_dir 로 채워짐.
