...
class RecordingPanel(QWidget):
    ...
    def _on_stop(self):
        """
        녹화 종료.
        T/C 검증 활성화 → 먼저 PASS/FAIL 다이얼로그 표시 후 종료.
        다이얼로그는 항상 UI 메인스레드에서 exec_() 로 블로킹 실행.
        """
    ...
    def _show_tc_dialog(self) -> str | None:
        """
        PASS / FAIL 선택 모달 다이얼로그.
        반환값: "PASS" | "FAIL" | None(취소)
        항상 UI 메인스레드에서 호출되어야 함.
        """
        dlg=QDialog(self)
        ...
        dlg.setStyleSheet(
            ...
            "QLabel{color:#dde;}")
        lay=QVBoxLayout(dlg); lay.setSpacing(16); lay.setContentsMargins(24,20,24,20)
        ...
        btn_pass=QPushButton("✅  PASS"); btn_pass.setMinimumHeight(44)
        btn_pass.setStyleSheet(
            "QPushButton{background:#1a6a3a;color:#afffcf;font-size:16px;"
            "font-weight:bold;border-radius:6px;border:2px solid #2a9a5a;}"
            "QPushButton:hover{background:#27ae60;color:white;}")
        btn_fail=QPushButton("❌  FAIL"); btn_fail.setMinimumHeight(44)
        btn_fail.setStyleSheet(
            "QPushButton{background:#6a1a1a;color:#ffaaaa;font-size:16px;"
            "font-weight:bold;border-radius:6px;border:2px solid #aa3a3a;}"
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v2.3")
