    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame, zoom_hint: float = 1.0):
            ...
            def _make_candidates(bgr):
                ...
                # ④ CLAHE 후 Otsu (대비 약한 이미지 대응)
                enh = _enhance_contrast(gray)
                _, th_enh = cv2.threshold(
                    enh, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                cands.append((th_enh, "clahe_otsu"))

                # ⑤ 고정 임계값 (Otsu 실패 시 대비)
                _, th_fix = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
                cands.append((th_fix, "fixed_127"))
            ...
            try:
                ...
                WL = "0123456789ABCDEFabcdef"
                # psm 7: 단일 행 / psm 8: 단어 / psm 6: 균일 블록
                # psm 10: 단일 문자 / psm 11: 드문 텍스트
                # psm 13: raw 라인 (레이아웃 분석 없음)
                DIGIT_CFGS = [
                    f"--psm 7  --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 8  --oem 3 -c tessedit_char_whitelist={WL}",
                    f"--psm 10 --oem 3 -c tessedit_char_whitelist={WL}",
