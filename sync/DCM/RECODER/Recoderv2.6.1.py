...
class RecordingPanel(QWidget):
    ...
    def _build(self):
        ...
        tc_l = QVBoxLayout(tc_g); tc_l.setSpacing(6)
        tc_hint = QLabel(
            "활성화된 기능에서 데이터 처리 완료 후\n"
            "TC-ID 폴더명 앞에 (PASS)/(FAIL) 태그가 붙습니다.\n"
            "⚠ 저장 경로 설정(TC-ID 포함)이 먼저 완료되어야 합니다.")
        tc_hint.setWordWrap(True)
        tc_hint.setStyleSheet("color:#888;font-size:9px;padding:2px 4px;")
        ...
        tc_row = QHBoxLayout(); tc_row.setSpacing(12)
        for attr_name, engine_attr, label, color in _TC_ITEMS:
            chk = QCheckBox(label)
            chk.setStyleSheet(
                f"QCheckBox{{font-size:11px;font-weight:bold;color:{color};"
                "spacing:5px;padding:2px 6px;"
                "border:1px solid #2a2a3a;border-radius:4px;}}"
                f"QCheckBox:checked{{background:#1a1a2a;border-color:{color};}}")
            chk.toggled.connect(self._make_tc_toggle(engine_attr))
            setattr(self, attr_name, chk)
            tc_row.addWidget(chk)
        tc_row.addStretch()
        tc_l.addLayout(tc_row)
        v.addWidget(tc_g)
    ...
    def _make_tc_toggle(self, engine_attr: str):
        """
        TC 체크박스 toggled 콜백 팩토리.
        체크 시 저장 경로(TC-ID) 미설정이면 경고 모달 표시 후 체크 취소.
        """
        def _toggle(checked: bool):
            ...
            if not self.engine.tc_id or not self.engine.tc_id.strip():
                QMessageBox.warning(
                    ...
                    "T/C 검증 목적을 활성화하려면\n"
                    "'저장 경로' 섹션에서\n"
                    "TC-ID를 먼저 설정하고 저장하세요.",
                    QMessageBox.Ok)
                # 해당 체크박스를 다시 off (attr 이름으로 역추적)
                _attr_to_chk = {
...
class PathSettingsPanel(QWidget):
    """
    저장 경로 설정.
    ─ 고정 항목  : 차종_버전, TC-ID (숫자 휠 조절)
    ─ 동적 항목  : 폴더 단계 자유 추가/삭제 (★ ScrollArea 내부에 배치해 크래시 방지)
    ─ 기능별 경로: Recording / 수동녹화 / 블랙아웃 / 캡처 → default vs 지정경로 선택
    """
    def __init__(self, engine: CoreEngine, db: SettingsDB, parent=None):
        super().__init__(parent)
        self.engine = engine
        self.db     = db
        self._extra_rows: list = []   # (container_widget, QLineEdit, QLabel)
        self._build()
    ...
    def _build(self):
        ...
        # ── 안내 ─────────────────────────────────────────────────────────────
        info = QLabel(
            "지정 경로 구조:  bltn_rec / [차종_버전] / YYYYMMDD / [TC-ID] / [추가항목…]\n"
            "기능별 '지정 경로 사용' ON → 위 경로 적용 / OFF → bltn_rec 아래 기본 경로\n"
            "T/C 검증 활성화 시: 처리 완료 후 TC-ID 폴더명 앞에 (PASS)/(FAIL) 태그 삽입")
        info.setWordWrap(True)
        info.setStyleSheet(
            "color:#778;font-size:9px;border:1px solid #1a2a3a;"
            "border-radius:4px;padding:7px 8px;background:#0d0d1e;"
            "line-height:150%;")
        v.addWidget(info)

        # ── 고정 경로 항목 ────────────────────────────────────────────────────
        fixed_g = QGroupBox("📁 고정 경로 항목")
        fixed_g.setStyleSheet(
        ...
        fl = QGridLayout(fixed_g)
        fl.setSpacing(8)
        fl.setContentsMargins(10, 8, 10, 10)

        lbl_vt = QLabel("차종_버전")
        lbl_vt.setStyleSheet("color:#aaa;font-size:11px;min-width:66px;")
        fl.addWidget(lbl_vt, 0, 0)
        ...
        fl.addWidget(self.vehicle_ed, 0, 1)

        lbl_tc = QLabel("TC-ID")
        lbl_tc.setStyleSheet("color:#aaa;font-size:11px;min-width:66px;")
        fl.addWidget(lbl_tc, 1, 0)
        self.tc_wid = _TcIdWidget()
        self.tc_wid.valueChanged.connect(lambda t: setattr(self.engine, 'tc_id', t))
        self.tc_wid.valueChanged.connect(self._upd_preview)
        fl.addWidget(self.tc_wid, 1, 1)

        hint_tc = QLabel("prefix(문자) + 숫자  |  숫자 부분 마우스 휠로 조절 가능")
        hint_tc.setStyleSheet("color:#556;font-size:9px;")
        fl.addWidget(hint_tc, 2, 1)
        fl.setColumnStretch(1, 1)
        v.addWidget(fixed_g)

        # ── 동적 추가 항목 ────────────────────────────────────────────────────
        extra_g = QGroupBox("➕ 추가 경로 항목  (원하는 폴더 단계 자유 추가)")
        extra_g.setStyleSheet(
            "QGroupBox{border:1px solid #2a3a5a;border-radius:6px;margin-top:16px;"
            "font-weight:bold;color:#7bc8e0;font-size:11px;padding-top:8px;}"
            "QGroupBox::title{subcontrol-origin:margin;left:10px;top:-2px;"
            "padding:2px 6px;background:#12122a;border-radius:3px;}")
        extra_outer = QVBoxLayout(extra_g)
        extra_outer.setContentsMargins(10, 8, 10, 8)
        extra_outer.setSpacing(6)

        # 동적 행 컨테이너 (크래시 방지용 별도 위젯)
        ...
        extra_outer.addWidget(self._extra_container)

        add_seg_btn = QPushButton("＋  경로 항목 추가")
        add_seg_btn.setFixedHeight(30)
        ...
        add_seg_btn.clicked.connect(lambda: self._add_segment())
        extra_outer.addWidget(add_seg_btn)
        v.addWidget(extra_g)
        ...
        feat_lay.setContentsMargins(10, 8, 10, 10)
        ...
        for i, (engine_attr, label, color, chk_attr) in enumerate(_FEATS):
            ...
            chk.toggled.connect(
                lambda c, a=engine_attr: setattr(self.engine, a, c))
        ...
        self.preview_lbl.setStyleSheet(
            ...
            "border-radius:4px;padding:8px 10px;line-height:160%;")
        ...
        save_btn.setStyleSheet(
            ...
            "QPushButton:pressed{background:#0d2a1a;}")
    ...
    def _add_segment(self, value: str = ""):
        """
        동적 경로 항목 행 추가.
        ★ 크래시 원인: 이전 구현은 삭제 버튼 clicked를 lambda로 연결할 때
          entry 튜플이 아직 _extra_rows 에 append되기 전에 캡처되어
          None 참조가 발생하거나 레이아웃 포인터가 꼬이는 문제가 있었음.
          → entry를 미리 만들어 append한 뒤 signal 연결로 순서를 보장.
        """
        ...
        # ★ entry 먼저 추가 후 signal 연결 (순서 역전 방지)
        entry = (row_w, ed, num_lbl)
        self._extra_rows.append(entry)
        self._extra_lay.addWidget(row_w)

        ed.textChanged.connect(self._upd_extra_engine)
        ed.textChanged.connect(self._upd_preview)
        del_btn.clicked.connect(lambda _checked=False, e=entry: self._del_segment(e))

        self._upd_extra_engine()
        self._upd_preview()

    def _del_segment(self, entry):
        row_w, ed, lbl = entry
        if entry not in self._extra_rows:
            return
        self._extra_rows.remove(entry)
        self._extra_lay.removeWidget(row_w)
        row_w.setParent(None)   # ★ deleteLater 전에 부모 해제
        row_w.deleteLater()
        # 레이블 번호 재정렬
        for i, (rw, e, lb) in enumerate(self._extra_rows):
            lb.setText(f"항목 {i+1}:")
        self._upd_extra_engine()
        self._upd_preview()

    def _upd_extra_engine(self):
        segs = [ed.text().strip() for _, ed, _ in self._extra_rows]
        self.engine.extra_segments = [s for s in segs if s]

    def _upd_preview(self):
        vv = self.vehicle_ed.text().strip() if hasattr(self, 'vehicle_ed') else ""
        tc = self.tc_wid.value().strip()    if hasattr(self, 'tc_wid')     else ""
        lines = []
        _FEATS = [
        ...
        for eng_attr, label, chk_attr in _FEATS:
            ...
            if use:
                ...
                for _, ed, _ in self._extra_rows:
                    s = ed.text().strip()
                    if s: parts.append(s)
            else:
                parts = ["~/Desktop/bltn_rec", "YYYYMMDD"]
            parts.append(label.split()[-1])   # 기능 폴더 접미사
            lines.append(f"{label}: {os.path.join(*parts)}")
    ...
    def load_from_db(self):
        ...
        self.tc_wid.set_value(ps.get('tc_id', ''))
        # 동적 항목 복원 — 기존 행 먼저 제거
        for entry in list(self._extra_rows):
            self._del_segment(entry)
...
class MainWindow(QMainWindow):
    ...
    def _save_settings(self):
        ...
        # 기능별 T/C 검증 플래그 저장
        db.set("tc_rec",      rp.tc_rec_chk.isChecked())
        db.set("tc_manual",   rp.tc_manual_chk.isChecked())
        db.set("tc_blackout", rp.tc_blackout_chk.isChecked())
        db.set("tc_capture",  rp.tc_capture_chk.isChecked())
        mp2=p["manual"]
    ...
    def _load_settings(self):
        ...
        # 기능별 T/C 검증 플래그 복원
        rp.tc_rec_chk.setChecked(     db.get_bool("tc_rec",      False))
        rp.tc_manual_chk.setChecked(  db.get_bool("tc_manual",   False))
        rp.tc_blackout_chk.setChecked(db.get_bool("tc_blackout", False))
        rp.tc_capture_chk.setChecked( db.get_bool("tc_capture",  False))
        mp2=p["manual"]
