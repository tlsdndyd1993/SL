# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v2.7
────────────────────────────────────────────────────────────────────────────
v2.6 → v2.7 수정사항

  [문제1] 7시간 녹화 시 30분 이후 Display 멈춤 + 영상 깨짐 (258 bytes) 버그 수정
    · 원인: _create_segment가 별도 스레드에서 실행되는 동안
            _screen_loop/camera_loop은 계속 돌고 _seg_start_time이 갱신 안된 상태에서
            elapsed가 30*60 이상 → diff 수만 → 같은 프레임 수만번 기록 → 파일 폭발
    · 수정1: _write_sync에 diff 상한선 (fps*2 = 최대 2초분) 적용
    · 수정2: _create_segment에 _seg_switching 플래그 추가
             플래그 ON → 루프의 writer.write 차단 → writer 생성 완료 후 fidx/seg_start_time 갱신 → 플래그 OFF

  [수정1] TC-ID 범위 설정 (시작 ~ 끝 번호 + 현재 번호 + 자릿수)
  [수정2] 경로 항목 드래그앤드랍 순서 변경
    · 차종_버전, TC-ID, YYYYMMDD, 기능폴더는 📌 고정
    · 추가 항목은 드래그로 순서 변경 가능
  [수정3] T/C 검증 목적 UI를 Recording → 저장 경로 섹션으로 이동
    · 저장 경로 패널 상단에 배치
    · TC 태그 적용 폴더 선택 콤보박스 추가 (TC-ID폴더/차종버전/Rec_폴더/직접입력)
  [수정4] 메모장 타임스탬프 삽입 → 좌클릭 → 더블클릭으로 변경
  [수정5] TC 검증 체크박스 default OFF 적용
  [수정6] Click Macro 중단 단축키 추가 (Ctrl+Alt+X)
  [추가1] 메모장 .txt 내보내기 버튼 (탭명_타임스탬프.txt 저장)
────────────────────────────────────────────────────────────────────────────
  [수정1] 블랙아웃 감지 영상에 타임스탬프·카운트 오버레이 추가
    · 수동녹화와 동일하게 PRE/POST·현재시각·감지발생 카운트(BLACKOUT#N) 표시
    · 감지 발생 프레임에 빨간 테두리 + "BLACKOUT DETECTED #N" 텍스트

  [수정2] '경로 항목 추가' 버튼 누르면 프로그램 뻗는 버그 수정
    · 원인: entry 튜플이 _extra_rows 에 append 되기 전에 삭제 버튼 signal 캡처
    · 수정: entry 먼저 append 후 signal 연결, row_w.setParent(None) 추가
    · ScrollArea 내부에 컨테이너 배치해 레이아웃 포인터 혼선 방지

  [수정3] 저장 경로 기능별 default/지정경로 선택
    · Recording / 수동녹화 / 블랙아웃 / 캡처 각각 OFF=default, ON=지정경로
    · PathSettingsPanel UI에 기능별 '지정 경로 사용' 체크박스 추가
    · DB path_settings 테이블: use_custom_* 컬럼 추가 (구버전 자동 마이그레이션)

  [수정4] T/C 검증 목적 인터페이스를 기능별로 분리
    · 기존 단일 체크박스 → ⏺녹화 / 🎬수동녹화 / ⚡블랙아웃 / 📸캡처 4개로 분리
    · TC-ID 미설정 상태에서 활성화 시 경고 모달창 표시 후 체크 자동 해제
    · engine.tc_verify_enabled 프로퍼티로 하위 호환 유지
────────────────────────────────────────────────────────────────────────────
  [문제1] Camera Feed 창이 메인 창 최소화 시 함께 최소화되는 현상
    · 수정: CameraWindow / DisplayWindow parent=None → 독립 최상위 창

  [문제2] 메모 오버레이 ON 스위치 눌러도 화면에 표시 안됨
    · 수정: _on_overlay_changed() 에서 preview timer 취소 + start_screen/camera
            _on_preview_ready() 에서도 스레드 시작 보장

  [추가1] T/C 검증 목적을 캡처·수동녹화로 확장
    · T/C 검증 체크박스를 Recording 섹션 최상단 배치 (모든 기능에 적용)
    · 캡처 버튼 → PASS/FAIL 선택 후 파일명에 태그 ((PASS)_capture_…png)
    · 수동녹화 → PASS/FAIL 선택 후 클립 저장
    · ManualClipPanel에 T/C 상태 표시 (Recording 패널 설정 반영)

  [추가2] Auto-Click 간격 빠른 증감 버튼
    · +10초 / +1분 / +10분 / -10초 / -1분 / -10분 버튼 추가

  [수정1] 저장 경로 설정 확장
    · 동적 경로 항목 추가/삭제 인터페이스 (＋ 경로 항목 추가)
    · TC-ID 입력 → prefix + 숫자 분리, 숫자 부분 마우스 휠로 조절
    · extra_segments DB 저장/복원
────────────────────────────────────────────────────────────────────────────
────────────────────────────────────────────────────────────────────────────
v2.3 → v2.4 수정사항
...
# =============================================================================
#  유니코드 / 한글 폰트  (백그라운드 로딩 — UI 스레드 블로킹 방지)
# =============================================================================
_FONT_CACHE: dict = {}
_FONT_LOCK = threading.Lock()   # 캐시 동시 접근 방지

def _find_unicode_font(size: int = 18):
    """
    한글 지원 트루타입 폰트 탐색.
    ★ 이 함수는 백그라운드 스레드에서만 호출해야 합니다.
      UI 스레드에서 직접 호출하면 레지스트리/파일시스템 탐색으로 인해 응답 없음 발생.
    """
...
def _get_font(size: int = 18):
    """
    폰트 캐시에서 반환.
    아직 로딩 중이면 None 반환 (오버레이 없이 cv2 fallback 사용).
    """
    with _FONT_LOCK:
...
class SettingsDB:
        ...
        with self._conn() as c:
            c.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS path_settings (
                    ...
                    use_custom_rec  INTEGER DEFAULT 0,
                    use_custom_manual INTEGER DEFAULT 0,
                    use_custom_blackout INTEGER DEFAULT 0,
                    use_custom_capture INTEGER DEFAULT 0,
    ...
    def get_path_settings(self) -> dict:
        with self._lock:
            with self._conn() as c:
                ...
                row = c.execute(
                    ...
                    " use_custom_rec, use_custom_manual, use_custom_blackout, use_custom_capture,"
                    " tc_rec, tc_manual, tc_blackout, tc_capture"
                    " FROM path_settings WHERE id=1"
                ).fetchone()
        if not row:
            return {'vehicle_type':'','tc_id':'','extra_segments':[],
                    'use_custom_rec':False,'use_custom_manual':False,
                    'use_custom_blackout':False,'use_custom_capture':False,
        ...
        return {
            ...
            'use_custom_rec':     bool(row[3]),
            'use_custom_manual':  bool(row[4]),
            'use_custom_blackout':bool(row[5]),
            'use_custom_capture': bool(row[6]),
            'tc_rec':             bool(row[7]),
            'tc_manual':          bool(row[8]),
            'tc_blackout':        bool(row[9]),
            'tc_capture':         bool(row[10]),
        }

    def set_path_settings(self, vehicle_type: str, tc_id: str,
                          extra_segments: list = None,
                          use_custom_rec:  bool = False,
                          use_custom_manual: bool = False,
                          use_custom_blackout: bool = False,
                          use_custom_capture: bool = False,
            ...
            with self._conn() as c:
                c.execute(
                    ...
                    (vehicle_type, tc_id,
                     ...
                     int(use_custom_rec), int(use_custom_manual),
                     int(use_custom_blackout), int(use_custom_capture),
...
class CoreEngine:
    ...
    def __init__(self, signals: Signals, base_dir: str = BASE_DIR):
        ...
        self._seg_switching: bool   = False  # ★ 세그먼트 전환 중 쓰기 차단 플래그
        ...
        # ── T/C 검증 (기능별 독립) ───────────────────────────────────────────
        # 활성화 여부 — 기능별 개별 제어
        self.tc_rec_enabled:      bool = False   # 녹화
        self.tc_manual_enabled:   bool = False   # 수동녹화
        self.tc_blackout_enabled: bool = False   # 블랙아웃
        self.tc_capture_enabled:  bool = False   # 캡처
        # 현재 진행 중인 결과값 (결과 선택 후 폴더명 적용에 사용)
        self.tc_verify_result: str = ""          # "PASS" | "FAIL" | ""
        # 태그를 적용할 폴더 경로 — PathSettingsPanel 콤보박스가 갱신 (기본 "")
        # "": output_dir 사용 / 경로 문자열: 해당 폴더에 태그 삽입
        self.tc_tag_target_dir: str = ""

        # ── 저장 경로 — 기능별 custom/default 전환 ──────────────────────────
        # True → PathSettingsPanel 에서 지정한 경로 사용
        # False → base_dir 아래 default 경로 사용
        self.use_custom_path_rec:      bool = False
        self.use_custom_path_manual:   bool = False
        self.use_custom_path_blackout: bool = False
        self.use_custom_path_capture:  bool = False
        ...
        self.tc_id:           str  = ""
        self.extra_segments:  list = []   # 동적 추가 경로 세그먼트 (순서 유지)

    # ── 하위 호환 프로퍼티 ────────────────────────────────────────────────────
    @property
    def tc_verify_enabled(self) -> bool:
        """RecordingPanel 등 기존 코드 호환 — 녹화 TC 플래그를 반환."""
    ...
    def _make_output_dir(self) -> str:
        """녹화용 출력 경로. use_custom_path_rec=True 면 지정 경로, 아니면 default."""
        return self._build_path("rec")

    def _build_path(self, feature: str) -> str:
        """
        feature: 'rec' | 'manual' | 'blackout' | 'capture'
        custom 경로 사용 여부에 따라 base 결정 후 날짜/TC-ID/extra_segments 조합.
        """
        use_custom = getattr(self, f"use_custom_path_{feature}", False)
        ...
        if use_custom:
            ...
            parts.append(ts_date)
        # 기능별 폴더 접미사
        _suffix = {'rec': f"Rec_{ts_time}", 'manual': f"manual_clip",
                   'blackout': "blackout",   'capture': "capture"}
        parts.append(_suffix.get(feature, feature))
        return os.path.join(*parts)

    def _apply_tc_result_to_folder(self, result: str, target_dir: str = "") -> str:
        """
        대상 폴더명 앞에 (PASS)/(FAIL) 태그 삽입.
        우선순위: target_dir 인자 > self.tc_tag_target_dir > self.output_dir
        성공 시 새 경로 반환, 실패 시 원본 반환.
        """
        d = target_dir or self.tc_tag_target_dir or self.output_dir
        if not d:
            self.signals.status_message.emit("[T/C] 태그 대상 경로 없음 — 건너뜀")
            return d
        # 폴더가 없으면 생성 시도 (TC-ID 폴더가 경로에 없던 경우 대비)
        if not os.path.isdir(d):
            try:
                os.makedirs(d, exist_ok=True)
                self.signals.status_message.emit(f"[T/C] 폴더 생성: {d}")
            except Exception as ex:
                self.signals.status_message.emit(f"[T/C] 폴더 생성 실패: {ex}")
                return d
    ...
    def scan_cameras(self):
        ...
        ※ cap.set() 호출 전체를 try-except 로 감쌈 —
          일부 카메라 드라이버(특히 가상 카메라, IP 카메라)가
          CAP_PROP_FRAME_WIDTH/HEIGHT 변경을 지원하지 않아
          C++ 예외를 발생시킬 수 있음. 무시하고 계속 진행.
        """
        ...
        for idx in range(8):
            ...
            # ★ 해상도 힌트 — 드라이버가 지원하지 않으면 무시
            try:
                cap.set(cv2.CAP_PROP_FRAME_WIDTH,  1280)
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
            except Exception:
                pass   # C++ exception 포함 모든 예외 무시
    ...
    def _save_bo_clip(self, source: str, timestamp: str):
        """
        블랙아웃 클립 저장.
        - 수동녹화와 동일하게 PRE/POST·현재시각·감지발생 카운트 오버레이 삽입
        - use_custom_path_blackout=True 면 지정 경로, 아니면 default 경로
        - tc_blackout_enabled=True 면 저장 완료 후 TC 검증 결과를 폴더명에 반영
        """
        # 저장 경로 결정
        if self.use_custom_path_blackout:
            base = self._build_path("blackout")
            src_dir = os.path.join(base, source.upper())
        else:
        ...
        for i, f in enumerate(all_f):
            ...
            # ── 타임바 (현재시간 + PRE/POST 카운트) ─────────────────────────
            draw_time_bar(fc, ts_str,
                          f"BLACKOUT#{bo_count}  {phase}  {elapsed}")
            # ── 감지 발생 프레임 강조 테두리 ──────────────────────────────────
            if i == bi:
        ...
        if self.tc_blackout_enabled and self.tc_verify_result in ("PASS", "FAIL"):
            tag_dir = self.tc_tag_target_dir or src_dir
    ...
    def _write_sync(self, writer, frame, fps, fidx, elapsed):
        """
        PTS(프레젠테이션 타임스탬프) 동기화 쓰기.
        ★ diff 상한선(fps*2 = 최대 2초분) 적용 —
          세그먼트 전환 직후 elapsed가 순간적으로 튀어 diff가 수만이 되는 버그 방지.
        """
    ...
    def _create_segment(self):
        ...
        새 세그먼트 파일을 생성.
        ★ 안전한 전환 절차:
          1. _seg_switching 플래그로 루프의 쓰기 차단
          2. 기존 writer release
          3. 새 writer 생성
          4. fidx/seg_start_time 갱신
          5. 플래그 해제
        """
        self._seg_switching = True      # ★ 쓰기 루프 차단 시작
        time.sleep(0.05)                # 진행 중인 write 완료 대기
        ...
        # ★ fidx/seg_start_time 을 writer 생성 완료 후 갱신
        self._seg_start_time = time.time()
        self._scr_fidx = 0
        self._cam_fidx = 0
        self._seg_switching = False     # ★ 쓰기 루프 재개
    ...
    def _screen_loop(self):
        with mss.mss() as sct:
            ...
            while not self._scr_stop.is_set():
                ...
                # ★ ROI 사각형을 저장 프레임에도 그림 (미리보기와 동일하게)
                if self.screen_rois:
                    for i, (rx,ry,rw,rh) in enumerate(self.screen_rois):
                        cv2.rectangle(stamped,(rx,ry),(rx+rw,ry+rh),(255,60,60),2)
                        cv2.putText(stamped,f"ROI{i+1}",(rx,max(ry-4,12)),
    ...
    def _camera_loop(self):
        ...
        while not self._cam_stop.is_set():
            ...
            # ★ ROI 사각형을 저장 프레임에도 그림
            if self.camera_rois:
                for i, (rx,ry,rw,rh) in enumerate(self.camera_rois):
                    cv2.rectangle(stamped,(rx,ry),(rx+rw,ry+rh),(255,60,60),2)
                    cv2.putText(stamped,f"ROI{i+1}",(rx,max(ry-4,12)),
                                cv2.FONT_HERSHEY_SIMPLEX,0.45,(255,60,60),1)
            with self._buf_lock:
    ...
    def _do_manual(self, source: str):
        ...
        # ── 저장 경로 결정 ──────────────────────────────────────────────────
        if self.use_custom_path_manual:
    ...
    def capture_frame(self, source: str, tc_tag: str = "") -> str:
        ...
        use_custom_path_capture=True 면 지정 경로, 아니면 base_dir/capture
        """
        if self.use_custom_path_capture:
...
class RecordingPanel(QWidget):
    ...
    def _on_stop(self):
        """
        녹화 종료.
        tc_rec_enabled 활성화 시 PASS/FAIL 모달 표시 후 종료.
        취소 선택 시 녹화 유지.
        """
        if self.engine.tc_rec_enabled and self.engine.recording:
            result = self._show_tc_dialog("녹화를 종료합니다.\nT/C 검증 결과를 선택하세요.",
    ...
    def _show_tc_dialog(self, prompt: str = "T/C 검증 결과를 선택하세요.",
                        cancel_label: str = "취소"):
        """
    ...
    def _on_cap_scr(self):
        """Display 캡처 — tc_capture_enabled 시 PASS/FAIL 선택 후 파일명에 태그."""
        if self.engine.tc_capture_enabled:
            result = self._show_tc_dialog("캡처 전 T/C 검증 결과를 선택하세요.")
    ...
    def _on_cap_cam(self):
        """Camera 캡처 — tc_capture_enabled 시 PASS/FAIL 선택 후 파일명에 태그."""
        if self.engine.tc_capture_enabled:
            result = self._show_tc_dialog("캡처 전 T/C 검증 결과를 선택하세요.")
            if result is None: return
    ...
    def _show_tc_capture_dialog(self):
        """하위 호환용 — _show_tc_dialog() 로 위임."""
        return self._show_tc_dialog("캡처 전 T/C 검증 결과를 선택하세요.")
...
class ManualClipPanel(QWidget):
    ...
    def _build(self):
        ...
        self.tc_status_lbl=QLabel("Recording 패널에서 설정")
    ...
    def _upd_tc_status(self):
        """T/C 검증 활성화 상태를 Recording 패널과 동기화해 표시."""
        if self.engine.tc_manual_enabled:
            ...
            self.tc_status_lbl.setText("비활성  (Recording 패널에서 설정)")
            self.tc_status_lbl.setStyleSheet("color:#556;font-size:9px;")
    ...
    def _on_clip(self):
        """
        수동 클립 저장.
        tc_manual_enabled 활성화 시 PASS/FAIL 선택 후 engine.tc_verify_result 설정.
        """
        if self.engine.tc_manual_enabled:
            # RecordingPanel의 공용 다이얼로그 재사용
            from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton
            result_box = [None]
            dlg = QDialog(self)
            dlg.setWindowTitle("T/C 수동녹화 결과")
            dlg.setWindowModality(Qt.ApplicationModal)
            dlg.setWindowFlags(Qt.Dialog | Qt.WindowTitleHint | Qt.WindowCloseButtonHint)
            dlg.setFixedSize(380, 200)
            dlg.setStyleSheet("QDialog{background:#0d0d1e;}QLabel{color:#dde;}")
            lay = QVBoxLayout(dlg); lay.setSpacing(16); lay.setContentsMargins(28, 20, 28, 18)
            lbl = QLabel("수동녹화 저장 전\nT/C 검증 결과를 선택하세요.")
            lbl.setAlignment(Qt.AlignCenter)
            lbl.setStyleSheet("font-size:13px;font-weight:bold;color:#f0c040;")
            lay.addWidget(lbl)
            btn_row = QHBoxLayout(); btn_row.setSpacing(16)
            for txt, res, bg, hov in [
                ("✅  PASS", "PASS", "#1a6a3a", "#27ae60"),
                ("❌  FAIL", "FAIL", "#6a1a1a", "#c0392b"),
            ]:
                b = QPushButton(txt); b.setMinimumHeight(44)
                b.setStyleSheet(
                    f"QPushButton{{background:{bg};color:white;font-size:15px;"
                    f"font-weight:bold;border-radius:7px;border:none;}}"
                    f"QPushButton:hover{{background:{hov};}}")
                b.clicked.connect(lambda _, r=res: (result_box.__setitem__(0, r), dlg.accept()))
                btn_row.addWidget(b)
            lay.addLayout(btn_row)
            btn_cancel = QPushButton("취소")
            btn_cancel.setStyleSheet(
                "QPushButton{background:#1a1a2a;color:#888;border:1px solid #3a3a5a;"
                "font-size:10px;padding:3px 12px;border-radius:3px;}")
            btn_cancel.clicked.connect(dlg.reject)
            lay.addWidget(btn_cancel, alignment=Qt.AlignCenter)
            dlg.exec_()
            if result_box[0] is None:
                return
            self.engine.tc_verify_result = result_box[0]
...
class AutoClickPanel(QWidget):
    ...
    def _build(self):
        ...
        # ★ 대형 증감 버튼 (+10초 / +1분 / +10분 / -10초 / -1분 / -10분)
        ...
        for delta, label in [(10,"+10초"),(60,"+1분"),(600,"+10분")]:
            ...
            b.clicked.connect(lambda _, d=delta: self.interval_spin.setValue(
...
class MemoPanel(QWidget):
    overlay_changed = pyqtSignal()   # 오버레이 ON/OFF 또는 내용 변경 시 발행
    def __init__(self, engine: CoreEngine, signals: Signals, parent=None):
        super().__init__(parent)
        self.engine=engine; self.signals=signals
        self._editors: list = []
        ...
        self._build()
        # ★ 녹화 시작 시 전체 메모를 녹화 폴더에 자동 저장
        signals.rec_started.connect(lambda _: QTimer.singleShot(500,
            lambda: self._export_txt_to_rec_dir()))
    ...
    def _build(self):
        ...
        exp_tb=QPushButton("📄 .txt 내보내기"); exp_tb.setFixedHeight(26)
        exp_tb.setStyleSheet("background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:4px;font-size:11px;")
        exp_tb.clicked.connect(self._export_txt)
        exp_rec_tb=QPushButton("📁 녹화폴더 저장"); exp_rec_tb.setFixedHeight(26)
        exp_rec_tb.setToolTip("현재 녹화 저장 폴더에 전체 메모를 .txt로 저장")
        exp_rec_tb.setStyleSheet("background:#1a2a1a;color:#8fa;border:1px solid #2a6a2a;border-radius:4px;font-size:11px;")
        exp_rec_tb.clicked.connect(lambda: self._export_txt_to_rec_dir())
        tc.addWidget(add_tb); tc.addWidget(del_tb); tc.addWidget(clr_tb)
        tc.addWidget(exp_tb); tc.addWidget(exp_rec_tb); tc.addStretch(); v.addLayout(tc)
    ...
    def _sync_texts(self):
        ...
        self.overlay_changed.emit()   # 텍스트 변경 시 미리보기에 즉시 반영
    ...
    def _export_txt(self):
        """
        현재 탭의 메모 내용을 .txt 파일로 내보내기.
        녹화 중이면 녹화 폴더에도 저장 여부 선택.
        """
        idx = self.tabs.currentIndex()
        if not 0 <= idx < len(self._editors): return
        ...
        try:
            with open(path, 'w', encoding='utf-8') as f:
                f.write(content)
            self.signals.status_message.emit(f"[메모 내보내기] → {path}")
            open_folder(save_dir)
        except Exception as ex:
            self.signals.status_message.emit(f"[메모 내보내기] 실패: {ex}")

    def _export_txt_to_rec_dir(self, tab_idx: int = None):
        """
        [추가1] 메모 내용을 현재 녹화 저장 디렉토리에 .txt 저장.
        tab_idx=None 이면 전체 탭을 하나의 파일로 합산 저장.
        녹화 폴더(engine.output_dir)가 없으면 base_dir/memo_export에 저장.
        """
        ...
        if tab_idx is None:
            ...
            if not 0 <= tab_idx < len(self._editors): return
            tab_name  = self.tabs.tabText(tab_idx)
            safe_name = re.sub(r'[\\/:*?"<>|]', '_', tab_name)
            content   = self._editors[tab_idx].toPlainText()
            path = os.path.join(save_dir, f"{safe_name}_{ts}.txt")
            try:
                with open(path, 'w', encoding='utf-8') as f:
                    f.write(content)
    ...
    def _add_ov_row(self, cfg):
        ...
        row.changed.connect(self.overlay_changed)   # ★ 변경 시 시그널 전파
    ...
    def _add_overlay(self):
        cfg=MemoOverlayCfg(0,"bottom-right","both",True)   # 새 오버레이는 ON
        self.engine.memo_overlays.append(cfg)
        self._add_ov_row(cfg)
        self.overlay_changed.emit()   # 즉시 반영
...
class _TcIdWidget(QWidget):
    """
    TC-ID 입력 위젯.
    텍스트 부분(prefix)과 숫자 부분(number)을 분리하여
    숫자는 마우스 휠로 쉽게 증감 가능.
    예: BLTN_CAM_TC_3-0001  →  prefix='BLTN_CAM_TC_3-', number=1
    """
    ...
    def __init__(self, parent=None):
        ...
        self._prefix_ed = QLineEdit()
        self._prefix_ed.setPlaceholderText("BLTN_CAM_TC_3-")
        self._prefix_ed.setToolTip("TC-ID의 문자열 부분 (예: BLTN_CAM_TC_3-)")
        self._prefix_ed.setStyleSheet(
            "QLineEdit{background:#1a1a3a;color:#7bc8e0;border:1px solid #2a4a6a;"
            "border-radius:3px;padding:3px 6px;font-size:12px;}")
        self._prefix_ed.textChanged.connect(self._emit)
        lay.addWidget(self._prefix_ed, 3)

        self._num_spin = QSpinBox()
        self._num_spin.setRange(0, 99999)
        self._num_spin.setValue(1)
        self._num_spin.setFixedWidth(72)
        self._num_spin.setToolTip("TC-ID의 숫자 부분 — 마우스 휠로 조절")
        self._num_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;"
            "border-radius:3px;padding:3px;font-size:13px;font-weight:bold;}")
        self._num_spin.valueChanged.connect(self._emit)
        lay.addWidget(self._num_spin, 1)

        self._digits_spin = QSpinBox()
        self._digits_spin.setRange(1, 6); self._digits_spin.setValue(4)
        self._digits_spin.setFixedWidth(44)
        self._digits_spin.setPrefix("0")
        self._digits_spin.setToolTip("숫자 자릿수 (예: 4 → 0001)")
        self._digits_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#888;border:1px solid #334;"
            "border-radius:3px;padding:2px;font-size:10px;}")
        self._digits_spin.valueChanged.connect(self._emit)
        lay.addWidget(self._digits_spin)

    def _emit(self):
        self.valueChanged.emit(self.value())

    def value(self) -> str:
        prefix = self._prefix_ed.text()
        num    = self._num_spin.value()
        digits = self._digits_spin.value()
        return f"{prefix}{num:0{digits}d}"

    def set_value(self, tc_id: str):
        """TC-ID 문자열을 prefix+숫자로 파싱해 설정."""
        import re
        m = re.match(r'^(.*?)(\d+)$', tc_id)
        if m:
            self._prefix_ed.blockSignals(True)
            self._num_spin.blockSignals(True)
            self._digits_spin.blockSignals(True)
            self._prefix_ed.setText(m.group(1))
            num_str = m.group(2)
            self._num_spin.setValue(int(num_str))
            self._digits_spin.setValue(max(1, len(num_str)))
            self._prefix_ed.blockSignals(False)
            self._num_spin.blockSignals(False)
            self._digits_spin.blockSignals(False)
        else:
            self._prefix_ed.setText(tc_id)
            self._num_spin.setValue(0)


class PathSettingsPanel(QWidget):
    """
    저장 경로 설정 v2.7
    ─ T/C 검증 목적 (기능별) — 상단 배치, 적용할 폴더 선택 UI 포함
    ─ 경로 항목 드래그앤드랍 순서 변경 (차종_버전, TC-ID 는 default 고정, 나머지 자유)
    ─ TC-ID 범위 설정 (시작~끝 번호)
    ─ 기능별 default/지정경로 선택
    """
    ...
    class _PathDragList(QWidget):
        ...
        def __init__(self, parent=None):
            ...
            self._items: list = []   # {'key':str, 'label':str, 'fixed':bool, 'widget':QWidget}
        ...
        def set_items(self, items: list):
            """items: [{'key':str,'label':str,'fixed':bool}, ...]"""
        ...
        def _make_row(self, item: dict) -> QWidget:
            ...
            row.setStyleSheet(
                f"QWidget{{background:{self._BG};border:1px solid #1a1a3a;"
                "border-radius:4px;}")
        ...
        def mouseMoveEvent(self, e):
            if self._drag_idx >= 0 and not self._dragging:
                if (e.pos()-self._drag_start).manhattanLength() > 6:
                    ...
                    if w:
                        w.setStyleSheet(
                            f"QWidget{{background:{self._BG_DG};"
    ...
    def __init__(self, engine: CoreEngine, db: SettingsDB, parent=None):
        ...
        self.engine = engine
        self.db     = db
        self._seg_rows: list = []   # {'key':str,'label':str,'ed':QLineEdit or None,'fixed':bool}
        self._build()

    def _build(self):
        v = QVBoxLayout(self)
        v.setContentsMargins(0, 0, 0, 6)
        v.setSpacing(0)   # 섹션 간격은 각 헤더버튼이 담당

        # ── 공통 헬퍼: 접기/펼치기 섹션 버튼 + 컨테이너 ──────────────────────
        def _make_collapsible(title: str, color: str):
            """(btn, body_widget, body_layout) 반환."""
            ...
            btn.setStyleSheet(
                ...
                "font-size:11px;font-weight:bold;text-align:left;padding:0 10px;margin-top:6px;}}"
                f"QPushButton:hover{{background:#22223a;}}"
                f"QPushButton:checked{{background:#0d0d1a;color:#556;}}")
            body = QWidget()
            body.setStyleSheet("QWidget{background:#0a0a1a;border:1px solid #1a1a3a;"
        ...
        tc_hint = QLabel(
            ...
            "→ '태그 적용 폴더'에 (PASS)/(FAIL) 태그 삽입\n"
            "⚠ TC-ID가 먼저 설정되어야 활성화 가능합니다.")
        tc_hint.setWordWrap(True)
        tc_hint.setStyleSheet("color:#888;font-size:9px;")
        ...
        for i, (attr_name, engine_attr, label, color) in enumerate(_TC_ITEMS):
            ...
            chk.setStyleSheet(
                ...
                "spacing:6px;padding:4px 8px;border:1px solid #2a2a3a;"
                "border-radius:5px;background:#0d0d1e;}}"
        ...
        folder_row.addWidget(QLabel("태그 적용 폴더:"))
        ...
        v.addWidget(tc_id_btn)
        v.addWidget(tc_id_body)
        ...
        mode_row.addWidget(QLabel("번호 형식:"))
        self.tc_mode_single = QCheckBox("개별  (예: -0041)")
        self.tc_mode_range  = QCheckBox("범위  (예: -0041~51)")
        self.tc_mode_single.setChecked(True)
        for chk in (self.tc_mode_single, self.tc_mode_range):
            chk.setStyleSheet(
                "QCheckBox{font-size:11px;color:#ccd;spacing:5px;padding:2px 6px;"
                "border:1px solid #2a2a3a;border-radius:4px;}"
                "QCheckBox:checked{background:#0d1525;border-color:#7bc8e0;}")
        self.tc_mode_single.toggled.connect(self._on_tc_mode_changed)
        self.tc_mode_range.toggled.connect(
            lambda c: self._on_tc_mode_changed(not c))
        mode_row.addWidget(self.tc_mode_single); mode_row.addWidget(self.tc_mode_range)
        mode_row.addStretch()
        tc_id_l.addLayout(mode_row)

        # 번호 스핀박스들 ─ 시작번호 행
        num_grid = QGridLayout(); num_grid.setSpacing(8)

        self._tc_start_lbl = QLabel("번호")
        num_grid.addWidget(self._tc_start_lbl, 0, 0)
        ...
        self.tc_start_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;"
            "border-radius:3px;padding:3px;font-size:13px;font-weight:bold;}")
        self.tc_start_spin.valueChanged.connect(self._upd_tc_id)
        ...
        self.tc_digits_spin.setFixedWidth(44)
        self.tc_digits_spin.setPrefix("0")
        ...
        # 끝 번호 행 — 범위 모드에서만 표시 (별도 QWidget으로 완전 show/hide)
        self._tc_end_row = QWidget()
        self._tc_end_row.setStyleSheet("background:transparent;")
        end_row_l = QHBoxLayout(self._tc_end_row)
        end_row_l.setContentsMargins(0, 0, 0, 0); end_row_l.setSpacing(8)
        self._tc_end_lbl = QLabel("끝 번호")
        self._tc_end_lbl.setStyleSheet("color:#aaa;font-size:11px;min-width:60px;")
        self.tc_end_spin = QSpinBox()
        self.tc_end_spin.setRange(0, 99999); self.tc_end_spin.setValue(51)
        self.tc_end_spin.setFixedWidth(88)
        self.tc_end_spin.setToolTip("범위 모드 끝 번호  (예: 0041~51 에서 51)")
        self.tc_end_spin.setStyleSheet(
            "QSpinBox{background:#1a1a3a;color:#2ecc71;border:1px solid #2a6a2a;"
            "border-radius:3px;padding:3px;font-size:13px;font-weight:bold;}")
        self.tc_end_spin.valueChanged.connect(self._upd_tc_id)
        end_hint = QLabel("  ← 범위 끝 (숫자만 입력)")
        end_hint.setStyleSheet("color:#556;font-size:9px;")
        end_row_l.addWidget(self._tc_end_lbl)
        end_row_l.addWidget(self.tc_end_spin)
        end_row_l.addWidget(end_hint)
        end_row_l.addStretch()
        self._tc_end_row.setVisible(False)   # 개별 모드 기본값 → 숨김
        ...
        self.tc_preview_lbl.setStyleSheet(
            ...
            "background:#0d0d1e;border:1px solid #2a2a1a;"
            "border-radius:3px;padding:4px 8px;")
        tc_id_l.addWidget(self.tc_preview_lbl)

        # ══ 3. 저장 경로 구성 ══════════════════════════════════════════════════
        path_btn, path_body, path_l = _make_collapsible(
            "📁 저장 경로 구성  (드래그로 순서 변경, 항목 삭제 가능)", "#7bc8e0")
        v.addWidget(path_btn)
        v.addWidget(path_body)

        path_hint = QLabel(
            "항목을 드래그해 순서를 변경하거나 ✕로 삭제하세요.\n"
            "기본 항목(차종_버전·날짜·TC-ID)도 삭제·이동 가능합니다.\n"
            "차종_버전:")
        path_hint.setStyleSheet("color:#556;font-size:9px;")
        ...
        # ══ 4. 기능별 저장 경로 ════════════════════════════════════════════════
        feat_btn, feat_body, feat_l = _make_collapsible(
            "🗂 기능별 저장 경로  (OFF=기본 / ON=지정)", "#7bc8e0")
        v.addWidget(feat_btn)
        v.addWidget(feat_body)

        _FEATS = [
            ("use_custom_path_rec",      "⏺  Recording",  "#27ae60", "rec_custom_chk"),
            ("use_custom_path_manual",   "🎬  수동녹화",   "#e67e22", "manual_custom_chk"),
            ("use_custom_path_blackout", "⚡  블랙아웃",   "#e74c3c", "blackout_custom_chk"),
            ("use_custom_path_capture",  "📸  캡처",       "#3498db", "capture_custom_chk"),
        ]
        feat_grid2 = QGridLayout(); feat_grid2.setSpacing(8)
        for i, (engine_attr, label, color, chk_attr) in enumerate(_FEATS):
            chk = QCheckBox(label)
            chk.setMinimumHeight(28)
            chk.setStyleSheet(
                f"QCheckBox{{font-size:11px;font-weight:bold;color:{color};"
                "spacing:6px;padding:4px 10px;border:1px solid #2a2a3a;"
                "border-radius:5px;background:#0d0d1e;}}"
                f"QCheckBox:checked{{background:#0d1525;border-color:{color};}}"
                "QCheckBox:hover{background:#14142a;}")
            chk.toggled.connect(lambda c, a=engine_attr: setattr(self.engine, a, c))
            chk.toggled.connect(lambda _: self._upd_preview())
            setattr(self, chk_attr, chk)
            feat_grid2.addWidget(chk, i//2, i%2)
        feat_l.addLayout(feat_grid2)

        # ══ 5. 경로 미리보기 ═══════════════════════════════════════════════════
        prev_btn, prev_body, prev_l = _make_collapsible("🔍 경로 미리보기", "#556")
        v.addWidget(prev_btn)
        v.addWidget(prev_body)

        self.preview_lbl = QLabel("")
        self.preview_lbl.setStyleSheet(
            "color:#7bc8e0;font-size:10px;font-family:monospace;"
            "background:#06060e;border:1px solid #1a2030;"
    ...
    def _make_tc_toggle(self, engine_attr: str):
        def _toggle(checked: bool):
            if not checked:
                setattr(self.engine, engine_attr, False)
                return
            if not self.engine.tc_id or not self.engine.tc_id.strip():
                QMessageBox.warning(
                    ...
                    QMessageBox.Ok)
                _attr_to_chk = {
                    'tc_rec_enabled':      'tc_rec_chk',
                    'tc_manual_enabled':   'tc_manual_chk',
                    'tc_blackout_enabled': 'tc_blackout_chk',
                    'tc_capture_enabled':  'tc_capture_chk',
                }
    ...
    def get_tc_target_folder(self) -> str:
        """
        TC 태그를 적용할 폴더 실제 경로 반환.
        idx 0 = TC-ID 폴더   : output_dir 역방향 탐색 → 없으면 output_dir 상위에 직접 구성
        idx 1 = 차종_버전 폴더: output_dir 역방향 탐색 → 없으면 base_dir/vehicle_type
        idx 2 = Rec_ 폴더    : output_dir 그 자체
        idx 3 = 직접 입력 경로
        반환된 경로는 존재하지 않으면 자동으로 생성합니다.
        """
        idx = self.tc_folder_combo.currentIndex()
        eng = self.engine

        def _find_parent_by_name(base_path: str, folder_name: str) -> str:
            """base_path 에서 folder_name 과 일치하는 폴더 경로를 역방향 탐색."""
        ...
        def _ensure(path: str) -> str:
            """경로가 존재하지 않으면 생성 후 반환. 빈 문자열이면 그대로 반환."""
            if path:
                try:
                    os.makedirs(path, exist_ok=True)
                except Exception:
        ...
        ts_date = datetime.now().strftime("%Y%m%d") if not od else                   os.path.basename(os.path.dirname(od)) if eng.use_custom_path_rec                   else os.path.basename(os.path.dirname(od))

        if idx == 0:
            # TC-ID 폴더 — output_dir 에서 역방향 탐색
            tc = eng.tc_id or ""
            if not tc:
                return od  # TC-ID 미설정이면 Rec_ 폴더에 태그

            found = _find_parent_by_name(od, tc)
            if found:
                return found  # 이미 존재 → 바로 반환

            # 경로에 TC-ID 폴더가 없는 경우 (use_custom_path_rec=False 등)
            # output_dir 상위에 tc_id 폴더를 구성해 반환
            # 구조: base_dir / [vehicle_type] / YYYYMMDD / tc_id
            if od:
                # output_dir 의 날짜 폴더(부모) 아래에 tc_id 폴더 생성
                date_dir = os.path.dirname(od)   # base_dir/YYYYMMDD
                tc_dir   = os.path.join(date_dir, tc)
                return _ensure(tc_dir)
            return ""
            ...
            # 없으면 base_dir/vehicle_type 직접 구성
            vt_dir = os.path.join(eng.base_dir, vt)
            return _ensure(vt_dir)
    ...
    def _sync_tc_tag_target(self):
        """콤보박스/직접입력 변경 시 engine.tc_tag_target_dir 를 즉시 갱신."""
    ...
    def _on_tc_mode_changed(self, single: bool):
        """개별번호 모드 ↔ 범위번호 모드 전환."""
        is_range = not single
        self.tc_mode_range.blockSignals(True)
        self.tc_mode_range.setChecked(is_range)
        self.tc_mode_range.blockSignals(False)
        # ★ 범위 모드일 때만 끝 번호 행 전체를 표시 (QWidget show/hide → 레이아웃 공간도 함께 조절)
        if hasattr(self, '_tc_end_row'):
            self._tc_end_row.setVisible(is_range)
        self._upd_tc_id()
    ...
    def _upd_tc_id(self):
        ...
        is_range = self.tc_mode_range.isChecked() if hasattr(self, 'tc_mode_range') else False
        if is_range:
    ...
    def _rebuild_drag_list(self):
        """드래그 리스트를 현재 _seg_rows 기준으로 재구성. 모든 항목 이동/삭제 가능."""
        items = []
        vt = self.engine.vehicle_type or '미입력'
        tc = self.engine.tc_id        or '미설정'
        # 기본 항목 — fixed=False 로 이동/삭제 가능 (예시 항목 개념)
    ...
    def _add_segment(self, value: str = ""):
        ...
        num_lbl = QLabel(f"항목 {idx+1}:")
        num_lbl.setStyleSheet("color:#aaa;font-size:10px;min-width:46px;")
        ...
        del_btn.setStyleSheet(
            "QPushButton{background:#3a1a1a;color:#f88;border:none;border-radius:3px;}"
    ...
    def _upd_preview(self):
        ...
        _FEATS = [
            ("use_custom_path_rec",      "⏺ Recording",   "rec_custom_chk"),
            ("use_custom_path_manual",   "🎬 수동녹화",    "manual_custom_chk"),
            ("use_custom_path_blackout", "⚡ 블랙아웃",    "blackout_custom_chk"),
            ("use_custom_path_capture",  "📸 캡처",        "capture_custom_chk"),
        ]
        lines = []
        for eng_attr, label, chk_attr in _FEATS:
            chk = getattr(self, chk_attr, None)
            use = chk.isChecked() if chk else False
            if use:
                parts = ["bltn_rec"]
    ...
    def _save(self):
        ...
        self.db.set_path_settings(
            ...
            extra_segments      = segs,
            use_custom_rec      = self.rec_custom_chk.isChecked(),
            use_custom_manual   = self.manual_custom_chk.isChecked(),
            use_custom_blackout = self.blackout_custom_chk.isChecked(),
            use_custom_capture  = self.capture_custom_chk.isChecked(),
        ...
        self.engine.use_custom_path_rec      = self.rec_custom_chk.isChecked()
        self.engine.use_custom_path_manual   = self.manual_custom_chk.isChecked()
        self.engine.use_custom_path_blackout = self.blackout_custom_chk.isChecked()
        self.engine.use_custom_path_capture  = self.capture_custom_chk.isChecked()
        self._sync_tc_tag_target()  # 저장 후 engine 즉시 동기화
    ...
    def load_from_db(self):
        ...
        # TC-ID 파싱 — 저장된 값에서 prefix + 숫자 추출
        tc_raw = ps.get('tc_id', '')
        # 범위 형식(예: prefix0041~51) 먼저 확인
        ...
        # 기능별 경로 체크박스
        self.rec_custom_chk.setChecked(     ps.get('use_custom_rec',      False))
        self.manual_custom_chk.setChecked(  ps.get('use_custom_manual',   False))
        self.blackout_custom_chk.setChecked(ps.get('use_custom_blackout', False))
        self.capture_custom_chk.setChecked( ps.get('use_custom_capture',  False))
        self.engine.use_custom_path_rec      = ps.get('use_custom_rec',      False)
        self.engine.use_custom_path_manual   = ps.get('use_custom_manual',   False)
        self.engine.use_custom_path_blackout = ps.get('use_custom_blackout', False)
        self.engine.use_custom_path_capture  = ps.get('use_custom_capture',  False)
        # TC 체크박스
        ...
        self._upd_preview()
        self._sync_tc_tag_target()  # DB 복원 후 engine 동기화
...
class LogPanel(QWidget):
    def __init__(self, signals: Signals, engine: CoreEngine = None, parent=None):
        super().__init__(parent)
        self._engine = engine
    ...
    def _build(self):
        ...
        cl=QPushButton("🗑 Clear"); cl.setFixedHeight(24)
        cl.clicked.connect(self.txt.clear)
        exp=QPushButton("📄 .txt 내보내기"); exp.setFixedHeight(24)
        exp.setStyleSheet("QPushButton{background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;"
                          "border-radius:3px;font-size:10px;}"
                          "QPushButton:hover{background:#1e3548;}")
        exp.clicked.connect(self._export_log)
        btn_row.addWidget(cl); btn_row.addWidget(exp); btn_row.addStretch()
        v.addWidget(self.txt); v.addLayout(btn_row)
    ...
    def _export_log(self):
        """로그 전체를 .txt로 최상단 디렉토리(base_dir)에 내보내기."""
        base = (self._engine.base_dir
                if self._engine else os.path.join(os.path.expanduser("~/Desktop"), "bltn_rec"))
        os.makedirs(base, exist_ok=True)
        ts   = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = os.path.join(base, f"log_{ts}.txt")
        content = self.txt.toPlainText()
        try:
            with open(path, 'w', encoding='utf-8') as f:
                f.write(content)
            self.append(f"[Log 내보내기] → {path}")
            open_folder(base)
        except Exception as ex:
            self.append(f"[Log 내보내기] 실패: {ex}")
...
class CameraWindow(QDialog):
    ...
    def _build(self):
        ...
        # ★ scan_cameras는 engine.start()에서 이미 호출되므로 중복 제거
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v2.5")
        ...
        self._cam_win  = CameraWindow(self.engine, self.signals, None)   # ★ parent=None → 독립창
        self._disp_win = DisplayWindow(self.engine, self.signals, None)  # ★ 동일
    ...
    def _build_sections(self):
        ...
        self._panels["log"]       = LogPanel(self.signals, self.engine)
        ...
        # ★ 메모 오버레이 ON/내용 변경 시 미리보기 스레드가 꺼져 있으면 자동 켜기
        self._panels["memo"].overlay_changed.connect(self._on_overlay_changed)

        # ★ T/C 태그 적용 폴더: 녹화 시작(output_dir 확정) 후 경로 동기화
        #   rec_started  → output_dir 가 확정된 직후 → 콤보 선택 반영
        #   rec_stopped  → 혹시 이후에 콤보가 바뀌었을 때를 대비
        self.signals.rec_started.connect(
            lambda _: QTimer.singleShot(0, self._panels["path"]._sync_tc_tag_target))
        self.signals.rec_stopped.connect(
            lambda: QTimer.singleShot(0, self._panels["path"]._sync_tc_tag_target))
    ...
    def _on_preview_ready(self):
        """10초 경과 후 미리보기 자동 활성화 — 스레드도 함께 시작."""
        ...
        # 스레드가 아직 안 켜진 경우 시작 (delayed_start 와 중복 방지)
    ...
    def _on_feat_toggle(self, key: str, visible: bool):
        """CollapsibleSection 헤더의 토글이 아닌 외부 호출 전용."""
        sec=self._sections.get(key)
        if sec: sec.setVisible(visible)

    def _on_overlay_changed(self):
        """
        메모 오버레이 ON/OFF 또는 텍스트 변경 시 호출.
        미리보기 스레드가 꺼져 있으면 즉시 켜서 오버레이가 바로 보이도록 함.
        """
        scr_alive = self.engine._scr_thread and self.engine._scr_thread.is_alive()
        cam_alive = self.engine._cam_thread and self.engine._cam_thread.is_alive()
        if not scr_alive:
            # 대기 타이머가 있으면 취소
            ...
            self._scr_lbl.set_active(True)
            self._preview_countdown_lbl.setText("미리보기: 활성화됨")
            self._preview_countdown_lbl.setStyleSheet(
    ...
    def _save_settings(self):
        ...
        db.set("camera_rec",      rp.cam_chk.isChecked())
        db.set("codec_idx",       rp.codec_cb.currentIndex())
        db.set("scale_idx",       rp.scale_cb.currentIndex())
        # T/C 검증 플래그는 PathSettingsPanel._save()에서 path_settings 테이블에 저장
        # (여기서는 settings 테이블 별도 저장 불필요 — load_from_db로 복원)
        mp2=p["manual"]
    ...
    def _load_settings(self):
        ...
        # T/C 검증 플래그는 p["path"].load_from_db() 에서 path_settings 테이블로 복원
        mp2=p["manual"]
    ...
    def _on_reset(self, do_settings, do_memo, do_sections, do_db):
        ...
        if do_settings:
            ...
            # TC 플래그 초기화 — PathSettingsPanel 체크박스 기준
            pp = self._panels["path"]
            for chk_attr in ('tc_rec_chk','tc_manual_chk','tc_blackout_chk','tc_capture_chk'):
                chk = getattr(pp, chk_attr, None)
    ...
    def _setup_hotkeys(self):
        ...
        def _macro_stop():
            QTimer.singleShot(0, self._panels["macro"]._on_stop)
        ...
        hk={
            ...
            '<ctrl>+<alt>+x': _macro_stop,          # ★ Click Macro 중단
    ...
    def closeEvent(self, e):
        # tc_rec_enabled 활성화 상태로 녹화 중이면 종료 전 PASS/FAIL 확인
        if self.engine.recording and self.engine.tc_rec_enabled:
            result = self._panels["recording"]._show_tc_dialog(
                "녹화를 종료합니다.\nT/C 검증 결과를 선택하세요.",
                cancel_label="취소  (종료 취소)")
            if result is None:
...
# =============================================================================
#  TutorialDialog  — 프로그램 시작 시 기능 안내 튜토리얼
# =============================================================================
class TutorialDialog(QDialog):
    """
    프로그램 최초 실행(또는 스킵 기간 만료) 시 표시되는 튜토리얼 다이얼로그.
    ─ 슬라이드 방식 (이전/다음 버튼)
    ─ 하단: Skip(이번만 건너뜀) / 1주일 뒤에 보기 / 다시 보지 않기
    ─ skip_duration_days: 0=즉시 / 7=1주일 / 36500=영구
    """
    ...
    _SLIDES = [
        {
            "icon":  "🎬",
            "title": "Screen & Camera Recorder 에 오신 것을 환영합니다!",
            "color": "#2ecc71",
            "body":  (
                "이 프로그램은 <b>화면 녹화</b>와 <b>카메라 녹화</b>를 동시에 수행하며,\n"
                "자동차 시험·검증(T/C), 블랙아웃 감지, 수동 클립 저장 등\n"
                "<b>차량 데이터 수집 환경에 최적화</b>된 올인원 레코더입니다.\n\n"
                "튜토리얼을 통해 핵심 기능을 빠르게 익혀보세요."
            ),
            "tips": [],
        },
        {
            "icon":  "⏺",
            "title": "Recording — 화면·카메라 녹화",
            "color": "#27ae60",
            "body":  (
                "Display(화면)와 Camera를 독립적으로 ON/OFF하여 녹화할 수 있습니다.\n"
                "녹화 영상은 <b>30분 세그먼트</b>로 자동 분할 저장되어\n"
                "장시간 녹화 시에도 파일이 손상되지 않습니다."
            ),
            "tips": [
                "⌨  Ctrl+Alt+W : 녹화 시작",
                "⌨  Ctrl+Alt+E : 녹화 종료",
                "⌨  Ctrl+Alt+D : Display 저장 ON/OFF 토글",
                "🖱  좌측 미리보기 화면에 드래그 → ROI(관심 영역) 지정",
                "🖱  미리보기 우클릭 → ROI 제거",
            ],
        },
        {
            "icon":  "🎬",
            "title": "수동 녹화 — 원하는 순간의 전·후 클립 저장",
            "color": "#e67e22",
            "body":  (
                "버튼을 누른 순간을 기준으로 <b>최대 ±40초</b>의 버퍼에서\n"
                "전(Pre)·후(Post) 구간을 자동으로 추출해 MP4로 저장합니다.\n"
                "블랙아웃 전후 상황, 이상 현상 포착에 유용합니다."
            ),
            "tips": [
                "⌨  Ctrl+Alt+M : 지금 클립 저장",
                "🕐  전/후 시간은 슬라이더로 0 ~ 40초 범위에서 조절",
                "📷  저장 소스를 Display / Camera / Both 중 선택 가능",
                "📁  저장 경로: bltn_rec/manual_clip/",
            ],
        },
        {
            "icon":  "⚡",
            "title": "Blackout Detection — 블랙아웃 자동 감지",
            "color": "#e74c3c",
            "body":  (
                "ROI 영역의 <b>밝기가 급격히 감소</b>하면 블랙아웃으로 판정하고\n"
                "감지 전·후 각 20초 영상을 자동 저장합니다.\n"
                "감지 영상에는 타임스탬프와 감지 횟수 오버레이가 삽입됩니다."
            ),
            "tips": [
                "🎚  밝기 변화 임계값(기본 30) — 낮을수록 민감",
                "⏱  쿨다운(기본 5초) — 연속 감지 방지",
                "📁  저장 경로: bltn_rec/blackout/",
                "🔲  ROI 미지정 시 전체 화면 기준으로 감지",
            ],
        },
        {
            "icon":  "📝",
            "title": "메모장 — 영상 위에 메모 오버레이",
            "color": "#f39c12",
            "body":  (
                "탭별로 메모를 작성하고, 영상 위에 <b>실시간 오버레이</b>로 표시할 수 있습니다.\n"
                "<b>더블클릭</b>으로 현재 시각 타임스탬프를 삽입하고,\n"
                "녹화 시작 시 전체 메모가 녹화 폴더에 .txt로 자동 저장됩니다."
            ),
            "tips": [
                "🖱  더블클릭 → 현재 시각 타임스탬프 삽입",
                "🖱  우클릭 → 타임스탬프 제거",
                "📄  📄 .txt 내보내기 버튼 → memo_export 폴더에 저장",
                "📁  📁 녹화폴더 저장 버튼 → 현재 녹화 폴더에 저장",
                "🎨  오버레이 위치·대상·글자 크기를 탭별 독립 설정 가능",
            ],
        },
        {
            "icon":  "🖱",
            "title": "Auto-Click — 자동 클릭",
            "color": "#2980b9",
            "body":  (
                "설정한 간격(초)으로 마우스 좌클릭을 자동 반복합니다.\n"
                "<b>+10초 / +1분 / +10분</b> 빠른 증감 버튼으로 간격을 손쉽게 조절할 수 있습니다."
            ),
            "tips": [
                "⌨  Ctrl+Alt+A : 시작",
                "⌨  Ctrl+Alt+S : 정지",
                "🔢  클릭 카운터 실시간 표시 및 초기화 가능",
            ],
        },
        {
            "icon":  "🎯",
            "title": "Click Macro — 마우스·키보드 동작 기록 및 재생",
            "color": "#16a085",
            "body":  (
                "클릭, 드래그, 키보드 입력을 기록해 <b>자동 반복 재생</b>할 수 있습니다.\n"
                "여러 슬롯에 서로 다른 매크로를 저장하고,\n"
                "딜레이·좌표를 테이블에서 직접 편집할 수 있습니다."
            ),
            "tips": [
                "⌨  Ctrl+Alt+X : 매크로 실행 중단",
                "📋  기록 대상을 클릭 / 드래그 / 키보드 중 선택 가능",
                "🔁  반복 횟수 0 = 무한 반복",
                "💾  슬롯별로 저장·복원 (DB 자동 저장)",
            ],
        },
        {
            "icon":  "⏰",
            "title": "Schedule — 예약 녹화",
            "color": "#8e44ad",
            "body":  (
                "시작/종료 시각을 지정해 <b>자동 예약 녹화</b>를 설정할 수 있습니다.\n"
                "매크로도 함께 예약 실행할 수 있어\n"
                "야간 자동 테스트 환경에서 활용하기 좋습니다."
            ),
            "tips": [
                "📅  날짜·시각 클릭 → 달력 팝업에서 선택",
                "🕒  '지금' 버튼 → 현재 시각 즉시 입력",
                "🔔  카운트다운 표시로 다음 예약까지 남은 시간 확인",
            ],
        },
        {
            "icon":  "📂",
            "title": "저장 경로 — 경로 구성 및 T/C 검증",
            "color": "#00b894",
            "body":  (
                "차종_버전 / YYYYMMDD / TC-ID 등 경로 항목을\n"
                "<b>드래그앤드랍</b>으로 순서를 자유롭게 구성할 수 있습니다.\n"
                "T/C 검증 목적 활성화 시 처리 완료 후 폴더명에 <b>(PASS)/(FAIL)</b> 태그를 삽입합니다."
            ),
            "tips": [
                "🔢  TC-ID: 개별 번호(0041) 또는 범위(0041~51) 선택 가능",
                "🗂  기능별(Recording/수동녹화/블랙아웃/캡처) 저장 경로 독립 설정",
                "💾  '경로 설정 저장' 버튼 — DB에 영구 저장",
            ],
        },
        {
            "icon":  "🚀",
            ...
            "color": "#7bc8e0",
            "body":  (
                "주요 단축키를 외워두면 훨씬 빠르게 사용할 수 있습니다.\n"
                "언제든지 도움말을 다시 보려면\n"
                "설정 DB(<b>settings.db</b>)에서 <code>tutorial_skip_until</code> 값을 삭제하세요."
            ),
            "tips": [
                "⌨  Ctrl+Alt+W : 녹화 시작",
                "⌨  Ctrl+Alt+E : 녹화 종료",
                "⌨  Ctrl+Alt+M : 수동 클립 저장",
                "⌨  Ctrl+Alt+A/S : 오토클릭 시작/정지",
                "⌨  Ctrl+Alt+X : 매크로 중단",
                "⌨  Ctrl+Alt+Q : 프로그램 종료",
            ],
        },
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.skip_duration_days = 0   # 0=이번만, 7=1주일, 36500=영구
        self._current = 0
        self._total   = len(self._SLIDES)
        self._build()
        self._go(0)

    # ── UI 구성 ───────────────────────────────────────────────────────────────
    def _build(self):
        self.setWindowTitle("Screen & Camera Recorder — 시작 가이드")
        ...
        self.setStyleSheet("""
            ...
            QLabel  { color: #dde; }
            QPushButton {
                background: #1e2a3a; border: 1px solid #4a5a7a;
                border-radius: 5px; padding: 6px 18px; color: #ccd; font-size: 11px;
            }
        ...
        self._prog_bar.setStyleSheet("background:#1a1a3a;")
        self._prog_inner = QWidget(self._prog_bar)
        self._prog_inner.setGeometry(0, 0, 0, 4)
        self._prog_inner.setStyleSheet("background:#2ecc71;border-radius:2px;")
        root.addWidget(self._prog_bar)

        # ── 슬라이드 영역 ──────────────────────────────────────────────────────
        self._slide_area = QWidget()
        self._slide_area.setStyleSheet("background:transparent;")
        slide_v = QVBoxLayout(self._slide_area)
        slide_v.setContentsMargins(48, 36, 48, 16)
        slide_v.setSpacing(14)

        # 아이콘 + 제목
        title_row = QHBoxLayout(); title_row.setSpacing(14)
        self._icon_lbl = QLabel("🎬")
        self._icon_lbl.setStyleSheet("font-size:44px;")
        self._icon_lbl.setFixedWidth(56)
        self._icon_lbl.setAlignment(Qt.AlignTop | Qt.AlignHCenter)
        self._title_lbl = QLabel("")
        self._title_lbl.setWordWrap(True)
        self._title_lbl.setStyleSheet(
            "font-size:18px;font-weight:bold;color:#f0c040;line-height:140%;")
        title_row.addWidget(self._icon_lbl)
        title_row.addWidget(self._title_lbl, 1)
        slide_v.addLayout(title_row)

        # 구분선
        sep = QFrame(); sep.setFrameShape(QFrame.HLine)
        sep.setStyleSheet("border:none;border-top:1px solid #2a2a4a;margin:4px 0;")
        slide_v.addWidget(sep)

        # 본문
        self._body_lbl = QLabel("")
        self._body_lbl.setWordWrap(True)
        self._body_lbl.setTextFormat(Qt.RichText)
        self._body_lbl.setStyleSheet(
            "font-size:13px;color:#ccd;line-height:160%;")
        slide_v.addWidget(self._body_lbl)

        # 팁 목록
        self._tips_widget = QWidget()
        self._tips_widget.setStyleSheet(
            "background:#0a0a18;border:1px solid #1a2a3a;"
            "border-radius:6px;")
        self._tips_lay = QVBoxLayout(self._tips_widget)
        self._tips_lay.setContentsMargins(16, 10, 16, 10)
        self._tips_lay.setSpacing(5)
        slide_v.addWidget(self._tips_widget)
        slide_v.addStretch()
        root.addWidget(self._slide_area, 1)

        # ── 하단 바 ────────────────────────────────────────────────────────────
        bottom = QWidget()
        bottom.setStyleSheet("background:#08081a;border-top:1px solid #1a1a3a;")
        ...
        self._skip_btn.setStyleSheet(
            "QPushButton{background:transparent;color:#556;border:none;font-size:11px;}"
            "QPushButton:hover{color:#7bc8e0;}")
        self._skip_btn.clicked.connect(self._on_skip)

        self._week_btn = QPushButton("📅  일주일 뒤에 보기")
        self._week_btn.setFixedHeight(34)
        self._week_btn.setStyleSheet(
            "QPushButton{background:#1a2a1a;color:#8fa;border:1px solid #2a5a2a;"
            "border-radius:5px;font-size:11px;}"
            "QPushButton:hover{background:#1e3a1e;}")
        self._week_btn.clicked.connect(self._on_week)

        self._never_btn = QPushButton("🚫  다시 보지 않기")
        self._never_btn.setFixedHeight(34)
        self._never_btn.setStyleSheet(
        ...
        bl.addWidget(self._week_btn)
        bl.addWidget(self._never_btn)
        bl.addStretch()

        # 페이지 표시 (가운데)
        self._page_lbl = QLabel(f"1 / {self._total}")
        self._page_lbl.setStyleSheet("color:#556;font-size:11px;min-width:60px;")
        self._page_lbl.setAlignment(Qt.AlignCenter)
        bl.addWidget(self._page_lbl)
        bl.addStretch()

        # 이전/다음 버튼 (오른쪽)
        self._prev_btn = QPushButton("◀  이전")
        self._prev_btn.setFixedSize(90, 34)
        self._prev_btn.clicked.connect(lambda: self._go(self._current - 1))

        self._next_btn = QPushButton("다음  ▶")
        self._next_btn.setFixedSize(90, 34)
    ...
    def _go(self, idx: int):
        ...
        # 진행 바 업데이트
        ratio = (idx + 1) / self._total
        self._prog_inner.setFixedWidth(int(700 * ratio))
        self._prog_inner.setStyleSheet(
            f"background:{slide['color']};border-radius:2px;")

        # 아이콘·제목
        self._icon_lbl.setText(slide["icon"])
        self._title_lbl.setText(slide["title"])
        self._title_lbl.setStyleSheet(
            f"font-size:18px;font-weight:bold;color:{slide['color']};line-height:140%;")
        ...
        # 팁 목록 재빌드
        while self._tips_lay.count():
            item = self._tips_lay.takeAt(0)
            if item.widget():
                item.widget().setParent(None)

        if slide["tips"]:
            self._tips_widget.setVisible(True)
            for tip in slide["tips"]:
                lbl = QLabel(tip)
                lbl.setStyleSheet(
                    "color:#9ab;font-size:11px;background:transparent;"
                    "border:none;padding:1px 0;")
                self._tips_lay.addWidget(lbl)
        else:
            self._tips_widget.setVisible(False)

        # 버튼 상태
        self._prev_btn.setEnabled(idx > 0)
        is_last = (idx == self._total - 1)
        self._next_btn.setText("✅  시작하기!" if is_last else "다음  ▶")
        if is_last:
            self._next_btn.setStyleSheet(
                "QPushButton{background:#1a6a3a;color:#afffcf;border:1px solid #2a9a5a;"
                "border-radius:5px;font-size:12px;font-weight:bold;}"
        ...
        # 페이지 표시
        self._page_lbl.setText(f"{idx + 1} / {self._total}")

    def _on_next(self):
        if self._current >= self._total - 1:
            self.accept()   # 마지막 슬라이드 → 시작하기
        else:
            self._go(self._current + 1)

    # ── 하단 버튼 콜백 ───────────────────────────────────────────────────────
    def _on_skip(self):
        """이번만 건너뛰기 — skip_duration_days=0 → DB 저장 안함."""
        self.skip_duration_days = 0
        self.accept()

    def _on_week(self):
        """7일 뒤에 다시 표시."""
        self.skip_duration_days = 7
        self.accept()

    def _on_never(self):
        """영구 스킵 (100년)."""
        self.skip_duration_days = 36500
        self.accept()
...
def main():
    ...
    try:
        _db_tmp = SettingsDB()
        _skip_until = _db_tmp.get("tutorial_skip_until", "")
        if _skip_until:
            _due = datetime.fromisoformat(_skip_until)
    ...
    if _show_tutorial:
        tut = TutorialDialog()
        tut.exec_()
        if tut.skip_duration_days > 0:
            try:
                from datetime import timedelta as _td
                _db_tmp = SettingsDB()
                _due = (datetime.now() + _td(days=tut.skip_duration_days)).isoformat()
                _db_tmp.set("tutorial_skip_until", _due)
            except Exception:
                pass
