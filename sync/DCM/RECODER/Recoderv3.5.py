...
class KernelEngine:
    ...
    def _ocr_now(self, roi_idx: int, source: str = "screen",
        ...
        target_h = min(600, int(200 * max(1.0, zoom_hint)))

        def _upscale(img, h=None):
            t = h if h is not None else target_h
            ih, iw = img.shape[:2]
            if ih < t:
                s = t / ih
...
class MainWindow(QMainWindow):
    ...
    def _ocr_loop(self):
        ...
        def _run_ocr_on_roi(roi, frame, zoom_hint: float = 1.0):
            ...
            · tessedit_char_whitelist=0123456789 숫자 전용
            · 신뢰도 기반 최적 결과 선택
            ...
            # ── zoom_hint 반영: 확대 상태면 OCR 전처리도 더 크게 ────
            # 기본 목표 높이 200px, 줌 2배 이상이면 그만큼 더 키움
            base_h    = 200
            zoom_mult = max(1.0, zoom_hint)
            # 줌이 클수록 더 큰 이미지로 업스케일 (최대 600px)
            target_h  = min(600, int(base_h * zoom_mult))

            # ── 전처리 헬퍼 ──────────────────────────────────────────
            def _upscale(img, th=None):
                """지정 높이 이상으로 확대."""
                t = th if th is not None else target_h
                h, w = img.shape[:2]
                if h < t:
                    s = t / h
