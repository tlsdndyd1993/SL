...
class CoreEngine:
    ...
    def scan_cameras(self):
        """
        연결된 카메라를 스캔.
        FPS 측정: OS 보고값이 신뢰할 수 없으면 실측(30프레임 타이밍)으로 보정.
        ...
        for idx in range(8):
            ...
            if reported_fps and 5.0 < reported_fps < 300.0:
                ...
                fps = frames / elapsed if (elapsed > 0 and frames > 0) else 15.0
            ...
            found.append({
                "idx":  idx,
                "name": f"Camera {idx} [{nm}] {fps:.1f}fps",
                "fps":  fps,
            })

        self.camera_list = found
        if found:
            # 현재 active_cam_idx가 여전히 유효한지 확인
...
class RecordingPanel(QWidget):
    ...
    def _on_stop(self):
        """
        T/C 검증이 활성화된 경우 녹화 종료 전에 PASS/FAIL 확인 창을 표시.
        창을 닫으면 engine.tc_verify_result 가 설정되고 stop_recording 이 호출됨.
        """
        if self.engine.tc_verify_enabled:
            self._show_tc_dialog()
        else:
    ...
    def _show_tc_dialog(self):
        ...
        dlg.setStyleSheet(
            ...
            "QPushButton{font-size:15px;font-weight:bold;padding:10px 30px;"
            "border-radius:6px;border:none;}")
        lay=QVBoxLayout(dlg)
        lbl=QLabel("녹화를 종료하기 전에\nT/C 검증 결과를 선택하세요.")
        lbl.setAlignment(Qt.AlignCenter)
        lbl.setStyleSheet("font-size:13px;color:#dde;padding:10px;")
        lay.addWidget(lbl)
        btn_row=QHBoxLayout(); btn_row.setSpacing(16)
        btn_pass=QPushButton("✅  PASS")
        btn_pass.setStyleSheet(
            "QPushButton{background:#1a6a3a;color:#afffcf;}"
            "QPushButton:hover{background:#27ae60;color:white;}")
        btn_fail=QPushButton("❌  FAIL")
        btn_fail.setStyleSheet(
            "QPushButton{background:#6a1a1a;color:#ffaaaa;}"
            "QPushButton:hover{background:#c0392b;color:white;}")
        btn_cancel=QPushButton("취소")
        btn_cancel.setStyleSheet(
            "QPushButton{background:#2a2a3a;color:#888;border:1px solid #4a4a6a;"
            "font-size:11px;padding:6px 14px;border-radius:4px;}")
        def _select(result):
            self.engine.tc_verify_result = result
            dlg.accept()
            self.engine.stop_recording()
        btn_pass.clicked.connect(lambda: _select("PASS"))
        btn_fail.clicked.connect(lambda: _select("FAIL"))
        btn_cancel.clicked.connect(dlg.reject)
        btn_row.addWidget(btn_pass); btn_row.addWidget(btn_fail); btn_row.addWidget(btn_cancel)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v2.2")
    ...
    def _build_feature_bar(self) -> QWidget:
        ...
        기능 표시/숨기기 패널.
        - 독립 스크롤 영역
        - 각 항목: 체크박스(ON/OFF) + 드래그 핸들(▲▼ 버튼으로 순서 변경)
        - 더블클릭 시 해당 섹션으로 스크롤
        """
        bar=QFrame()
        ...
        self._feat_list_w=QWidget(); self._feat_list_w.setStyleSheet("background:#0c0c1e;")
        self._feat_list_l=QVBoxLayout(self._feat_list_w)
        self._feat_list_l.setContentsMargins(4,4,4,4); self._feat_list_l.setSpacing(2)

        self._feat_cbs={}       # key → QCheckBox
        self._feat_order=[]     # key 순서 리스트 (현재 표시 순서)
        ...
        for key in [d[0] for d in self._SECTION_DEFS]:
            self._feat_order.append(key)
            row=self._make_feat_row(key, labels.get(key, key))
            self._feat_list_l.addWidget(row)
        self._feat_list_l.addStretch()
    ...
    def _make_feat_row(self, key: str, label: str) -> QWidget:
        """기능 한 항목 행 위젯."""
        row=QFrame(); row.setFixedHeight(30)
        row.setObjectName(f"feat_row_{key}")
        row.setStyleSheet(
            "QFrame{background:#10102a;border-radius:3px;}"
            "QFrame:hover{background:#1a1a3a;}")
        hl=QHBoxLayout(row); hl.setContentsMargins(4,2,4,2); hl.setSpacing(5)

        cb=QCheckBox(label); cb.setChecked(True)
        cb.setStyleSheet("QCheckBox{color:#ccd;font-size:10px;spacing:4px;background:transparent;}")
        cb.toggled.connect(lambda v,k=key: self._on_feat_toggle(k,v))
        self._feat_cbs[key]=cb

        # 더블클릭 → 해당 섹션으로 스크롤
        cb.mouseDoubleClickEvent = lambda e, k=key: self._on_scroll_to_section(k)

        hl.addWidget(cb, 1)

        # 순서 변경 버튼
        for sym,fn in [("▲","_feat_up"),("▼","_feat_dn")]:
            b=QPushButton(sym); b.setFixedSize(20,20)
            b.setStyleSheet(
                "QPushButton{background:#1a2a3a;color:#7ab4d4;border:none;"
                "border-radius:2px;font-size:9px;}"
                "QPushButton:hover{background:#2a3e56;}")
            b.clicked.connect(lambda _,k=key,f=fn: getattr(self,f)(k))
            hl.addWidget(b)
        return row

    def _feat_move(self, key: str, direction: int):
        """direction: -1=위, +1=아래."""
        idx=self._feat_order.index(key)
        new_idx=idx+direction
        if not (0<=new_idx<len(self._feat_order)): return
        # 순서 교환
        self._feat_order[idx], self._feat_order[new_idx] = \
            self._feat_order[new_idx], self._feat_order[idx]
        # 레이아웃 재구성
        while self._feat_list_l.count():
            it=self._feat_list_l.takeAt(0)
        ...
        for k in self._feat_order:
            row=self._make_feat_row(k, labels.get(k,k))
            self._feat_list_l.addWidget(row)
            # 체크 상태 복원
            self._feat_cbs[k].setChecked(
                self._sections[k].isVisible() if k in self._sections else True)
        self._feat_list_l.addStretch()
        # 패널 순서도 갱신
        self._reorder_sections()

    def _feat_up(self, key: str): self._feat_move(key, -1)
    def _feat_dn(self, key: str): self._feat_move(key, +1)
    ...
    def _reorder_sections(self):
        ...
        for k in self._feat_order:
            ...
            if sec:
                ...
                sec.setVisible(self._feat_cbs[k].isChecked())
        self._panel_l.addStretch()
    ...
    def _pump_preview(self):
        ...
        try:
            ...
            # _scr_toggle 이 '일시정지' 상태가 아닐 때만 렌더
    ...
    def _save_settings(self):
        ...
        db.set("feat_order", _j.dumps(
            getattr(self, '_feat_order', [d[0] for d in self._SECTION_DEFS])))
        # 섹션 접힘 상태
    ...
    def closeEvent(self, e):
        ...
        if self.engine.recording and self.engine.tc_verify_enabled:
            self._panels["recording"]._show_tc_dialog()
            # 다이얼로그가 종료를 취소하면 창 닫기 무시
            if self.engine.recording:
                e.ignore(); return
        # 미리보기 타이머 정리
