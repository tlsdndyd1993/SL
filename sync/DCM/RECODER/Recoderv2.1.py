# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v2.1
────────────────────────────────────────────────────────────────────────────
v2.0 → v2.1  버그수정 / 안정화
  [B1] MemoPanel ＋탭 버튼 → getattr(self,'_add_tab') 연결 오류 (TypeError) 수정
       각 버튼에 전용 메서드(_add_tab_new/_del_tab/_clear_cur) 직접 연결
  [B2] MemoPanel monkey-patch 블록(_patched_build_memo) 완전 제거
       클래스 외부에서 _build를 덮어쓰던 취약한 패턴 제거
  [B3] MemoPanel 중복 메서드(_add_tab_new/add_tab_public/_memo_add_tab_outer) 정리
       _add_tab_new 하나로 통일
  [B4] CoreEngine.restart_screen에서 QTimer.singleShot 사용
       CoreEngine은 QObject가 아님 → threading.Thread + time.sleep 으로 교체
  [B5] 매크로 필터 race condition 수정
       UI _on_step에서 pop하던 방식 → engine emit loop 내부에서 필터 적용으로 변경
  [B6] 미사용 함수 제거: _put_text_pil, _style_btn, _sep_h
  [B7] 미사용 import 제거: QAction, QMenuBar, QFont, QSize
────────────────────────────────────────────────────────────────────────────
v1 → v2 변경사항:
  [신규] 매크로 이벤트 확장 (MacroStep: click / drag / key)
  [유지] 저장 경로 UI (차종/TC-ID) / 언어 혼용(한/영)
────────────────────────────────────────────────────────────────────────────
구조:
