# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v11.0
─────────────────────────────────────────────────────────────────────────────
v10 → v11 버그수정:
  1. [수정] Auto-Click ON 상태에서 매크로 기록 시작 시 프로그램 뻗는 버그
           → 매크로 기록 시작 시 auto-click 자동 일시중단, 기록 종료 후 복원
  2. [수정] 메모 글꼴 크기 컨트롤이 컨트롤 패널 에디터에만 적용되던 문제
           → 에디터 크기 / 영상 오버레이 크기 컨트롤을 완전히 분리
  3. [수정] 한글 오버레이 ㅁㅁㅁ 깨짐 재발 문제
           → 폰트 유효성 검증을 실제 픽셀 렌더링으로 강화
             (DejaVu 등 한글 미지원 폰트가 캐시되는 문제 원천 차단)
─────────────────────────────────────────────────────────────────────────────
"""
...
def _find_unicode_font(size: int = 18):
    ...
    for path in candidates:
        ...
        try:
            ...
            # ★ v11: 실제 픽셀 렌더링으로 한글 지원 여부 확인
            try:
                test_img = _PIL_Image.new("RGB", (size * 6, size * 2), (0, 0, 0))
                test_draw = _PIL_Draw.Draw(test_img)
                test_draw.text((2, 2), _test_text, font=fnt, fill=(255, 255, 255))
                arr = np.array(test_img)
                # 흰색 픽셀(글자)이 충분히 많으면 한글 렌더링 성공
                white_pixels = (arr > 128).sum()
                if white_pixels >= 30:
                    return fnt
                # 픽셀이 부족하면 이 폰트는 한글 미지원 → 다음 후보로
            except Exception:
                # 검증 자체가 실패하면 일단 사용 (구버전 PIL 등)
                return fnt
        except Exception:
            continue

    # 한글 지원 폰트를 못 찾으면 None 반환 (DejaVu fallback 제거 — 어차피 한글 안됨)
    return None
...
class RecorderEngine:
    ...
    def macro_start_recording(self):
        ...
        # ★ v11: auto-click이 켜져있으면 충돌 방지를 위해 일시 중단
        self._ac_was_running = self.auto_click_enabled
        if self.auto_click_enabled:
            self.stop_auto_click()
        self.macro_recording = True
        self.signals.status_message.emit("매크로 기록 준비 중… (auto-click 일시 중단)")
        def _delayed():
            time.sleep(0.5)   # ★ v11: 대기 시간을 늘려 기록 시작 전 클릭이 잡히지 않게
    ...
    def macro_stop_recording(self):
        ...
        def _stop():
            ...
            # ★ v11: 기록 전에 auto-click이 켜져있었으면 복원
            if getattr(self, '_ac_was_running', False):
                self._ac_was_running = False
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v11.0")
