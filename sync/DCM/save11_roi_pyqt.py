...
class RecorderEngine:
    ...
    def save_blackout_clip(self, source: str, timestamp: str):
        ...
        for i, frame in enumerate(all_frames):
            f = frame.copy()
            t_sec = i / fps
            hh = int(t_sec // 3600); mm = int((t_sec % 3600) // 60)
            ss = int(t_sec % 60);    ms = int((t_sec % 1) * 1000)
            cv2.putText(f, f"{hh:02d}:{mm:02d}:{ss:02d}.{ms:03d}", (10, 35),
    ...
    def _pump_preview(self):
        ...
        try:
            ...
            if self._cam_win.isVisible():
                self._cam_win.get_preview_label().update_frame(cf)
        except queue.Empty:
            pass
