# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v10.0
─────────────────────────────────────────────────────────────────────────────
v9 → v10 변경사항:
  1. [신규] EXE 배포 지원 (build_exe.bat / build_exe.sh 동봉)
  2. [신규] 시작 튜토리얼 다이얼로그
           Skip / 일주일 안보기 / 다시는 안보기 — DB 반영
  3. [신규] 메모장 글꼴 크기 조절 (8–36pt 슬라이더 + 프리셋)
  4. [신규] 클릭 매크로 슬롯 (1·2·3… 탭처럼 저장/전환) — DB 반영
  5. [신규] 예약 DateTimeEdit 실시간 카운트다운 + 과거 시각 빨간 경고
           표시/순서 설정에서 해당 섹션이 ON일 때만 타이머 동작
  6. [신규] 표시/순서 설정 기본 순서 변경 + 순서 DB 저장
           Recording→수동녹화→메모→Auto-Click→Schedule→
           Click Macro→매크로예약→Blackout→Log→설정초기화
─────────────────────────────────────────────────────────────────────────────
"""
...
def _find_unicode_font(size: int = 18):
    ...
    if _sys == "Windows":
        try:
            import winreg
            win_fonts_dir = os.path.join(
                os.environ.get("WINDIR", "C:\\Windows"), "Fonts")
            _ko_keywords = ("맑은", "Malgun", "malgun",
                            "나눔", "Nanum", "nanum",
                            "굴림", "Gulim", "바탕", "Batang",
                            "돋움", "Dotum", "궁서", "Gungsuh",
                            "Gothic", "gothic", "고딕")
                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            i += 1
                            if not any(k in name for k in _ko_keywords):
                                continue
                            if os.path.isabs(data):
                                candidates.append(data)
                            else:
                                candidates.append(
                                    os.path.join(win_fonts_dir, data))
                        except OSError:
                            break
        ...
        for fname in [
            ...
            "gulim.ttc", "batang.ttc", "dotum.ttc", "gungsuh.ttc",
        ...
        _mac_ko = [
            "AppleSDGothicNeo.ttc", "AppleSDGothicNeo-Regular.otf",
            "NanumGothic.ttf", "NanumGothicBold.ttf",
            "Arial Unicode.ttf",
        ]
        for d in _mac_dirs:
        ...
        for d in _mac_dirs:
            for p in _glob.glob(os.path.join(d, "*[Kk]orean*")):
                candidates.append(p)
        ...
        _linux_patterns = [
            ...
            "/usr/local/share/fonts/**/Noto*CJK*.ttc",
            "/usr/share/fonts/**/Nanum*.ttf",
            "/usr/share/fonts/**/nanum*.ttf",
            "/usr/share/fonts/**/Un*.ttf",
            "/usr/share/fonts/**/fonts-japanese*.ttf",
            "/usr/share/fonts/**/ipag*.ttf",
        ]
        for pat in _linux_patterns:
            for found in _glob.glob(pat, recursive=True):
    ...
    for path in candidates:
        ...
        try:
            ...
            try:
                bbox = fnt.getbbox(_test_text)
                if bbox and (bbox[2] - bbox[0]) > 5:
                    return fnt
            except AttributeError:
                try:
                    tw, _ = fnt.getsize(_test_text)
                    if tw > 5:
                        return fnt
                except Exception:
                    return fnt
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v10.0")
    ...
    def _build_memo_grp(self) -> QWidget:
        ...
        # ★ v10: 글꼴 크기 조절
        fg=QGroupBox("글꼴 크기"); fl=QHBoxLayout(fg); fl.setSpacing(8)
        fl.addWidget(QLabel("크기:"))
        self._memo_font_spin=QSpinBox(); self._memo_font_spin.setRange(8,36); self._memo_font_spin.setValue(11); self._memo_font_spin.setFixedWidth(52)
        ...
        self._memo_font_sl=QSlider(Qt.Horizontal); self._memo_font_sl.setRange(8,36); self._memo_font_sl.setValue(11)
        ...
        self._memo_font_spin.valueChanged.connect(lambda val: (
            self._memo_font_sl.blockSignals(True), self._memo_font_sl.setValue(val),
            self._memo_font_sl.blockSignals(False), self._apply_memo_font(val)))
        self._memo_font_sl.valueChanged.connect(lambda val: (
            self._memo_font_spin.blockSignals(True), self._memo_font_spin.setValue(val),
            self._memo_font_spin.blockSignals(False), self._apply_memo_font(val)))
        fl.addWidget(self._memo_font_sl, 1); fl.addWidget(self._memo_font_spin)
        for lbl,sz in [("S",10),("M",13),("L",16),("XL",20)]:
            ...
            b.clicked.connect(lambda _,s=sz: self._memo_font_spin.setValue(s)); fl.addWidget(b)
        v.addWidget(fg)
    ...
    def _save_settings(self):
        ...
        db.set("memo_font_size", self._memo_font_spin.value())          # ★ v10
