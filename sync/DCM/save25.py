# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v7.0
─────────────────────────────────────────────────────────────────────────────
v6 → v7 변경사항:
  1. 프로그램 종료 시 녹화 중이면 정상 종료(stop_recording) 후 파일 완성
  2. CameraWindow — 카메라 목록 체크박스 + 접기/펼치기 복원
  3. 종료 시 세팅 DB 저장 / 시작 시 자동 복원 (완전 동기식)
─────────────────────────────────────────────────────────────────────────────
"""
...
class RecorderEngine:
    ...
    def stamp_preview(frame: np.ndarray, engine: "RecorderEngine", source: str) -> np.ndarray:
        if not engine.recording or engine.start_time is None: return frame
        out = frame.copy()
        now = datetime.now()
        now_str = now.strftime("%Y-%m-%d  %H:%M:%S.") + f"{now.microsecond//1000:03d}"
...
def _draw_memo_block(frame: np.ndarray, lines: list[str],
                     position: str, fw: int, fh: int) -> None:
    line_h  = 22; font_sc = 0.52
    box_h   = len(lines) * line_h + 14
    max_len = max(len(l) for l in lines)
    box_w   = min(max_len * 11 + 24, fw - 20); pad = 8
    if position == "top-left":     x0, y0 = pad, pad + 30
    elif position == "top-right":  x0 = fw-box_w-pad; y0 = pad+30
    elif position == "bottom-left":x0 = pad; y0 = fh-box_h-pad
    elif position == "center":     x0 = (fw-box_w)//2; y0 = (fh-box_h)//2
    else:                          x0 = fw-box_w-pad; y0 = fh-box_h-pad
    x0 = max(0, x0); y0 = max(0, y0)
    ov = frame.copy()
    cv2.rectangle(ov,(x0-4,y0-4),(x0+box_w+4,y0+box_h+4),(0,0,0),-1)
    cv2.addWeighted(ov,0.55,frame,0.45,0,frame)
    for j, line in enumerate(lines):
        cy = y0 + j * line_h + line_h
        if cy > fh-4: break
        cv2.putText(frame, line,(x0,cy),cv2.FONT_HERSHEY_SIMPLEX,font_sc,(255,240,100),1,cv2.LINE_AA)
...
class FeatureBar(QFrame):
    ...
    def __init__(self,parent=None):
        ...
        hdr=QHBoxLayout(); t=QLabel("⚙  표시 · 순서 설정"); t.setStyleSheet("color:#7ab4d4;font-size:12px;font-weight:bold;")
        h=QLabel("드래그로 순서 변경"); h.setStyleSheet("color:#446;font-size:10px;")
        ...
        scroll.setStyleSheet(
            ...
            "QScrollBar::add-line:vertical,QScrollBar::sub-line:vertical{height:0;}")
        self._list=FeatureListWidget(); self._list.toggled.connect(self.toggled)
        self._list.order_changed.connect(self.order_changed); scroll.setWidget(self._list)
...
class TimestampMemoEdit(QPlainTextEdit):
    def __init__(self, parent=None):
        super().__init__(parent); self.timestamp_enabled=True
    def mousePressEvent(self, e):
        super().mousePressEvent(e)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v7.0")
    ...
    def _build_ui(self):
        ...
        self._feat_bar=FeatureBar(); self._feat_bar.toggled.connect(self._on_feature_toggle); self._feat_bar.order_changed.connect(self._on_feature_order); rv.addWidget(self._feat_bar)
        ...
        scroll.setWidget(self._panel_w); rv.addWidget(scroll,1)
        root.addLayout(left,1); root.addWidget(rw,0)
    ...
    def _build_recording_grp(self) -> QWidget:
        ...
        self._btn_start=QPushButton("⏺  Start Recording  [Ctrl+Alt+W]"); self._btn_start.setStyleSheet("background:#27ae60;color:white;font-size:12px;padding:8px;border-radius:5px;border:none;"); self._btn_start.clicked.connect(self._on_start_rec)
        self._btn_stop=QPushButton("⏹  Stop Recording  [Ctrl+Alt+E]"); self._btn_stop.setStyleSheet("background:#c0392b;color:white;font-size:12px;padding:8px;border-radius:5px;border:none;"); self._btn_stop.clicked.connect(self._on_stop_rec); self._btn_stop.setEnabled(False)
    ...
    def _build_macro_grp(self) -> QWidget:
        ...
        self._mac_rec_btn.setStyleSheet("QPushButton{background:#1a4a2a;color:#afffcf;border:1px solid #2a8a5a;border-radius:5px;font-size:12px;font-weight:bold;}QPushButton:checked{background:#c0392b;color:#fff;border:1px solid #e74c3c;}")
        ...
        self._mac_run_btn=QPushButton("▶  실행"); self._mac_run_btn.setFixedHeight(32); self._mac_run_btn.setStyleSheet("QPushButton{background:#2980b9;color:#fff;border:none;border-radius:5px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#3498db;}QPushButton:disabled{background:#1a3a5a;color:#555;}"); self._mac_run_btn.clicked.connect(self._on_mac_run)
        self._mac_stop_btn=QPushButton("■  중단"); self._mac_stop_btn.setFixedHeight(32); self._mac_stop_btn.setStyleSheet("QPushButton{background:#7f8c8d;color:#fff;border:none;border-radius:5px;font-size:12px;}QPushButton:hover{background:#95a5a6;}QPushButton:disabled{background:#2a2a2a;color:#555;}"); self._mac_stop_btn.setEnabled(False); self._mac_stop_btn.clicked.connect(self._on_mac_stop)
