# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v14.0
─────────────────────────────────────────────────────────────────────────────
v13 → v14 변경사항:
  [수정] Click Macro 기록 뻗는 버그 완전 해결 (race condition 제거)
    • v13 문제: _emit_loop와 on_click 훅이 _macro_last_ts를 동시 접근
                → race condition으로 간헐적 crash
    • v14 해결: on_click은 (x, y, timestamp) 튜플을 Queue에 put_nowait만
                _emit_loop만 타임스탬프 계산·append·시그널emit 전담
                sentinel 패턴으로 종료 시점 정확히 처리
  [신규] 디스플레이(모니터) 선택 기능
    • 화면 상단 🖥 Display 버튼으로 연결된 모니터 목록 팝업
    • 카메라 선택과 동일한 체크박스 UI
    • 선택 즉시 스크린 스레드 재시작으로 반영
    • mss monitors 인덱스 기반 (0=전체합성, 1~=개별)
─────────────────────────────────────────────────────────────────────────────
"""
...
class RecorderEngine:
    ...
    def macro_start_recording(self):
        ...
        def _delayed():
            ...
            def _emit_loop():
                last_ts = active_ts
                SENTINEL = None   # 종료 신호용 sentinel

                while True:
                    try:
                        item = _q.get(timeout=0.1)
                    except Exception:
                        # 타임아웃: 기록 중이면 계속, 종료됐으면 빠져나옴
                    ...
                    last_ts = t
                    self._macro_last_ts = t   # 참조용 (훅 스레드는 읽기 전용)

                    step = ClickStep(x, y, delay)
                    self.macro_steps.append(step)
                    # 일반 Python 스레드 → PyQt5 시그널 emit 안전
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v14.0")
