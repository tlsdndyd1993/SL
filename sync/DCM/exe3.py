# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v18.0
─────────────────────────────────────────────────────────────────────────────
v17 → v18 변경사항:
  [신규] 메모 탭별 독립 설정
    · 에디터 글꼴 크기: 탭마다 개별 값 유지 (8~36pt)
    · 타임스탬프 삽입 ON/OFF: 탭마다 개별 상태 유지
    · 탭 전환 시 해당 탭의 설정이 컨트롤 패널에 자동 반영
    · DB memo_tabs 테이블에 font_size / ts_enabled 컬럼 추가
      (구버전 DB는 ALTER TABLE로 자동 마이그레이션, 재시작 불필요)
    · 영상 오버레이 글자 크기는 모든 탭 공통 유지
─────────────────────────────────────────────────────────────────────────────
"""
...
class SettingsDB:
    ...
    def save_memo_tabs(self, tabs: list):
        """tabs: list of {title, content, font_size, ts_enabled}"""
        with self._lock:
            with self._conn() as c:
                # 구버전 DB 컬럼 자동 추가
                for col, default in [("font_size", 11), ("ts_enabled", 1)]:
                    try:
                        c.execute(f"ALTER TABLE memo_tabs ADD COLUMN {col} INTEGER DEFAULT {default}")
                    except Exception:
                        pass
                ...
                c.executemany(
                    ...
                    [(t.get('title','메모'), t.get('content',''), i,
                      int(t.get('font_size', 11)),
                      int(bool(t.get('ts_enabled', True))))
        ...
        with self._lock:
            with self._conn() as c:
                ...
                rows = c.execute(
                    ...
                    " FROM memo_tabs ORDER BY sort_order"
                ).fetchall()
        return [{'title': r[0], 'content': r[1],
                 'font_size':  int(r[2]) if r[2] is not None else 11,
    ...
    def _wipe_settings(self):
        """★ v17: settings 테이블 전체 행 삭제 (파일은 유지).
        프로세스가 DB 파일을 열고 있어 os.remove가 불가능할 때 사용."""
        with self._lock:
            with self._conn() as c:
...
class RecorderEngine:
    ...
    def __init__(self, signals: Signals):
        ...
        # ★ v14: 모니터(디스플레이) 선택
        self.monitor_list: list = []     # [{'idx':1,'name':'Display 1','w':1920,'h':1080}, ...]
        self.active_monitor_idx: int = 1 # mss monitors[N] (1-based, 0=전체 합성)
    ...
    # ── 모니터 스캔 ★ v14 ───────────────────────────────────────────────────
    def scan_monitors(self) -> list:
        """mss로 연결된 모니터 목록을 가져옵니다."""
        found = []
        try:
            with mss.mss() as sct:
                # sct.monitors[0] = 전체 합성, [1]~ = 개별 모니터
                for i, m in enumerate(sct.monitors):
                    if i == 0:
                        name = f"전체 합성  ({m['width']}×{m['height']})"
                    else:
    ...
    def macro_start_recording(self):
        ...
        # ★ v14: 훅 스레드 ↔ emit 스레드 사이의 유일한 통신 채널
        # on_click은 이 큐에 (x, y, timestamp) 튜플만 넣고 즉시 반환
        # emit 스레드만 시그널 emit / macro_steps.append / 타임스탬프 계산을 담당
        _q: queue.Queue = queue.Queue()
        self._macro_click_q = _q
        ...
        def _delayed():
            ...
            active_ts = time.time()
            self._macro_listen_active_ts = active_ts
            ...
            def on_click(x, y, button, pressed):
                ...
                t = time.time()
                if t < active_ts: return          # 활성화 이전 클릭 무시
                # put_nowait — 절대 블로킹 없음, 예외도 무시
                try: _q.put_nowait((int(x), int(y), t))
                except Exception: pass

            # ── emit 스레드: 일반 Python 스레드에서 시그널 emit ─────────────
            def _emit_loop():
                last_ts = active_ts
                ...
                while True:
                    try:
                        ...
                        if not self.macro_recording and _q.empty():
                            break
                        continue
                    ...
                    x, y, t = item
                    delay = round(t - last_ts, 3)
                    last_ts = t

                    step = ClickStep(x, y, delay)
                    # ★ v15 수정: 슬롯 전환 시 engine.macro_steps가 새 리스트로
                    # 교체될 수 있으므로 매번 현재 리스트에 append
    ...
    def macro_stop_recording(self):
        self.macro_recording = False
        def _stop():
            time.sleep(0.15)   # emit_loop가 마지막 항목 처리할 시간
    ...
    def start(self):
        self.running = True
        threading.Thread(target=self.scan_monitors, daemon=True).start()  # ★ v14
...
# =============================================================================
#  ★ v14: DisplayWindow — 모니터 선택 다이얼로그
# =============================================================================
class DisplayWindow(QDialog):
    """카메라 선택 창과 같은 패턴으로 모니터(디스플레이) 목록을 보여주고 선택."""
    def __init__(self, engine, signals, parent=None):
        ...
        self.resize(480, 260); self.setMinimumSize(360, 200)
        ...
        hl.addWidget(QLabel("🖥"))
        t = QLabel("디스플레이 선택"); t.setStyleSheet("color:#9ab;font-weight:bold;font-size:13px;")
        hl.addWidget(t); hl.addStretch()
        self._sel_lbl = QLabel("선택: —")
        self._sel_lbl.setStyleSheet("color:#f0c040;font-size:11px;font-weight:bold;")
        hl.addWidget(self._sel_lbl)
        root.addWidget(hdr)

        # 스캔 버튼
        body = QWidget(); body.setStyleSheet("background:#0c0c1e;")
        bv = QVBoxLayout(body); bv.setContentsMargins(12,10,12,10); bv.setSpacing(8)

        scan_row = QHBoxLayout()
        self._scan_btn = QPushButton("🔍  디스플레이 스캔"); self._scan_btn.setFixedHeight(28)
        ...
        self._info_lbl = QLabel("스캔하여 모니터 목록을 불러오세요")
        self._info_lbl.setStyleSheet("color:#556;font-size:10px;font-family:monospace;")
        scan_row.addWidget(self._scan_btn); scan_row.addStretch()
        scan_row.addWidget(self._info_lbl)
        bv.addLayout(scan_row)
    ...
    def _bg_scan(self):
        self.engine.scan_monitors()
        from PyQt5.QtCore import QTimer
        QTimer.singleShot(0, self._populate)

    def _on_scan(self):
        self._scan_btn.setEnabled(False)
        self._info_lbl.setText("스캔 중…")
        threading.Thread(target=self._bg_scan, daemon=True).start()
        from PyQt5.QtCore import QTimer
    ...
    def _populate(self):
        ...
        mons = self.engine.monitor_list
        if not mons:
            lbl = QLabel("  모니터를 찾을 수 없습니다.")
            lbl.setStyleSheet("color:#666;font-size:11px;padding:6px;")
            self._cb_layout.addWidget(lbl)
            self._info_lbl.setText("모니터 없음"); return
        ...
        for m in mons:
            ...
            cb.toggled.connect(lambda checked, idx=m["idx"], c=cb:
                               self._on_toggled(idx, checked, c))
            self._cb_map[m["idx"]] = cb
            self._cb_layout.addWidget(cb)
        self._cb_layout.addStretch()

        n = len(mons)
        self._info_lbl.setText(f"총 {n}개 감지  |  하나만 선택  |  변경 즉시 반영")
        self._update_sel_lbl(self.engine.active_monitor_idx)

    def _on_toggled(self, idx: int, checked: bool, cb):
        if not checked:
            cb.blockSignals(True); cb.setChecked(True); cb.blockSignals(False); return
        if idx == self.engine.active_monitor_idx: return
        # 나머지 해제
        for other_idx, other_cb in self._cb_map.items():
            if other_idx != idx:
                other_cb.blockSignals(True); other_cb.setChecked(False)
                other_cb.blockSignals(False)
        # 스레드 재시작 없이 인덱스만 변경 (루프는 다음 프레임부터 새 모니터 사용)
        self.engine.active_monitor_idx = idx
        self.engine.stop_screen_thread()
        from PyQt5.QtCore import QTimer
        QTimer.singleShot(100, self.engine.start_screen_thread)
        self._update_sel_lbl(idx)
        self.signals.status_message.emit(
            f"디스플레이 변경 → {next((m['name'] for m in self.engine.monitor_list if m['idx']==idx), str(idx))}")

    def _update_sel_lbl(self, idx: int):
        m = next((m for m in self.engine.monitor_list if m["idx"] == idx), None)
        self._sel_lbl.setText(f"선택: {m['name'] if m else f'Display {idx}'}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v18.0")
        ...
        self._disp_win = DisplayWindow(self.engine, self.signals, self)   # ★ v14
    ...
    def _build_ui(self):
        ...
        # ★ v14: Display 선택 버튼
        self._disp_win_btn=QPushButton("🖥 Display"); self._disp_win_btn.setCheckable(True)
        self._disp_win_btn.setStyleSheet("QPushButton{background:#1a3a2a;color:#7be0bc;border:1px solid #2a6a4a;border-radius:13px;font-size:10px;padding:2px 10px;}QPushButton:checked{background:#1a4035;}")
        self._disp_win_btn.toggled.connect(self._on_disp_win_toggle); scr_hdr.addWidget(self._disp_win_btn)
    ...
    def _build_memo_grp(self) -> QWidget:
        ...
        # ── 현재 탭 설정 패널 (탭마다 독립) ─────────────────────────────────
        cur_g = QGroupBox("📝  현재 탭 설정  (탭을 전환하면 해당 탭 값으로 자동 변경)")
        cur_l = QVBoxLayout(cur_g); cur_l.setSpacing(6)

        # 에디터 글꼴 크기
        font_row = QHBoxLayout(); font_row.setSpacing(6)
        font_row.addWidget(QLabel("에디터 글꼴:"))
        self._memo_font_spin = QSpinBox()
        self._memo_font_spin.setRange(8, 36); self._memo_font_spin.setValue(11); self._memo_font_spin.setFixedWidth(52)
        self._memo_font_spin.setStyleSheet("QSpinBox{background:#1a1a3a;color:#f0c040;border:1px solid #4a4a20;border-radius:3px;font-weight:bold;font-size:13px;}")
        self._memo_font_sl = QSlider(Qt.Horizontal)
        self._memo_font_sl.setRange(8, 36); self._memo_font_sl.setValue(11)
        self._memo_font_sl.setStyleSheet(
            "QSlider::groove:horizontal{background:#1a2030;height:6px;border-radius:3px;}"
            "QSlider::handle:horizontal{background:#f0c040;width:16px;height:16px;margin-top:-5px;margin-bottom:-5px;border-radius:8px;}"
            "QSlider::sub-page:horizontal{background:#5a5a20;border-radius:3px;}")
        self._memo_font_spin.valueChanged.connect(
            lambda val: (self._memo_font_sl.blockSignals(True), self._memo_font_sl.setValue(val),
                         self._memo_font_sl.blockSignals(False), self._apply_memo_font_cur(val)))
        self._memo_font_sl.valueChanged.connect(
            lambda val: (self._memo_font_spin.blockSignals(True), self._memo_font_spin.setValue(val),
                         self._memo_font_spin.blockSignals(False), self._apply_memo_font_cur(val)))
        font_row.addWidget(self._memo_font_sl, 1); font_row.addWidget(self._memo_font_spin)
        for lbl, sz in [("S",10),("M",13),("L",16),("XL",20)]:
            ...
            b.clicked.connect(lambda _,s=sz: self._memo_font_spin.setValue(s))
            font_row.addWidget(b)
        cur_l.addLayout(font_row)

        # 타임스탬프 토글
        ts_row = QHBoxLayout(); ts_row.setSpacing(10)
        self._memo_ts_chk = QCheckBox("좌클릭 타임스탬프 삽입")
        self._memo_ts_chk.setChecked(True)
        self._memo_ts_chk.setStyleSheet("font-size:11px;font-weight:bold;color:#7bc8e0;")
        self._memo_ts_chk.toggled.connect(self._on_ts_toggled)
        ts_row.addWidget(self._memo_ts_chk)
        ts_row.addWidget(QLabel("(우클릭: 제거)"))
        ts_row.addStretch()
        cur_l.addLayout(ts_row)
        v.addWidget(cur_g)

        # ── 영상 오버레이 글자 크기 (모든 탭 공통) ─────────────────────────
        og = QGroupBox("🎬  영상 오버레이 글자 크기  (녹화 영상, 모든 탭 공통)")
        ov_l = QHBoxLayout(og); ov_l.setSpacing(6)
        self._overlay_font_spin = QSpinBox()
        self._overlay_font_spin.setRange(8, 72); self._overlay_font_spin.setValue(18); self._overlay_font_spin.setFixedWidth(52)
        self._overlay_font_spin.setStyleSheet("QSpinBox{background:#1a1a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:3px;font-weight:bold;font-size:13px;}")
        self._overlay_font_sl = QSlider(Qt.Horizontal)
        self._overlay_font_sl.setRange(8, 72); self._overlay_font_sl.setValue(18)
        self._overlay_font_sl.setStyleSheet(
            "QSlider::groove:horizontal{background:#1a2030;height:6px;border-radius:3px;}"
            "QSlider::handle:horizontal{background:#7bc8e0;width:16px;height:16px;margin-top:-5px;margin-bottom:-5px;border-radius:8px;}"
            "QSlider::sub-page:horizontal{background:#2a4a6a;border-radius:3px;}")
        self._overlay_font_spin.valueChanged.connect(
            lambda val: (self._overlay_font_sl.blockSignals(True), self._overlay_font_sl.setValue(val),
                         self._overlay_font_sl.blockSignals(False), self._apply_overlay_font(val)))
        self._overlay_font_sl.valueChanged.connect(
            lambda val: (self._overlay_font_spin.blockSignals(True), self._overlay_font_spin.setValue(val),
                         self._overlay_font_spin.blockSignals(False), self._apply_overlay_font(val)))
        ov_l.addWidget(self._overlay_font_sl, 1); ov_l.addWidget(self._overlay_font_spin)
        for lbl, sz in [("S",14),("M",18),("L",24),("XL",32)]:
            ...
            b.clicked.connect(lambda _,s=sz: self._overlay_font_spin.setValue(s))
            ov_l.addWidget(b)
        v.addWidget(og)
        ...
        for lbl, fn, style in [
            ("＋ 탭", self._on_memo_add_tab, "background:#1a3a1a;color:#8fa;border:1px solid #2a6a2a;border-radius:4px;font-size:11px;"),
            ("－ 탭", self._on_memo_del_tab, "background:#3a1a1a;color:#f88;border:1px solid #6a2a2a;border-radius:4px;font-size:11px;"),
            ("현재 탭 지우기", self._on_memo_clear_cur, "background:#1a1a3a;color:#aaa;border:1px solid #334;border-radius:4px;font-size:11px;"),
        ]:
            b = QPushButton(lbl); b.setFixedHeight(26); b.setStyleSheet(style); b.clicked.connect(fn)
        ...
        self._memo_tabs.setStyleSheet(
            ...
            "QTabBar::tab{background:#1a1a3a;color:#888;border:1px solid #334;border-bottom:none;"
            "border-radius:4px 4px 0 0;padding:4px 10px;font-size:11px;min-width:60px;}"
        ...
        add_ov_btn.setStyleSheet("background:#1a2a3a;color:#7bc8e0;border:1px solid #2a4a6a;border-radius:4px;font-size:11px;")
        add_ov_btn.clicked.connect(self._on_add_overlay); ovl.addWidget(add_ov_btn)
        v.addWidget(ov_g)
    ...
    def _build_reset_grp(self) -> QWidget:
        ...
        self._rst_chk_db       = QCheckBox("DB 모든 값 초기화 (settings 테이블 전체 삭제)")
        self._rst_chk_db.setChecked(False)
        self._rst_chk_db.setStyleSheet("color:#e09040;font-weight:bold;")
    ...
    def _on_reset_settings(self):
        ...
        # ★ v17: 파일 삭제 대신 settings 테이블 내용만 삭제
        # (프로세스가 DB 파일을 사용 중이므로 os.remove 불가)
        if do_db:
            try:
                self.db._wipe_settings()
                self._log("[초기화] DB settings 테이블 전체 초기화 완료")
            except Exception as ex:
                self._rst_result_lbl.setText(f"❌ DB 초기화 실패: {ex}")
                self._rst_result_lbl.setStyleSheet("color:#e74c3c;font-size:11px;")
                return
        if do_settings:
            ...
            self._overlay_font_spin.setValue(18)
            # 현재 탭 기본값으로
            self._memo_font_spin.setValue(11)
            self._memo_ts_chk.setChecked(True)
            self._apply_memo_font_cur(11)
            self._on_ts_toggled(True)
    ...
    def _save_settings(self):
        ...
        db.set("overlay_font_size", self._overlay_font_spin.value())
        ...
        # ★ v18: 탭별 font_size / ts_enabled 포함하여 저장
        tabs = []
        for i in range(self._memo_tabs.count()):
            if i >= len(self._memo_editors): break
            ed = self._memo_editors[i]
            ...
            tabs.append({
                ...
                'font_size':  fs,
                'ts_enabled': ed.timestamp_enabled,
            })
        ...
        if tabs:
            ...
            for t in tabs:
                self._add_memo_tab(
                    ...
                    ts_enabled=t.get('ts_enabled', True))
        # 첫 탭 컨트롤 동기화
        self._sync_tab_controls(self._memo_tabs.currentIndex())
        # ★ v10: 매크로 슬롯 복원
    ...
    def _mac_slot_sync(self):
        ...
        ★ v15: 리스트 객체를 교체하지 않고 내용만 교체
               (기록 중인 _emit_loop가 캡처한 참조가 깨지지 않도록)
        """
        if 0 <= self._active_mac_slot < len(self._macro_slots):
            new_steps = self._macro_slots[self._active_mac_slot]['steps']
            self.engine.macro_steps.clear()
            self.engine.macro_steps.extend(new_steps)
        else:
            self.engine.macro_steps.clear()

    def _mac_slot_save_cur(self):
        """현재 engine.macro_steps → 활성 슬롯에 백업 (복사본 저장)."""
        if 0 <= self._active_mac_slot < len(self._macro_slots):
    ...
    def _on_disp_win_toggle(self, checked):   # ★ v14
    ...
    def _mac_append_row(self, step, editable=True):
        """테이블에 ClickStep 한 행 추가."""
        self._mac_tbl.blockSignals(True)
        r = self._mac_tbl.rowCount()
        self._mac_tbl.insertRow(r)
        ...
        idx_it.setFlags(Qt.ItemIsEnabled)
        idx_it.setTextAlignment(Qt.AlignCenter)
        idx_it.setForeground(QColor("#556"))
        self._mac_tbl.setItem(r, 0, idx_it)
    ...
    # Memo
    def _add_memo_tab(self, title, content="", font_size=11, ts_enabled=True):
    ...
    def _on_memo_add_tab(self):
        n = self._memo_tabs.count() + 1
        self._add_memo_tab(f"메모 {n}")
    ...
    def _on_memo_tab_changed(self, idx):
        """탭 전환 시 해당 탭의 설정을 컨트롤 패널에 반영."""
        self._sync_tab_controls(idx)

    def _sync_tab_controls(self, idx: int):
        """에디터 idx의 font_size·ts_enabled → 패널 컨트롤에 반영 (시그널 없이)."""
        if not (0 <= idx < len(self._memo_editors)):
            return
        ed = self._memo_editors[idx]
        fs = ed.font().pointSize()
        if fs <= 0: fs = 11
        for w in (self._memo_font_spin, self._memo_font_sl):
            w.blockSignals(True); w.setValue(fs); w.blockSignals(False)
        self._memo_ts_chk.blockSignals(True)
        self._memo_ts_chk.setChecked(ed.timestamp_enabled)
        self._memo_ts_chk.blockSignals(False)
    ...
    def _on_ts_toggled(self, enabled):
        """현재 탭에만 타임스탬프 설정 적용."""
        idx = self._memo_tabs.currentIndex()
        if 0 <= idx < len(self._memo_editors):
            self._memo_editors[idx].timestamp_enabled = enabled

    def _apply_memo_font_cur(self, size: int):
        """현재 탭에만 에디터 글꼴 크기 적용."""
        from PyQt5.QtGui import QFont
        idx = self._memo_tabs.currentIndex()
        if 0 <= idx < len(self._memo_editors):
            ed = self._memo_editors[idx]
            f = ed.font(); f.setPointSize(max(8, size)); ed.setFont(f)
