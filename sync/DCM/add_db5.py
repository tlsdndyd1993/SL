# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v9.0
─────────────────────────────────────────────────────────────────────────────
v8 → v9 변경사항:
  1. [수정] 한글 오버레이 ㅁㅁㅁ 표시 버그 수정
     - _put_unicode_text: ROI 좌표 계산 오류 수정 (y 기준점 통일)
     - PIL 전체 프레임 렌더링 방식으로 변경 (ROI 슬라이싱 제거)
  2. [수정] 메모장 우클릭 컨텍스트 메뉴(undo/redo 등) 비활성화
  3. [신규] 영상 용량 절감 옵션
     - 코덱 선택: mp4v / avc1(H.264) / xvid
     - 해상도 스케일 (50% / 75% / 100%)
     - 녹화 FPS 상한 설정
  4. [신규] 클릭 매크로 예약 시스템
     - 기존 Schedule에 "매크로 실행" 액션 추가
     - 예약별로 녹화 시작/종료 + 매크로 실행 조합 가능
     - 매크로 예약 목록 별도 관리
─────────────────────────────────────────────────────────────────────────────
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v9.0")
        ...
        # ★ v9: 매크로 예약 목록
        self._macro_schedules: list = []  # list of dict
    ...
    def _build_ui(self):
        ...
        add_sec("macro_schedule",  "🤖  매크로 예약",            "#00b894", self._build_macro_schedule_grp)  # ★ v9
    ...
    def _add_memo_tab(self,title,content=""):
        ed=TimestampMemoEdit(); ed.setPlaceholderText("메모 입력… (좌클릭: 타임스탬프 삽입 | 우클릭: 제거)")
