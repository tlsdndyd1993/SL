# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v15.0
─────────────────────────────────────────────────────────────────────────────
v14 → v15 버그 수정:

  [버그 1] _on_mac_clear 함수 손상으로 즉시 crash
    · 원인: 이전 수정 과정에서 _mac_append_row 바디가 _on_mac_clear 안에
            잘못 합쳐짐. step/editable 변수 미정의 → NameError 즉시 crash
    · 수정: _on_mac_clear 단독 정의로 복원, 중복 제거

  [버그 2] 슬롯 전환 시 engine.macro_steps 리스트 객체 교체
    · 원인: _mac_slot_sync()가 engine.macro_steps = 새리스트 대입
            _emit_loop는 시작 시점 리스트 참조를 클로저로 캡처하므로
            슬롯 전환 후 append가 버려진 리스트에 쌓여 UI 미반영
    · 수정: .clear() + .extend()로 객체 동일성 유지하며 내용만 교체

  [버그 3] _on_mac_clear 중복 정의
    · 원인: 같은 메서드가 두 곳에 정의, Python은 마지막 정의 사용
            마지막 것은 _mac_slot_info_upd() 호출 없어 슬롯 카운트 미갱신
    · 수정: 중복 정의 제거, 올바른 단일 정의 유지
─────────────────────────────────────────────────────────────────────────────
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v15.0")
