# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v16.0
─────────────────────────────────────────────────────────────────────────────
v15 → v16 버그 수정:

  [버그] _mac_append_row / _make_mac_editable 두 메서드가 뒤섞여 손상
    · 증상 1: 기록 중 클릭 스텝이 테이블에 표시 안 됨
              (_mac_append_row가 insertRow 없이 editable 처리만 함)
    · 증상 2: 기록 중단 버튼 누르면 crash
              (_make_mac_editable 메서드 자체가 존재하지 않아 AttributeError)
    · 원인: 이전 수정 과정에서 두 메서드 바디가 잘못 합쳐짐
    · 수정: v8 원본 기반으로 두 메서드를 완전히 새로 정의
            _mac_append_row: insertRow → 셀 생성 → scrollToBottom
            _make_mac_editable: 기존 행 전체 editable 플래그 설정
─────────────────────────────────────────────────────────────────────────────
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v16.0")
