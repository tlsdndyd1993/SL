# -*- coding: utf-8 -*-
"""
Screen & Camera Recorder  v12.0
─────────────────────────────────────────────────────────────────────────────
v11 → v12 변경사항:
  [수정 1] 한글 오버레이 ㅁㅁㅁ 완전 해결 — Missing Glyph 근본 원인 수정
    • 원인: PIL이 DejaVu 등 한글 글리프 없는 폰트를 로드했을 때
            getbbox()가 0이 아닌 값(대체 글리프 크기)을 반환해서
            한글 지원 폰트로 잘못 판정 → □(ㅁ) 렌더링
    • 해결: 실제 픽셀 렌더링 후 비한글 레퍼런스(ASCII 전용 폰트)와
            픽셀 분포를 비교해 진짜 한글 글리프 보유 여부 검증
            + 윈도우 레지스트리 경유 폰트 경로를 1순위로 탐색
  [수정 2] Auto-Click ON 상태 매크로 기록 충돌(프로그램 뻗음) 완전 해결
    • 원인: mc.click()이 OS 레벨 synthetic 이벤트를 발생시키고
            pynput Listener가 이를 잡아 on_click → PyQt 시그널 emit →
            UI 스레드에서 deadlock 발생
    • 해결: v8 방식 참고 — Listener 생성 시 suppress=False 유지하되
            클릭 이벤트가 auto_click 루프에서 온 synthetic 이벤트인지
            타임스탬프로 필터링. auto_click 중단 없이 공존 가능.
─────────────────────────────────────────────────────────────────────────────
"""
    ...
    def macro_start_recording(self):
        ...
        def _delayed():
            ...
            def on_click(x, y, button, pressed):
                # ★ v12: pressed=True, 왼쪽 버튼만
                if not pressed or button != pynput_mouse.Button.left: return
                if not self.macro_recording: return
                now = time.time()
                # ★ v12: 활성화 시각 이전 이벤트 무시 (기록 시작 클릭 자체 제외)
                if now < self._macro_listen_active_ts: return
                # ★ v12: auto_click이 synthetic 이벤트를 만들 때의 간격 필터
                # auto_click 인터벌이 0.1s 이상이므로, 2ms 이내 연속 이벤트는 synthetic
                if now - self._macro_last_ts < 0.002: return
            ...
            # ★ v12: v8 방식 그대로 — suppress=False, on_click 콜백만
            self._macro_listener = pynput_mouse.Listener(on_click=on_click)
            self._macro_listener.start()
            self.signals.status_message.emit("매크로 기록 중 (Auto-Click과 공존 가능)")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen & Camera Recorder  v12.0")
